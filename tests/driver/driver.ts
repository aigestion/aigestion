...
export abstract class GenericTrunkDriver {
  ...
  runTrunkSync(args: string[], execOptions?: ExecFileOptions) {
    const [bin, execArgs, options] = this.buildExecArgs(args, execOptions);
    return execFileSync(bin, execArgs, options as ExecFileSyncOptions);
  }
  ...
  runTrunkAsync(args: string[], execOptions?: ExecFileOptions) {
    const [bin, execArgs, options] = this.buildExecArgs(args, execOptions);
    return execFile(bin, execArgs, options, null);
  }
