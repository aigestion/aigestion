...
export const DanielaOmniWidget: React.FC = () => {
    ...
    const { isListening, transcript, speak, toggleListening } = useDanielaVoice();
    ...
    return (
        <div className="fixed bottom-8 right-8 z-[9000] font-sans">
            <AnimatePresence>
                {isExpanded && (
                    <motion.div
                        ...
                        className="absolute bottom-20 right-0 w-80 premium-glass p-6 rounded-3xl mb-4 border border-nexus-cyan/30 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
                    >
                        ...
                        <div className="min-h-[100px] text-sm text-nexus-silver">
                            {transcript ? (
                                <p className="italic text-white">"{transcript}"</p>
                            ) : (
                                <p className="opacity-50">Esperando comando de voz...</p>
                            )}
                        </div>
                        ...
                        <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-center">
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${isListening ? 'bg-red-500/20 text-red-400 animate-pulse' : 'bg-white/10 hover:bg-white/20'}`}
                            >
                                {isListening ? '● ESCUCHANDO' : '🎙️ ACTIVAR VOZ'}
                            </button>
                        </div>
                    </motion.div>
                )}
            ...
            >
                <div className="absolute inset-0 bg-gradient-to-br from-nexus-violet to-nexus-cyan rounded-full blur-md opacity-70 group-hover:opacity-100 transition-opacity animate-pulse-glow" />
