...
function analyzeBundle() {
  ...
  if (mainBundle) {
    ...
    console.log('\n📦 Main Bundle:')
    console.log(`📄 ${mainBundle}: ${formatBytes(mainBundleSize)} (${formatBytes(mainBundleGzipped)} gzipped)`)
    ...
    if (mainBundleSize > BUNDLE_LIMITS.main) {
      ...
      console.log(`✅ Main bundle within limit (${formatBytes(BUNDLE_LIMITS.main - mainBundleSize)} remaining)`)
    }
  }
  ...
  if (totalSize > BUNDLE_LIMITS.total) {
    ...
    console.log(`✅ Total bundle within limit (${formatBytes(BUNDLE_LIMITS.total - totalSize)} remaining)`)
  }
