...
class PerformanceMonitor {
  ...
  generateReport(): {
    ...
    if (this.metrics.length === 0) {
      return {
        ...
        vitals: { avg: { lcp: 0, fid: 0, cls: 0, fcp: 0, ttfb: 0 }, p75: { lcp: 0, fid: 0, cls: 0, fcp: 0, ttfb: 0 }, p95: { lcp: 0, fid: 0, cls: 0, fcp: 0, ttfb: 0 } },
    ...
    const vitals = {
      avg: { lcp: lcpStats.avg, fid: fidStats.avg, cls: clsStats.avg, fcp: fcpStats.avg, ttfb: ttfbStats.avg },
      p75: { lcp: lcpStats.p75, fid: fidStats.p75, cls: clsStats.p75, fcp: fcpStats.p75, ttfb: ttfbStats.p75 },
      p95: { lcp: lcpStats.p95, fid: fidStats.p95, cls: clsStats.p95, fcp: fcpStats.p95, ttfb: ttfbStats.p95 },
    }

    // Generar recomendaciones
    const recommendations: string[] = []

    if (vitals.avg.lcp > 2500) recommendations.push('Optimizar Largest Contentful Paint (LCP > 2.5s)')
    ...
    if (vitals.avg.fcp > 1800) recommendations.push('Optimizar First Contentful Paint (FCP > 1.8s)')
   ...
   */
  private getPerformanceStatus(vitals: WebVitals): 'excellent' | 'good' | 'needs_improvement' | 'poor' {
    let score = 0
