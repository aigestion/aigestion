...
const plans = [
  ...
  },
  {
    ...
    features: ['Consultas Ilimitadas', 'Prioridad de Proceso', 'Soporte Prioritario', 'Almacenamiento 50GB', 'Acceso a Herramientas Beta'],
    color: 'nexus-cyan',
    icon: Zap,
    popular: true
  },
  {
    ...
    features: ['Infraestructura Dedicada', 'Soporte 24/7 Dedicado', 'Almacenamiento Ilimitado', 'Acceso API Completo', 'Consultoría Personalizada', 'Hardware Integration'],
    color: 'nexus-violet',
    icon: Crown
  }
];
            ...
            return (
                <motion.div
                  ...
                  className={`premium-glass p-8 rounded-2xl border transition-all duration-300 relative group flex flex-col ${
                      plan.popular ? 'border-nexus-cyan/50 shadow-[0_0_30px_rgba(0,245,255,0.1)]' : 'border-white/5'
                ...
                >
                    ...
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 bg-${plan.color === 'nexus-violet' ? 'nexus-violet' : plan.color === 'nexus-cyan' ? 'nexus-cyan' : 'white'}/10`}>
                        <Icon className={`h-6 w-6 text-${plan.color === 'nexus-silver' ? 'white' : plan.color}`} />
                    </div>
                    ...
                    <ul className="space-y-4 mb-8 flex-grow">
                        {plan.features.map((feature) => (
                            <li key={feature} className="flex items-start text-sm text-nexus-silver/80">
                                <Check className={`h-4 w-4 mr-3 shrink-0 ${plan.color === 'nexus-violet' ? 'text-nexus-violet' : 'text-nexus-cyan'}`} />
                                {feature}
                            </li>
                        ))}
                    </ul>
