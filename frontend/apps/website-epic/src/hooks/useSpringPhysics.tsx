...
export function useElasticButton(
  options: { readonly scale?: number; readonly config?: SpringConfig } = {}
) {
...
export function SpringPhysics({
  ...
  return (
    <motion.div className={className} style={style} {...handlers}>
      {children}
    </motion.div>
  );
}
...
export function ElasticButton({
  ...
  return (
    <motion.button className={className} style={style} {...handlers} onClick={onClick}>
      {children}
    </motion.button>
  );
}
