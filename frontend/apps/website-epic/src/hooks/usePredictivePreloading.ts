...
export function usePredictivePreloading(options: PredictivePreloaderOptions = {}) {
  ...
  const analyzeBehaviorPatterns = useCallback(
    (behaviors: UserBehavior[]) => {
      ...
      if (enableScrollPreloading) {
        ...
        const scrollFrequency = scrollPatterns.reduce(
          (acc, target) => {
      ...
      if (enableNavigationPrediction) {
        const navigationPatterns = behaviors.filter(b => b.action === 'navigation').slice(-20); // Last 20 navigations
        ...
        if (navigationPatterns.length > 1) {
          for (let i = 1; i < navigationPatterns.length; i++) {
            ...
            navigationPatternsRef.current.set(
              pattern,
  ...
  const executePreloads = useCallback(
  ...
  const preloadComponent = useCallback(
    (componentName: string, urgency: 'low' | 'medium' | 'high') => {
...
export function useComponentPreload(
  ...
  options: { readonly preloadOnHover?: boolean } = {}
) {
