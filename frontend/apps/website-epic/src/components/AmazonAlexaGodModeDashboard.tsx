  ...
  const getServiceIcon = (serviceType: string) => {
    switch (serviceType) {
      ...
      case 'video':
        return <Video className="w-5 h-5" />;
      case 'shopping':
        return <Shopping className="w-5 h-5" />;
      case 'kindle':
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-blue-200 text-xs">
                    {stats.skills.total_invocations} invocaciones
                  </p>
                </div>
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`p-2 rounded-lg ${device.status === 'online' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}
                    >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <div
                      className={`px-2 py-1 rounded text-xs ${device.bluetooth_connected ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}
                    >
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <div
                      className={`px-2 py-1 rounded text-xs ${device.do_not_disturb ? 'bg-orange-100 text-orange-700' : 'bg-gray-100 text-gray-700'}`}
                    >
            ...
            {skills.map(skill => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`p-2 rounded-lg ${skill.enabled ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'}`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Invocación: "Alexa, {skill.invocation_name}"
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div
                      className={`px-2 py-1 rounded-full text-xs font-medium ${skill.enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}
                    >
                      {skill.enabled ? 'Activo' : 'Inactivo'}
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-gray-500">
                        {skill.usage_stats.total_invocations} usos
                      </p>
                      <p className="text-xs text-gray-500">
                        {skill.usage_stats.daily_users} usuarios/día
                      </p>
                    </div>
                  </div>
                </div>
            ...
            {services.map(service => (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`p-2 rounded-lg ${service.subscription_active ? 'bg-purple-100 text-purple-600' : 'bg-gray-100 text-gray-600'}`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {service.subscription_tier.toUpperCase()}
                      </p>
                    </div>
                  </div>
                  <div
                    className={`px-2 py-1 rounded-full text-xs font-medium ${service.subscription_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                  >
...
const Headphones = ({ className }: { className?: string }) => (
  <svg className={className} fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M3 5a7 7 0 0114 0v7a7 7 0 01-14 0V5zM7 12h10M7 12l-2 2m2-2l2 2m6-2l2 2m-2-2l-2 2"
    />
  </svg>
);
