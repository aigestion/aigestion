...
export function useABTesting(config: ABTestConfig = {}) {
  ...
  const saveAssignments = useCallback((assignmentsMap: Map<string, ABTestResult>) => {
    if (!persistToLocalStorage) return;
  ...
  const isInTargetAudience = useCallback((test: ABTest): boolean => {
    ...
    const { country, device, browser, customAttributes } = test.targetAudience;
  ...
  const selectVariant = useCallback((test: ABTest): ABTestVariant | null => {
  ...
  const getAssignment = useCallback((testId: string): ABTestResult | null => {
  ...
  // Track impression
  const trackImpression = useCallback(async (assignment: ABTestResult) => {
    if (!endpoint) return;
  ...
  // Get variant config
  const getVariantConfig = useCallback((testId: string): any => {
    const assignment = assignments.get(testId);
  ...
  // Get test metrics
  const getTestMetrics = useCallback(async (testId: string): Promise<ABTestMetrics[]> => {
    if (!endpoint) return [];
...
export function useABTest(testId: string) {
  ...
  const trackTestConversion = useCallback((value?: number, data?: Record<string, any>) => {
            ...
            <div key={test.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {test.variants.map(variant => {
                  ...
                  return (
                    <div key={variant.id} className="text-center p-3 bg-gray-50 dark:bg-gray-700 rounded">
