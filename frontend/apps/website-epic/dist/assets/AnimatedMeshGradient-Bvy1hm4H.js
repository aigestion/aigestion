import { j as e } from './vendor-3d-BTgeB28l.js';
import { m as s } from './vendor-motion-I6wcTx_q.js';
import './vendor-react-DzSuaLpV.js';
const t = () =>
  e.jsxs('div', {
    className: 'fixed inset-0 -z-50 overflow-hidden pointer-events-none',
    children: [
      e.jsx(s.div, {
        animate: { scale: [1, 1.2, 1], rotate: [0, 45, 0], x: [0, 100, 0], y: [0, -50, 0] },
        transition: { duration: 20, repeat: 1 / 0, ease: 'easeInOut' },
        className:
          'absolute top-[-20%] left-[-10%] w-[70%] h-[70%] bg-nexus-violet/5 blur-[120px] rounded-full',
      }),
      e.jsx(s.div, {
        animate: { scale: [1.2, 1, 1.2], rotate: [0, -30, 0], x: [0, -100, 0], y: [0, 50, 0] },
        transition: { duration: 25, repeat: 1 / 0, ease: 'easeInOut' },
        className:
          'absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-nexus-obsidian/5 blur-[100px] rounded-full',
      }),
      e.jsx('div', { className: 'absolute inset-0 bg-nexus-obsidian/40 backdrop-blur-[20px]' }),
    ],
  });
export { t as AnimatedMeshGradient };
