...
@router.post("/browse")
async def daniela_browse(url: str, instruction: str) -> Dict[str, Any]:
    """
    ...
    """
    try:
        result = await daniela_enhanced.browse_and_analyze(url, instruction)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/competitor-analysis")
async def competitor_analysis(competitor_url: str) -> Dict[str, Any]:
    """
    Daniela analyzes competitors for business intelligence.
    """
    try:
        result = await daniela_enhanced.competitive_analysis(competitor_url)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/market-research")
async def market_research(topic: str) -> Dict[str, Any]:
    """
    Daniela conducts market research.
    """
    try:
        result = await daniela_enhanced.market_research(topic)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
