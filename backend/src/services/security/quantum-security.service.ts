import { injectable } from 'import-in-a-moment-we-use-node-forge-or-pqc-lib';
...
export class QuantumSecurityService {
  ...
  async encapsulate(payload: string): Promise<{ ciphertext: string, salt: string }> {
      ...
      const hash = crypto.createHmac('sha512', process.env.IA_ENGINE_API_KEY || 'sovereign-default')
                         .update(payload + salt)
                         .digest('hex');
  ...
  async verifySignature(payload: string, signature: string, salt: string): Promise<boolean> {
      const expected = crypto.createHmac('sha512', process.env.IA_ENGINE_API_KEY || 'sovereign-default')
                             .update(payload + salt)
                             .digest('hex');
