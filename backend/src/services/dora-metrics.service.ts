...
export class DoraMetricsService {
    ...
    async recordFailureAndRestoration(failureTime: Date, restorationTime: Date): Promise<void> {
        ...
        logger.info(`[DoraMetrics] Failure recorded. Restoration took ${restorationDuration.toFixed(2)} hours.`);
    }
    ...
    async getMetrics(): Promise<DoraMetrics> {
        const client = getRedisClient();
        if (!client?.isOpen) return { deploymentFrequency: 0, leadTimeForChanges: 0, changeFailureRate: 0, timeToRestoreService: 0 };
        ...
        return {
            ...
            changeFailureRate: (Number.parseInt(stats.failures_count || '0') / (Number.parseInt(stats.deployments_count || '1'))) * 100,
