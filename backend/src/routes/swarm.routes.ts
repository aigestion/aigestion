...
swarmRouter.post('/tool-call', validateBody(schemas.swarm.toolCall), (req, res, next) =>
  controller.executeTool(req, res, next),
);
swarmRouter.get('/god-state', (req, res, next) => controller.getGodState(req, res, next));
swarmRouter.post('/missions', validateBody(schemas.swarm.createMission), (req, res, next) =>
