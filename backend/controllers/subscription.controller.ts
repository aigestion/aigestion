...
export const getUserSubscription = async (req: Request, res: Response) => {
  try {
    ...
    if (role !== 'admin' && userId !== adminId) {
      return res.status(403).json(buildResponse(null, 'Access denied', 403));
    }
    ...
    if (!subscription) {
      const freePlan = await Subscription.findOne({ name: 'free' }).exec();
    ...
    res.status(500).json(buildResponse(null, 'Internal server error', 500));
  }
};
...
export const validateSubscriptionAccess = async (req: Request, res: Response) => {
  try {
    ...
    res.json(buildResponse(validation));
  } catch (error) {
...
export const validateMobileSubscription = async (req: Request, res: Response) => {
  try {
    ...
    res.json(buildResponse(mobileData));
  } catch (error) {
...
export const createPaymentSession = async (req: Request, res: Response) => {
  try {
    ...
    // Get plan details
    const plan = await Subscription.findOne({ id: planId }).exec();
    if (!plan) {
...
export const updateSubscription = async (req: Request, res: Response) => {
  try {
    ...
    res.json(buildResponse(subscription));
  } catch (error) {
...
async function handleSubscriptionCancellation(stripeSubscription: Stripe.Subscription) {
  try {
    ...
    if (subscription) {
      ...
      await subscription.save();
      console.log(`[Subscription] Cancellation processed for subscription ${stripeSubscription.id}`);
    }
  } catch (error) {
