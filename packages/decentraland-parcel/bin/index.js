...
var DanielaMetaverseIntegration = class {
  ...
  async processUserQuery(query, userId) {
    ...
    const responses = {
      ...
      default:
        'Entendido tu consulta. Como IA de AIGestion, estoy aqu\xED para optimizar tus procesos y proporcionar insights estrat\xE9gicos.',
    };
...
function createDanielaAICenter() {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    import_ecs2.Transform.create(orb, {
      position: import_math2.Vector3.create(
        6 + i * 1.5,
        2 + Math.sin(i) * 0.5,
        6 + Math.cos(i) * 1.5
      ),
