...
export class WeatherSystem {
  ...
  private updateCelestialBodies() {
    this.celestialBodies.forEach((body) => {
      ...
      transform.scale = isVisible ?
        Vector3.create(body.type === 'sun' ? 3 : 2, body.type === 'sun' ? 3 : 2, body.type === 'sun' ? 3 : 2) :
  ...
  private applyWeatherToSkyColor(baseColor: Color4): Color4 {
    switch (this.currentState.type) {
      case 'cloudy':
        return Color4.lerp(baseColor, Color4.create(0.6, 0.6, 0.6, 1), this.currentState.intensity * 0.7)
      case 'rainy':
      case 'stormy':
        return Color4.lerp(baseColor, Color4.create(0.3, 0.3, 0.4, 1), this.currentState.intensity * 0.8)
      case 'foggy':
        return Color4.lerp(baseColor, Color4.create(0.7, 0.7, 0.75, 1), this.currentState.intensity * 0.6)
      default:
        return baseColor
    }
  }
  ...
  private updateWeatherEffects() {
    this.weatherEffects.forEach(effect => {
      ...
      transform.scale = shouldBeVisible ?
  ...
  public changeWeather(newWeather?: Partial<WeatherState>) {
    ...
    console.log(`🌤️ Weather changed to: ${this.currentState.type} (intensity: ${this.currentState.intensity.toFixed(2)})`)
  }
