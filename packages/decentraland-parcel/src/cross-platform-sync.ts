...
export class CrossPlatformSyncSystem {
  ...
  private getPlatformColor(platform: Platform): Color4 {
    switch (platform) {
      case 'web':
        return Color4.create(0.2, 0.6, 1, 1);
      case 'mobile':
        return Color4.create(0.8, 0.4, 0.2, 1);
      case 'desktop':
        return Color4.create(0.2, 0.8, 0.4, 1);
      case 'vr':
        return Color4.create(0.8, 0.2, 0.8, 1);
      case 'ar':
        return Color4.create(0.2, 0.8, 0.8, 1);
      case 'console':
        return Color4.create(0.8, 0.8, 0.2, 1);
      default:
  ...
  private updateSyncStatus() {
    ...
    console.log(
      `📊 Sync Status: ${this.currentSession.status} | Entities: ${totalEntities} | Conflicts: ${pendingConflicts} | Queue: ${queueSize}`
    );
  }
