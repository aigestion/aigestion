    ...
    for (let x = 0; x < gridSize; x++) {
      for (let y = 0; y < gridSize; y++) {
        ...
        Transform.create(gridCell, {
          parent: this.entity,
          position: Vector3.create(-7.5 + x * gridSpacing, -3.5 + y * gridSpacing, 0.1),
  ...
  private renderBarChart(chartId: string, config: ChartConfig) {
    ...
    config.data.forEach((point, index) => {
      ...
      Transform.create(bar, {
        parent: this.entity,
        position: Vector3.create(startX + index * spacing, -2 + height / 2, 0.2),
      ...
      Transform.create(label, {
        parent: this.entity,
        position: Vector3.create(startX + index * spacing, -3.5, 0.3),
    ...
    for (let x = 0; x < gridSize; x++) {
      for (let y = 0; y < gridSize; y++) {
        ...
        Transform.create(heatCell, {
          parent: this.entity,
          position: Vector3.create(-6 + x * cellSize, -2 + y * cellSize, 0.2),
