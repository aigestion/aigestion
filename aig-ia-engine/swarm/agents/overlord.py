...
class Overlord(BaseAgent):
    ...
    def process_message(self, message: Message):
        # 0. Recursive Reasoning Trigger
        if message.msg_type == MessageType.TASK_START:
            directive = str(message.content).upper()
            if "RECURSIVE" in directive:
                self.log("automatic_recursive_reasoning_triggered")
                self.delegate_task(
                    "Sub-mission generated by recursive reasoning: "
                    + str(message.content),
                    metadata={"reason": "Complex task requiring multi-stage enquiry"},
                )
    ...
    def delegate_task(self, description: str, metadata: dict = None):
        """Triggers a recursive sub-mission via MissionControl."""
        self.log("delegating_sub_mission", description=description)
        sub_metadata = metadata or {}
        # The MISSION_CONTROL agent in SwarmService will bridge this back to trigger_mission
        self.send_message(
            AgentType.MISSION_CONTROL,
            {"description": description, "metadata": sub_metadata},
            MessageType.SUB_MISSION_TRIGGER,
        )
