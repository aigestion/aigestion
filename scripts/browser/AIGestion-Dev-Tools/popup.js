...
class AIGestionPopup {
    ...
    async updateStatus() {
        ...
        const isAIGestion = this.currentTab.url.includes('aigestion.net') || 
                           this.currentTab.url.includes('localhost');
    ...
    displayMetrics(metrics) {
        ...
        if (metrics.memory) {
            document.getElementById('memoryValue').textContent = `${Math.round(metrics.memory / 1048576)}MB`;
        }
