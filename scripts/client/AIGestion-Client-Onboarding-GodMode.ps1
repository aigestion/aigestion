...
function Get-BestContactChannel {
    ...
    $channels = $channels | Sort-Object -Property @{Expression={$_.Priority}; Ascending}, @{Expression={$_.Confidence}; Descending}
...
function Start-InteractiveMode {
    ...
    if ($confirm -eq 'S' -or $confirm -eq 's') {
        # Crear perfil
        $profile = New-ClientProfile -ClientData $clientData
        
        Write-Host "`n🎉 Perfil creado exitosamente!" -ForegroundColor Green
        Write-Host "📋 ID de Cliente: $($profile.ClientId)" -ForegroundColor White
        Write-Host "📊 Perfil IA: $($profile.AIProfile.CommunicationStyle) - $($profile.AIProfile.TechSavviness)" -ForegroundColor Cyan
        
        # Enviar bienvenida
        $sendWelcome = Read-Host "📤 ¿Desea enviar mensaje de bienvenida ahora? (S/N)"
        if ($sendWelcome -eq 'S' -or $sendWelcome -eq 's') {
            Send-WelcomeMessage -ClientProfile $profile
            Write-Host "✅ Mensaje de bienvenida enviado" -ForegroundColor Green
        }
        
        # Programar seguimiento
        $profile.NextFollowUp = (Get-Date).AddDays(3)
        Update-ClientProfile -Profile $profile
        Write-Host "📅 Seguimiento programado para: $($profile.NextFollowUp)" -ForegroundColor Yellow
