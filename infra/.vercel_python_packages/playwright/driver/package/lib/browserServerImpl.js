...
class BrowserServerLauncherImpl {
  ...
  async launchServer(options = {}) {
    const playwright = (0, import_playwright.createPlaywright)({
    ...
    const validatorContext = {
      tChannelImpl: (names, arg, path2) => {
        throw new validatorPrimitives.ValidationError(
          `${path2}: channels are not expected in launchServer`
        );
      },
      binary: 'buffer',
    ...
    try {
      ...
      browser = await controller.run(async progress => {
        if (options._userDataDir !== void 0) {
          ...
          launchOptions = validator(
            { ...launchOptions, userDataDir: options._userDataDir },
            '',
            validatorContext
          );
          const context = await playwright[this._browserName].launchPersistentContext(
            progress,
            options._userDataDir,
            launchOptions
          );
          ...
          return await playwright[this._browserName].launch(
            progress,
            launchOptions,
    ...
    const path = options.wsPath
      ? options.wsPath.startsWith('/')
        ? options.wsPath
        : `/${options.wsPath}`
      : `/${(0, import_crypto.createGuid)()}`;
    const server = new import_playwrightServer.PlaywrightServer({
