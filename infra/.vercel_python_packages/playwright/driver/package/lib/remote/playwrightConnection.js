...
class PlaywrightConnection {
  constructor(semaphore, ws, controller, playwright, initialize, id) {
      ...
      if (ws.readyState !== ws.CLOSING) {
        ...
        if (import_debugLogger.debugLogger.isEnabled('server:channel'))
          import_debugLogger.debugLogger.log(
            'server:channel',
            `[${this._id}] ${(0, import_utils.monotonicTime)() * 1e3} SEND \u25BA ${messageString}`
          );
    ...
    ws.on('message', async message => {
      ...
      if (import_debugLogger.debugLogger.isEnabled('server:channel'))
        import_debugLogger.debugLogger.log(
          'server:channel',
          `[${this._id}] ${(0, import_utils.monotonicTime)() * 1e3} \u25C0 RECV ${messageString}`
        );
      if (import_debugLogger.debugLogger.isEnabled('server:metadata'))
    ...
    if (controller) {
      ...
      this._root = new import_debugControllerDispatcher.DebugControllerDispatcher(
        this._dispatcherConnection,
        playwright.debugController
      );
      return;
    }
    this._root = new import_server.RootDispatcher(
      this._dispatcherConnection,
      async (scope, params) => {
        ...
        if (options.dispose) this._cleanups.push(options.dispose);
  ...
  async close(reason) {
    ...
    import_debugLogger.debugLogger.log(
      'server',
      `[${this._id}] force closing connection: ${reason?.reason || ''} (${reason?.code || 0})`
    );
    try {
