...
class Android extends import_channelOwner.ChannelOwner {
  ...
  async connect(wsEndpoint, options = {}) {
    return await this._wrapApiCall(async () => {
      ...
      const connection = await (0, import_webSocket.connectOverWebSocket)(
        this._connection,
        connectParams
      );
      let device;
...
class AndroidDevice extends import_channelOwner.ChannelOwner {
  ...
  _onWebViewRemoved(socketName) {
    ...
    if (view) view.emit(import_events.Events.AndroidWebView.Close);
  }
  setDefaultTimeout(timeout) {
    this._timeoutSettings.setDefaultTimeout(timeout);
  }
  ...
  async wait(selector, options = {}) {
    await this._channel.wait({
      androidSelector: toSelectorChannel(selector),
      ...options,
  ...
  async launchBrowser(options = {}) {
    ...
    context.once(import_events.Events.BrowserContext.Close, () =>
      selectors._contextsForSelectors.delete(context)
    );
    ...
    return await this._wrapApiCall(async () => {
      ...
      if (event !== import_events.Events.AndroidDevice.Close)
        waiter.rejectOnEvent(
          this,
          import_events.Events.AndroidDevice.Close,
          () => new import_errors.TargetClosedError()
        );
...
class AndroidSocket extends import_channelOwner.ChannelOwner {
  ...
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on('data', ({ data }) =>
      this.emit(import_events.Events.AndroidSocket.Data, data)
    );
...
async function loadFile(platform, file) {
  if ((0, import_rtti.isString)(file)) return await platform.fs().promises.readFile(file);
  return file;
}
class AndroidInput {
  constructor(device) {
...
function toSelectorChannel(selector) {
  ...
  const toRegex = value => {
    ...
    if ((0, import_rtti.isRegExp)(value)) return value.source;
  ...
  return {
    ...
    hasDescendant: hasDescendant
      ? {
          androidSelector: toSelectorChannel(hasDescendant.selector),
