...
class BrowserType extends import_channelOwner.ChannelOwner {
  ...
  async launch(options = {}) {
    ...
    return await this._wrapApiCall(async () => {
      const browser = import_browser.Browser.from(
        (await this._channel.launch(launchOptions)).browser
      );
  ...
  async launchPersistentContext(userDataDir, options = {}) {
    ...
    const logger = options.logger || this._playwright._defaultLaunchOptions?.logger;
    const contextParams = await (0, import_browserContext.prepareBrowserContextParams)(
      this._platform,
      options
    );
    const persistentParams = {
      ...
      userDataDir:
        this._platform.path().isAbsolute(userDataDir) || !userDataDir
          ? userDataDir
          : this._platform.path().resolve(userDataDir),
      timeout: new import_timeoutSettings.TimeoutSettings(this._platform).launchTimeout(options),
    };
  ...
  async _connectOverCDP(endpointURL, params = {}) {
    ...
    const headers = params.headers
      ? (0, import_headers.headersObjectToArray)(params.headers)
      : void 0;
    ...
    if (result.defaultContext)
      await this._instrumentation.runAfterCreateBrowserContext(
        import_browserContext.BrowserContext.from(result.defaultContext)
      );
    return browser;
  }
}
