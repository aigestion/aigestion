...
class Page extends import_channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on('download', ({ url, suggestedFilename, artifact }) => {
      ...
      this.emit(
        import_events.Events.Page.Download,
        new import_download.Download(this, url, suggestedFilename, artifactObject)
      );
    });
    this._channel.on('fileChooser', ({ element, isMultiple }) =>
      this.emit(
        import_events.Events.Page.FileChooser,
        new import_fileChooser.FileChooser(
          this,
          import_elementHandle.ElementHandle.from(element),
          isMultiple
        )
      )
    );
    this._channel.on('frameAttached', ({ frame }) =>
      this._onFrameAttached(import_frame.Frame.from(frame))
    );
    this._channel.on('frameDetached', ({ frame }) =>
      this._onFrameDetached(import_frame.Frame.from(frame))
    );
    ...
    this._channel.on('webSocketRoute', ({ webSocketRoute }) =>
      this._onWebSocketRoute(import_network.WebSocketRoute.from(webSocketRoute))
    );
    ...
    this._channel.on('webSocket', ({ webSocket }) =>
      this.emit(import_events.Events.Page.WebSocket, import_network.WebSocket.from(webSocket))
    );
    ...
    this.once(import_events.Events.Page.Close, () =>
      this._closedOrCrashedScope.close(this._closeErrorWithReason())
    );
    this.once(import_events.Events.Page.Crash, () =>
      this._closedOrCrashedScope.close(new import_errors.TargetClosedError())
    );
  ...
  async _onRoute(route) {
    ...
    for (const routeHandler of routeHandlers) {
      if (this._closeWasCalled || this._browserContext._closingStatus !== 'none') return;
  ...
  _forceVideo() {
    if (!this._video) this._video = new import_video.Video(this, this._connection);
    return this._video;
  }
  video() {
  ...
  async reload(options = {}) {
    const waitUntil = (0, import_frame.verifyLoadState)(
      'waitUntil',
      options.waitUntil === void 0 ? 'load' : options.waitUntil
    );
    return import_network.Response.fromNullable(
      (
  ...
  async waitForRequest(urlOrPredicate, options = {}) {
    const predicate = async request => {
      if ((0, import_rtti.isString)(urlOrPredicate) || (0, import_rtti.isRegExp)(urlOrPredicate))
        return (0, import_urlMatch.urlMatches)(
    ...
    return await this._waitForEvent(
      import_events.Events.Page.Request,
  ...
  async waitForResponse(urlOrPredicate, options = {}) {
    ...
    return await this._waitForEvent(
      import_events.Events.Page.Response,
      { predicate, timeout: options.timeout },
      logLine
    );
  }
  ...
  _closeErrorWithReason() {
    return new import_errors.TargetClosedError(
  ...
  async _waitForEvent(event, optionsOrPredicate, logLine) {
    return await this._wrapApiCall(async () => {
      ...
      if (event !== import_events.Events.Page.Close)
        waiter.rejectOnEvent(this, import_events.Events.Page.Close, () =>
          this._closeErrorWithReason()
        );
  ...
  async route(url, handler, options = {}) {
    this._routes.unshift(
      new import_network.RouteHandler(
        this._platform,
        this._browserContext._options.baseURL,
        url,
        handler,
  ...
  async routeWebSocket(url, handler) {
    this._webSocketRoutes.unshift(
      new import_network.WebSocketRouteHandler(this._browserContext._options.baseURL, url, handler)
    );
  ...
  async _updateWebSocketInterceptionPatterns(options) {
    const patterns = import_network.WebSocketRouteHandler.prepareInterceptionPatterns(
      this._webSocketRoutes
    );
    await this._wrapApiCall(
  ...
  async screenshot(options = {}) {
    ...
    if (!copy.type) copy.type = (0, import_elementHandle.determineScreenshotType)(options);
    if (mask) {
  ...
  async close(options = {}) {
    ...
    try {
      ...
      if ((0, import_errors.isTargetClosedError)(e) && !options.runBeforeUnload) return;
      throw e;
    }
  }
  isClosed() {
  ...
  async consoleMessages() {
    ...
    return messages.map(
      message => new import_consoleMessage.ConsoleMessage(this._platform, message, this, null)
    );
  }
  async pageErrors() {
  ...
  async agent(options = {}) {
    const params = {
      ...
      secrets: options.secrets
        ? Object.entries(options.secrets).map(([name, value]) => ({ name, value }))
        : void 0,
...
class BindingCall extends import_channelOwner.ChannelOwner {
  ...
  async call(func) {
    try {
      ...
      else result = await func(source, ...this._initializer.args.map(import_jsHandle.parseResult));
      this._channel
        .resolve({ result: (0, import_jsHandle.serializeArgument)(result) })
        .catch(() => {});
    } catch (e) {
