...
class Electron extends import_channelOwner.ChannelOwner {
  ...
  async launch(options = {}) {
    ...
    const params = {
      ...
      env: (0, import_clientHelper.envObjectToArray)(
        options.env ? options.env : this._platform.env
      ),
    ...
    app.once(import_events.Events.ElectronApplication.Close, () =>
...
class ElectronApplication extends import_channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on('console', event =>
      this.emit(
        import_events.Events.ElectronApplication.Console,
        new import_consoleMessage.ConsoleMessage(this._platform, event, null, null)
      )
    );
    this._setEventToSubscriptionMapping(
      /* @__PURE__ */ new Map([[import_events.Events.ElectronApplication.Console, 'console']])
    );
  }
    ...
    return await this._wrapApiCall(async () => {
      ...
      if (event !== import_events.Events.ElectronApplication.Close)
        waiter.rejectOnEvent(
          this,
          import_events.Events.ElectronApplication.Close,
