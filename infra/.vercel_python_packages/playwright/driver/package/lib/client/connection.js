...
class Root extends import_channelOwner.ChannelOwner {
  ...
  async initialize() {
    return import_playwright.Playwright.from(
      (
...
class Connection extends import_eventEmitter.EventEmitter {
  constructor(platform, localUtils, instrumentation, headers = []) {
    ...
    this._instrumentation =
      instrumentation || (0, import_clientInstrumentation.createInstrumentation)();
    ...
    if (id) {
      ...
      if (error && !result) {
        ...
        (0, import_stackTrace.rewriteErrorMessage)(
          parsedError,
          parsedError.message + formatCallLog(this._platform, log)
        );
        callback.reject(parsedError);
      } else {
        const validator2 = (0, import_validator.findValidator)(
          callback.type,
          callback.method,
          'Result'
        );
