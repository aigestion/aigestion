...
class Frame extends import_channelOwner.ChannelOwner {
  ...
  _timeout(options) {
    const timeoutSettings =
      this._page?._timeoutSettings || new import_timeoutSettings.TimeoutSettings(this._platform);
  ...
  _setupNavigationWaiter(options) {
    ...
    waiter.rejectOnEvent(
      this._page,
      import_events.Events.Page.Crash,
      new Error('Navigation failed because page crashed!')
    );
    waiter.rejectOnEvent(
      this._page,
      import_events.Events.Page.FrameDetached,
      ...
      async () => {
        ...
        const navigatedEvent = await waiter.waitForEvent(this._eventEmitter, 'navigated', event => {
          ...
          return (0, import_urlMatch.urlMatches)(
            this._page?.context()._options.baseURL,
            event.url,
            options.url
          );
        });
  ...
  async _evaluateFunction(functionDeclaration) {
    const result = await this._channel.evaluateExpression({
      ...
      isFunction: true,
      arg: (0, import_jsHandle.serializeArgument)(void 0),
    });
    return (0, import_jsHandle.parseResult)(result.value);
  }
  ...
  async dispatchEvent(selector, type, eventInit, options = {}) {
    await this._channel.dispatchEvent({
      selector,
      type,
      eventInit: (0, import_jsHandle.serializeArgument)(eventInit),
      ...options,
      timeout: this._timeout(options),
    });
  }
  ...
  async addScriptTag(options = {}) {
    ...
    return import_elementHandle.ElementHandle.from(
      (await this._channel.addScriptTag({ ...copy })).element
    );
  }
  async addStyleTag(options = {}) {
    ...
    return import_elementHandle.ElementHandle.from(
      (await this._channel.addStyleTag({ ...copy })).element
    );
  }
  ...
  getByTestId(testId) {
    return this.locator(
      (0, import_locatorUtils.getByTestIdSelector)(
        (0, import_locator.testIdAttributeName)(),
        testId
      )
    );
  }
  ...
  async selectOption(selector, values, options = {}) {
    return (
      await this._channel.selectOption({
        selector,
        ...(0, import_elementHandle.convertSelectOptionValues)(values),
        ...options,
        timeout: this._timeout(options),
      })
    ).values;
  ...
  async setInputFiles(selector, files, options = {}) {
    const converted = await (0, import_elementHandle.convertInputFiles)(
      this._platform,
      files,
      this.page().context()
    );
