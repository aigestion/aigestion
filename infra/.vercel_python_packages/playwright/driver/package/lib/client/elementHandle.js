...
function convertSelectOptionValues(values) {
  ...
  if (values[0] instanceof ElementHandle) return { elements: values.map(v => v._elementChannel) };
...
function filePayloadExceedsSizeLimit(payloads) {
  return (
    ...
    import_fileUtils.fileUploadSizeLimit
  );
}
...
async function convertInputFiles(platform, files, context) {
  ...
  if (items.some(item => typeof item === 'string')) {
    ...
    if (localDirectory) context._checkFileAccess(localDirectory);
    if (context._connection.isRemote()) {
      const files2 = localDirectory
        ? (
            await platform
              .fs()
            ...
            .filter(f => f.isFile())
            .map(f => platform.path().join(f.path, f.name))
        : localPaths;
