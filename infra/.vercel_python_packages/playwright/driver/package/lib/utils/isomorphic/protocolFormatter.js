  ...
  for (const name of alternatives.split('|')) {
    if (name === 'url') {
      try {
        ...
        if (urlObject.protocol === 'about:') return params[name];
        return urlObject.pathname + urlObject.search;
      } catch (error) {
        if (params[name] !== void 0) return params[name];
      }
    }
...
function renderTitleForCall(metadata) {
  const titleFormat =
    metadata.title ??
    import_protocolMetainfo.methodMetainfo.get(metadata.type + '.' + metadata.method)?.title ??
    metadata.method;
