...
class TraceModel {
  constructor(traceUri, contexts) {
    ...
    for (const action of this.actions) {
      action.group =
        action.group ??
        (0, import_protocolFormatter.getActionGroup)({ type: action.class, method: action.method });
      if (action.group)
  ...
  renderActionTree(filter) {
    ...
    const visit = (actionItem, indent) => {
      const title = (0, import_protocolFormatter.renderTitleForCall)({
        ...actionItem.action,
...
function indexModel(context2) {
  for (const page of context2.pages) page[contextSymbol] = context2;
  ...
  for (const event of context2.events) event[contextSymbol] = context2;
  for (const resource of context2.resources) resource[contextSymbol] = context2;
}
function mergeActionsAndUpdateTiming(contexts) {
  ...
  for (let i = 1; i < result.length; ++i) result[i][prevByEndTimeSymbol] = result[i - 1];
  result.sort((a1, a2) => {
    if (a2.parentId === a1.callId) return -1;
    if (a1.parentId === a2.callId) return 1;
    return a1.startTime - a2.startTime;
  });
  for (let i = 0; i + 1 < result.length; ++i) result[i][nextByStartTimeSymbol] = result[i + 1];
  return result;
}
let lastTmpStepId = 0;
...
function adjustMonotonicTime(contexts, monotonicTimeDelta) {
  for (const context2 of contexts) {
    ...
    for (const event of context2.events) event.time += monotonicTimeDelta;
    for (const event of context2.stdio) event.timestamp += monotonicTimeDelta;
    ...
    for (const resource of context2.resources) {
      if (resource._monotonicTime) resource._monotonicTime += monotonicTimeDelta;
    }
  }
}
