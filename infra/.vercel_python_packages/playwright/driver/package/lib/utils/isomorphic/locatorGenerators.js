...
function asLocators(lang, selector, isFrameLocator = false, maxOutputSize = 20, preferredQuote) {
  try {
    return innerAsLocators(
      ...
      (0, import_selectorParser.parseSelector)(selector),
      isFrameLocator,
      maxOutputSize
    );
  } catch (e) {
      ...
      if (['xpath', 'css'].includes(lastPart.name)) {
        transformed.push(
          factory.generateLocator(
            base,
            'frame-locator',
            (0, import_selectorParser.stringifySelector)({ parts: [lastPart] })
          ),
          factory.generateLocator(
            base,
            'frame-locator',
            (0, import_selectorParser.stringifySelector)({ parts: [lastPart] }, true)
          )
        );
      }
  ...
  generateLocator(base, kind, body, options = {}) {
    ...
    switch (kind) {
      ...
      case 'role':
        ...
        for (const { name, value } of options.attrs)
          attrs.push(
            `.set${(0, import_stringUtils.toTitleCase)(name)}(${typeof value === 'string' ? this.quote(value) : value})`
          );
  ...
  generateLocator(base, kind, body, options = {}) {
    switch (kind) {
      ...
      case 'role':
        ...
        for (const { name, value } of options.attrs)
          attrs.push(
            `${(0, import_stringUtils.toTitleCase)(name)} = ${typeof value === 'string' ? this.quote(value) : value}`
          );
