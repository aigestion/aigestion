...
class SnapshotRenderer {
  ...
  closestScreenshot() {
    ...
    const closestFrame =
      wallTime && this._screencastFrames[0]?.frameSwapWallTime
  ...
  render() {
    ...
    const visit = (n, snapshotIndex, parentTag, parentAttrs) => {
      if (typeof n === 'string') {
        ...
        else result.push((0, import_stringUtils.escapeHTML)(n));
        return;
      }
      if (isSubtreeReferenceSnapshot(n)) {
        ...
        const isSourceInsidePictureWithCurrentSrc =
          nodeName === 'SOURCE' &&
          parentTag === 'PICTURE' &&
          parentAttrs?.some(a => a[0] === kCurrentSrcAttribute);
        for (const [attr, value] of attrs) {
          ...
          result.push(
            ...
            attrName,
            '="',
            (0, import_stringUtils.escapeHTMLAttribute)(attrValue),
            '"'
          );
        }
        result.push('>');
        for (const child of children) visit(child, snapshotIndex, nodeName, attrs);
        if (!autoClosing.has(nodeName)) result.push('</', nodeName, '>');
        return;
      } else {
        return;
      }
  ...
  resourceByUrl(url, method) {
    ...
    for (const resource of this._resources) {
      if (
        typeof resource._monotonicTime === 'number' &&
        resource._monotonicTime >= snapshot.timestamp
      )
        break;
      ...
      if (resource.request.url === url && resource.request.method === method) {
        if (resource._frameref === snapshot.frameId) sameFrameResource = resource;
        else otherFrameResource = resource;
      }
    }
...
const autoClosing = /* @__PURE__ */ new Set([
  'AREA',
  'BASE',
  'BR',
  'COL',
...
function snapshotScript(viewport, ...targetIds) {
  function applyPlaywrightAttributes(viewport2, ...targetIds2) {
    ...
    const shouldPopulateCanvasFromScreenshot = searchParams.has(
    ...
    const visit = root => {
      ...
      for (const element of root.querySelectorAll(`[__playwright_dialog_open_]`)) {
        try {
          if (element.getAttribute('__playwright_dialog_open_') === 'modal') element.showModal();
          else element.show();
        } catch {}
      ...
      if (search.get('pointX') && search.get('pointY')) {
        ...
        for (const target of hasTargetElements ? targetElements : roots) {
          ...
          if (hasTargetElements) {
            ...
            if (
              isTopFrame &&
              (Math.abs(centerX - pointX) >= 10 || Math.abs(centerY - pointY) >= 10)
            ) {
        ...
        img.onload = () => {
          for (const canvas of canvasElements) {
            ...
            while (currWindow !== topSnapshotWindow) {
              ...
              const iframeInfo =
                currWindow['__playwright_frame_bounding_rects__']?.frames.get(iframe);
...
function rewriteURLForCustomProtocol(href) {
  if (href.startsWith(kLegacyBlobPrefix)) href = href.substring(kLegacyBlobPrefix.length);
  try {
    const url = new URL(href);
    if (url.protocol === 'javascript:' || url.protocol === 'vbscript:') return 'javascript:void(0)';
    const isBlob = url.protocol === 'blob:';
    const isFile = url.protocol === 'file:';
    if (!isBlob && !isFile && schemas.includes(url.protocol)) return href;
...
function rewriteURLsInStyleSheetForCustomProtocol(text) {
  return text.replace(urlInCSSRegex, (match, protocol) => {
    ...
    if (!isBlob && !isFile && schemas.includes(protocol)) return match;
    return match.replace(protocol + '//', `https://pw-${protocol.slice(0, -1)}--`);
  });
}
const urlToEscapeRegex1 = /url\(\s*'([^']*)'\s*\)/gi;
const urlToEscapeRegex2 = /url\(\s*"([^"]*)"\s*\)/gi;
