...
class TraceModernizer {
  ...
  _appendEvent(line) {
    ...
    for (const event of events) this._innerAppendEvent(event);
  }
  _innerAppendEvent(event) {
    ...
    switch (event.type) {
      ...
      case 'after': {
        ...
        if (event.point) existing.point = event.point;
        break;
      }
      case 'action': {
      ...
      case 'frame-snapshot':
        this._snapshotStorage.addFrameSnapshot(
          ...
          event.snapshot,
          this._pageEntry(event.snapshot.pageId).screencastFrames
        );
        break;
    }
    if ('pageId' in event && event.pageId) this._pageEntry(event.pageId);
  ...
  _modernize_1_to_2(events) {
    for (const event of events) {
      if (event.type !== 'frame-snapshot' || !event.snapshot.isMainFrame) continue;
      event.snapshot.viewport = this._contextEntry.options?.viewport || {
        width: 1280,
        height: 720,
      };
    }
  ...
  _modernize_2_to_3(events) {
    for (const event of events) {
      if (event.type !== 'resource-snapshot' || event.snapshot.request) continue;
  ...
  _modernize_5_to_6(events) {
    ...
    for (const event of events) {
      result.push(event);
      if (event.type !== 'after' || !event.log.length) continue;
  ...
  _modernize_6_to_7(events) {
    ...
    for (const event of events) {
      ...
      if (event.type === 'before' || event.type === 'action') {
        if (!this._contextEntry.wallTime) this._contextEntry.wallTime = event.wallTime;
