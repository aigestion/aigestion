...
class ElectronApplication extends import_instrumentation.SdkObject {
  constructor(parent, browser, nodeConnection, process2) {
    ...
    this._nodeSession.on('Runtime.executionContextCreated', async event => {
      ...
      const crExecutionContext = new import_crExecutionContext.CRExecutionContext(
        this._nodeSession,
        event.context
      );
  ...
  async _onConsoleAPI(event) {
    ...
    const args = event.args.map(arg =>
      (0, import_crExecutionContext.createHandle)(this._nodeExecutionContext, arg)
    );
    const message = new import_console.ConsoleMessage(
      null,
      null,
      event.type,
      void 0,
...
class Electron extends import_instrumentation.SdkObject {
  ...
  async launch(progress, options) {
    ...
    if (process.platform === 'win32') {
      shell = true;
      command = [command, ...electronArguments]
        .map(arg => `"${escapeDoubleQuotes(arg)}"`)
        .join(' ');
      electronArguments = [];
    }
    ...
    const waitForXserverError = waitForLine(
      progress,
      launchedProcess,
    ...
    try {
      ...
      const nodeTransport = await import_transport.WebSocketTransport.connect(
        progress,
        nodeMatch[1]
      );
      const nodeConnection = new import_crConnection.CRConnection(
        this,
        nodeTransport,
      ...
      const chromeTransport = await import_transport.WebSocketTransport.connect(
        progress,
        chromeMatch[1]
      );
...
async function waitForLine(progress, process2, regex) {
  ...
  const listeners = [
    ...
    import_eventsHelper.eventsHelper.addEventListener(process2, 'exit', () =>
      promise.reject(failError)
    ),
    // It is Ok to remove error handler because we did not create process and there is another listener.
    import_eventsHelper.eventsHelper.addEventListener(process2, 'error', () =>
      promise.reject(failError)
    ),
  ];
