...
def test_example(page: Page) -> None {`);
      if (recordHar)
        formatter.add(
          `    page.route_from_har(${quote(recordHar.path)}${typeof recordHar.urlFilter === 'string' ? `, url=${quote(recordHar.urlFilter)}` : ''})`
        );
...
async def run(playwright: Playwright) -> None {
    ...
    context = await browser.new_context(${formatContextOptions(options.contextOptions, options.deviceName)})`);
      if (recordHar)
        formatter.add(
          `    await context.route_from_har(${quote(recordHar.path)}${typeof recordHar.urlFilter === 'string' ? `, url=${quote(recordHar.urlFilter)}` : ''})`
        );
    } else {
      formatter.add(`
import re
...
def run(playwright: Playwright) -> None {
    ...
    context = browser.new_context(${formatContextOptions(options.contextOptions, options.deviceName)})`);
      if (recordHar)
        formatter.add(
          `    context.route_from_har(${quote(recordHar.path)}${typeof recordHar.urlFilter === 'string' ? `, url=${quote(recordHar.urlFilter)}` : ''})`
        );
    }
    return formatter.format();
  }
...
function formatOptions(value, hasArguments, asDict) {
  ...
  return (
    ...
    keys
      .map(key => {
        if (asDict) return `"${(0, import_utils.toSnakeCase)(key)}": ${formatValue(value[key])}`;
...
function formatContextOptions(options, deviceName, asDict) {
  ...
  return (
    ...
    formatOptions((0, import_language.sanitizeDeviceOptions)(device, options), true, asDict)
  );
}
