...
{`);
  ...
  _formatPageAlias(pageAlias) {
    if (this._mode === 'library') return pageAlias;
    if (pageAlias === 'page') return 'Page';
    return pageAlias;
  }
  _generateActionCall(subject, actionInContext) {
    ...
    switch (action.name) {
      ...
      case 'hover': {
        const optionsString = action.position
          ? formatObject({ position: action.position }, '    ')
          : '';
  ...
  generateStandaloneHeader(options) {
    ...
    if (options.contextOptions.recordHar) {
      ...
      formatter.add(
        `      await context.RouteFromHARAsync(${quote(options.contextOptions.recordHar.path)}${url ? `, ${formatObject({ url }, '    ')}` : ''});`
      );
    }
    formatter.newLine();
    return formatter.format();
