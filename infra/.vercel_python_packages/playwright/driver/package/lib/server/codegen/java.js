...
class JavaLanguageGenerator {
  ...
  _generateActionCall(subject, actionInContext, inFrameLocator) {
    ...
    switch (action.name) {
      ...
      case 'hover': {
        const optionsText = action.position
          ? `new Locator.HoverOptions().setPosition(${action.position.x}, ${action.position.y})`
          : '';
  ...
  generateHeader(options) {
    ...
    if (this._mode === 'junit') {
      ...
      ${
        options.contextOptions.recordHar
      ...
      if (options.contextOptions.recordHar) {
        ...
        const recordHarOptions =
          typeof url === 'string'
            ...
            : '';
        formatter.add(
          `          page.routeFromHAR(Paths.get(${quote(options.contextOptions.recordHar.path)})${recordHarOptions});`
        );
      }
      return formatter.format();
    }
    ...
    if (options.contextOptions.recordHar) {
      ...
      const recordHarOptions =
        typeof url === 'string'
          ...
          : '';
      formatter.add(
        `          context.routeFromHAR(Paths.get(${quote(options.contextOptions.recordHar.path)})${recordHarOptions});`
      );
    }
    return formatter.format();
  }
