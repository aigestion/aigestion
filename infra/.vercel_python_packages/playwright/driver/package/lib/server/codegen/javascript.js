...
${useText ? '\ntest.use(' + useText + ');\n' : ''}
    ...
    if (options.contextOptions.recordHar) {
      ...
      formatter.add(
        `  await page.routeFromHAR(${quote(options.contextOptions.recordHar.path)}${url ? `, ${formatOptions({ url }, false)}` : ''});`
      );
    }
    return formatter.format();
  }
  ...
  generateStandaloneHeader(options) {
    ...
    if (options.contextOptions.recordHar)
      formatter.add(
        `        await context.routeFromHAR(${quote(options.contextOptions.recordHar.path)});`
      );
    return formatter.format();
  }
...
function formatContextOptions(options, deviceName, isTest) {
  ...
  if (!device) return (0, import_utils.formatObjectOrVoid)(options);
  let serializedObject = (0, import_utils.formatObjectOrVoid)(
    (0, import_language.sanitizeDeviceOptions)(device, options)
  );
...
function quoteMultiline(text, indent = '  ') {
  const escape = text2 =>
    text2.replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${');
