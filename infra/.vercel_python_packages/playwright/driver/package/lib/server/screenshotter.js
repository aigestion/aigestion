...
class Screenshotter {
    ...
    return this._queue.postTask(async () => {
      ...
      try {
        ...
        return await this._screenshot(
          progress,
          format,
          import_helper.helper.enclosingIntRect(documentRect),
          void 0,
          fitsViewport,
          options
        );
  ...
  async _preparePageForScreenshot(progress, frame, screenshotStyle, hideCaret, disableAnimations) {
    ...
    await progress.race(
      this._page.safeNonStallingEvaluateInAllFrames(
        '(' +
  ...
  async _maskElements(progress, options) {
    ...
    try {
      const promises = frames.map(frame =>
        frame.maskSelectors(framesToParsedSelectors.get(frame), options.maskColor || '#F0F')
      );
  ...
  async _screenshot(progress, format, documentRect, viewportRect, fitsViewport, options) {
    ...
    try {
      ...
      const buffer = await this._page.delegate.takeScreenshot(
        progress,
        format,
        documentRect,
        viewportRect,
      ...
      if (shouldSetDefaultBackground) await this._page.delegate.setBackgroundColor();
      if (options.__testHookAfterScreenshot)
        await progress.race(options.__testHookAfterScreenshot());
      return buffer;
    } catch (error) {
      cleanupHighlight().catch(() => {});
      if (shouldSetDefaultBackground) this._page.delegate.setBackgroundColor().catch(() => {});
      throw error;
    }
  }
}
