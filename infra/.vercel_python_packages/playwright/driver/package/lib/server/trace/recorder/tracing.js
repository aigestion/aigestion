...
class Tracing extends import_instrumentation.SdkObject {
  constructor(context, tracesDir) {
    ...
    const testIdAttributeName =
      'selectors' in context ? context.selectors().testIdAttributeName() : void 0;
  ...
  _sdkLanguage() {
    return this._context instanceof import_browserContext.BrowserContext
      ? this._context._browser.sdkLanguage()
  ...
  async resetForReuse(progress) {
    ...
    if (this._snapshotter) await progress.race(this._snapshotter.resetForReuse());
  }
  start(options) {
    ...
    if (options.snapshots) this._harTracer.start({ omitScripts: !options.live });
  }
  async startChunk(progress, options = {}) {
    if (this._state && this._state.recording) await this.stopChunk(progress, { mode: 'discard' });
    ...
    this._eventListeners.push(
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        this._context,
        import_browserContext.BrowserContext.Events.PageError,
  ...
  _currentGroupId() {
    return this._state?.groupStack.length
      ? this._state.groupStack[this._state.groupStack.length - 1]
      : void 0;
  }
  ...
  _startScreencast() {
    if (!(this._context instanceof import_browserContext.BrowserContext)) return;
    for (const page of this._context.pages()) this._startScreencastInPage(page);
    this._screencastListeners.push(
      import_eventsHelper.eventsHelper.addEventListener(
        this._context,
        import_browserContext.BrowserContext.Events.Page,
  ...
  _stopScreencast() {
    ...
    if (!(this._context instanceof import_browserContext.BrowserContext)) return;
    for (const page of this._context.pages()) page.screencast.setOptions(null);
  }
  _allocateNewTraceFile(state) {
    const suffix = state.chunkOrdinal ? `-chunk${state.chunkOrdinal}` : ``;
    state.chunkOrdinal++;
    state.traceFile = import_path.default.join(
      state.tracesDir,
  ...
  async stop(progress) {
    ...
    const promise = progress
      ? progress.race(this._fs.syncAndGetError())
      : this._fs.syncAndGetError();
    await promise.finally(() => {
      this._state = void 0;
    });
  }
  async deleteTmpTracesDir() {
    if (this._tracesTmpDir) await (0, import_fileUtils.removeFolders)([this._tracesTmpDir]);
  }
  _createTracesDirIfNeeded() {
    if (this._precreatedTracesDir) return this._precreatedTracesDir;
    this._tracesTmpDir = import_fs.default.mkdtempSync(
      import_path.default.join(import_os.default.tmpdir(), 'playwright-tracing-')
    );
  ...
  async stopChunk(progress, params) {
    ...
    if (this._state.options.snapshots) this._snapshotter?.stop();
    this.flushHarEntries();
    const newNetworkFile = import_path.default.join(
    ...
    for (const sha1 of /* @__PURE__ */ new Set([
      ...this._state.traceSha1s,
      ...this._state.networkSha1s,
    ]))
      entries.push({
        name: import_path.default.join('resources', sha1),
        value: import_path.default.join(this._state.resourcesDir, sha1),
      });
    ...
    if (error) {
      if (
        !(0, import_progress.isAbortError)(error) &&
        this._context instanceof import_browserContext.BrowserContext &&
        !this._context._browser.isConnected()
      )
        return {};
      throw error;
    }
    if (params.mode === 'entries') return { entries };
  ...
  async _captureSnapshot(snapshotName, sdkObject, metadata) {
    if (!snapshotName || !sdkObject.attribution.page) return;
    await this._snapshotter
      ?.captureSnapshot(sdkObject.attribution.page, metadata.id, snapshotName)
      .catch(() => {});
  }
  _shouldCaptureSnapshot(sdkObject, metadata) {
    return (
      !!this._snapshotter?.started() &&
      shouldCaptureSnapshot(metadata) &&
...
function visitTraceEvent(object, sha1s) {
  ...
  if (object instanceof import_dispatcher.Dispatcher) return `<${object._type}>`;
...
function shouldCaptureSnapshot(metadata) {
  const metainfo = import_protocolMetainfo.methodMetainfo.get(
