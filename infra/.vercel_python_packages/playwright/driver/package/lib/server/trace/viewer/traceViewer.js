...
function validateTraceUrl(traceFileOrUrl) {
  if (!traceFileOrUrl) return traceFileOrUrl;
  if (traceFileOrUrl.startsWith('http://') || traceFileOrUrl.startsWith('https://'))
    return traceFileOrUrl;
  let traceFile = traceFileOrUrl;
  if (traceFile.endsWith('.json')) return toFilePathUrl(traceFile);
  try {
    const stat = import_fs.default.statSync(traceFile);
    if (stat.isDirectory()) traceFile = import_path.default.join(traceFile, tracesDirMarker);
    ...
    if (relativePath.startsWith('/file')) {
      try {
        ...
        if (filePath.endsWith('.json')) {
          ...
          return sendTraceDescriptor(
            response,
            import_path.default.dirname(fullPrefix),
            import_path.default.basename(fullPrefix)
          );
        }
...
async function installRootRedirect(server, traceUrl, options) {
  ...
  if (traceUrl) params.append('trace', traceUrl);
  if (server.wsGuid()) params.append('ws', server.wsGuid());
  if (options?.isServer) params.append('isServer', '');
  if ((0, import_utils2.isUnderTest)()) params.append('isUnderTest', 'true');
...
async function runTraceViewerApp(traceUrl, browserName, options, exitOnClose) {
  ...
  if (exitOnClose) page.on('close', () => (0, import_utils.gracefullyProcessExitDoNotHang)(0));
  return page;
}
...
async function openTraceViewerApp(url, browserName, options) {
  const traceViewerPlaywright = (0, import_playwright.createPlaywright)({
    sdkLanguage: 'javascript',
    isInternalPlaywright: true,
  });
  const traceViewerBrowser = (0, import_utils2.isUnderTest)() ? 'chromium' : browserName;
  const { context, page } = await (0, import_launchApp2.launchApp)(
...
function traceDescriptor(traceDir, tracePrefix) {
  ...
  if (import_fs.default.existsSync(resourcesDir)) {
    for (const name of import_fs.default.readdirSync(resourcesDir))
      result.entries.push({
        name: 'resources/' + name,
        path: toFilePathUrl(import_path.default.join(resourcesDir, name)),
      });
  }
  return result;
}
