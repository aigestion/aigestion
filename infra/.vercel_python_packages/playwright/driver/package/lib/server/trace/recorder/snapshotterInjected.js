  ...
  class Streamer {
    constructor() {
      ...
      this._interceptNativeSetter(window.StyleSheet.prototype, 'disabled', sheet => {
        if (sheet instanceof CSSStyleSheet) this._invalidateStyleSheet(sheet);
      });
    ...
    _invalidateStyleSheet(sheet) {
      ...
      if (sheet.href !== null) this._modifiedStyleSheets.add(sheet);
    }
    ...
    captureSnapshot(needsReset) {
      ...
      const visitNode = node => {
        ...
        const checkAndReturn = n => {
          ...
          if (equals) return { equals: true, n: [[snapshotNumber - data.ref[0], data.ref[1]]] };
        ...
        if (nodeType === Node.ELEMENT_NODE) {
          ...
          for (let i = 0; i < element.attributes.length; i++) {
            ...
            if (nodeName === 'DIALOG' && name === 'open') continue;
