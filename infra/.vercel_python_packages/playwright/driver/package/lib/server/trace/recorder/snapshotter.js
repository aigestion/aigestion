...
class Snapshotter {
  ...
  async start() {
    this._started = true;
    if (!this._initScript) await this._initialize();
    await this.reset();
  }
  async reset() {
    if (this._started)
      await this._context.safeNonStallingEvaluateInAllFrames(
        `window["${this._snapshotStreamer}"].reset()`,
        'main'
      );
  }
  stop() {
    ...
    const snapshots = page.frames().map(async frame => {
      ...
      for (const { url, content, contentType } of data.resourceOverrides) {
        if (typeof content === 'string') {
          ...
          const sha1 =
            (0, import_crypto.calculateSha1)(buffer) +
            '.' +
  ...
  _onPage(page) {
    ...
    this._eventListeners.push(
      import_eventsHelper.eventsHelper.addEventListener(
        page,
        import_page.Page.Events.FrameAttached,
  ...
  async _annotateFrameHierarchy(frame) {
    try {
      ...
      await context?.evaluate(
        ({ snapshotStreamer, frameElement: frameElement2, frameId }) => {
