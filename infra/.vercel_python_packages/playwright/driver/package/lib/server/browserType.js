...
class BrowserType extends import_instrumentation.SdkObject {
  ...
  async launch(progress, options, protocolLogger) {
    ...
    return this._innerLaunchWithRetries(
      ...
      options,
      void 0,
      import_helper.helper.debugProtocolLogger(protocolLogger)
    ).catch(e => {
  ...
  async launchPersistentContext(progress, userDataDir, options) {
    ...
    if (options.clientCertificates?.length) {
      clientCertificatesProxy =
        await import_socksClientCertificatesInterceptor.ClientCertificatesProxy.create(
          progress,
          options
        );
    ...
    try {
      const browser = await this._innerLaunchWithRetries(
        ...
        launchOptions,
        options,
        import_helper.helper.debugProtocolLogger(),
        userDataDir
      ).catch(e => {
        throw this._rewriteStartupLog(e);
      });
  ...
  async _innerLaunch(progress, options, persistent, protocolLogger, maybeUserDataDir) {
    options.proxy = options.proxy
      ? (0, import_browserContext.normalizeProxySettings)(options.proxy)
      : void 0;
    ...
    try {
      ...
      const browserOptions = {
        ...
        browserLogsCollector,
        wsEndpoint:
          transport instanceof import_transport.WebSocketTransport ? transport.wsEndpoint : void 0,
        originalLaunchOptions: options,
      };
      if (persistent)
        (0, import_browserContext.validateBrowserContextOptions)(persistent, browserOptions);
      copyTestHooks(options, browserOptions);
      const browser = await progress.race(
        this.connectToTransport(transport, browserOptions, browserLogsCollector)
      );
  ...
  async _prepareToLaunch(options, isPersistent, userDataDir) {
    ...
    const artifactsDir = await import_fs.default.promises.mkdtemp(
      import_path.default.join(import_os.default.tmpdir(), 'playwright-artifacts-')
    );
    tempDirectories.push(artifactsDir);
    if (userDataDir) {
      (0, import_assert.assert)(
        import_path.default.isAbsolute(userDataDir),
      ...
      userDataDir = await import_fs.default.promises.mkdtemp(
        import_path.default.join(import_os.default.tmpdir(), `playwright_${this._name}dev_profile-`)
      );
    ...
    if (ignoreAllDefaultArgs) browserArguments.push(...args);
      ...
      browserArguments.push(
        ...(await this.defaultArgs(options, isPersistent, userDataDir)).filter(
          arg => ignoreDefaultArgs.indexOf(arg) === -1
        )
      );
    else browserArguments.push(...(await this.defaultArgs(options, isPersistent, userDataDir)));
    let executable;
    if (executablePath) {
      ...
      const registryExecutable = import_registry.registry.findExecutable(
        this.getExecutableName(options)
      );
      ...
      await import_registry.registry.validateHostRequirementsForExecutablesIfNeeded(
        [registryExecutable],
        this.attribution.playwright.options.sdkLanguage
      );
    }
  ...
  async _launchProcess(progress, options, isPersistent, browserLogsCollector, userDataDir) {
    ...
    const env = options.env
      ? (0, import_processLauncher.envArrayToObject)(options.env)
      : process.env;
    ...
    import_processLauncher.launchProcess)({
      ...
      attemptToGracefullyClose: async () => {
        ...
        if (transport) {
          ...
          throw new Error(
            'Force-killing the browser because no transport is available to gracefully close it.'
          );
        }
      },
    ...
    browserProcess = {
      ...
      close: () =>
        closeOrKill(
          options.__testHookBrowserCloseTimeout || import_time.DEFAULT_PLAYWRIGHT_TIMEOUT
        ),
      kill,
    };
    try {
...
${updatedLog}`);
  ...
  _validateLaunchOptions(options) {
    ...
    if ((0, import_debug.debugMode)() === 'inspector') headless = false;
    ...
    if (options.socksProxyPort) proxy = { server: `socks5://127.0.0.1:${options.socksProxyPort}` };
  ...
  _rewriteStartupLog(error) {
    if (!(0, import_protocolError.isProtocolError)(error)) return error;
    if (error.logs) error.logs = this.doRewriteStartupLog(error.logs);
    return error;
  }
