...
class HarTracer {
  constructor(context, page, delegate, options) {
    ...
    this._baseURL =
      context instanceof import_fetch.APIRequestContext
  ...
  start(options) {
    ...
    this._started = true;
    const apiRequest =
      this._context instanceof import_fetch.APIRequestContext
        ? this._context
        : this._context.fetchRequest;
    this._eventListeners = [
      import_eventsHelper.eventsHelper.addEventListener(
        apiRequest,
        import_fetch.APIRequestContext.Events.Request,
        event => this._onAPIRequest(event)
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        apiRequest,
        import_fetch.APIRequestContext.Events.RequestFinished,
    ...
    if (this._context instanceof import_browserContext.BrowserContext) {
      this._eventListeners.push(
        ...
        import_eventsHelper.eventsHelper.addEventListener(
          this._context,
          import_browserContext.BrowserContext.Events.Request,
        ...
        import_eventsHelper.eventsHelper.addEventListener(
          this._context,
          import_browserContext.BrowserContext.Events.RequestFailed,
          request => this._onRequestFailed(request)
        ),
        import_eventsHelper.eventsHelper.addEventListener(
          this._context,
          import_browserContext.BrowserContext.Events.Response,
          response => this._onResponse(response)
        ),
        import_eventsHelper.eventsHelper.addEventListener(
          this._context,
          import_browserContext.BrowserContext.Events.RequestAborted,
          request => this._onRequestAborted(request)
        ),
        import_eventsHelper.eventsHelper.addEventListener(
          this._context,
          import_browserContext.BrowserContext.Events.RequestFulfilled,
          request => this._onRequestFulfilled(request)
        ),
        import_eventsHelper.eventsHelper.addEventListener(
          this._context,
          import_browserContext.BrowserContext.Events.RequestContinued,
          request => this._onRequestContinued(request)
        )
      );
      for (const page of this._context.pages()) this._createPageEntryIfNeeded(page);
    }
  }
  _shouldIncludeEntryWithUrl(urlString) {
    return (
      !this._options.urlFilter ||
      (0, import_utils2.urlMatches)(this._baseURL, urlString, this._options.urlFilter)
    );
  }
  ...
  _onAPIRequestFinished(event) {
    ...
    if (!this._options.omitSizes) harEntry.response.bodySize = event.body?.length ?? 0;
  ...
  _recordRequestOverrides(harEntry, request) {
    if (!request.overrides() || !this._options.recordRequestOverrides) return;
  ...
  async _onRequestFinished(request, response) {
    ...
    harEntry.timings.receive =
      response.request()._responseEndTiming !== -1
        ? import_helper.helper.millisToRoundishMillis(
  ...
  _storeResponseContent(buffer, content, resourceType) {
    ...
    if (this._options.content === 'embed') {
      ...
      const sha1 =
        (0, import_utils.calculateSha1)(buffer) +
        '.' +
        (import_utilsBundle.mime.getExtension(content.mimeType) || 'dat');
  ...
  _onResponse(response) {
    ...
    if (!this._options.omitTiming) {
      ...
      const connect =
        timing.connectEnd !== -1
          ? import_helper.helper.millisToRoundishMillis(timing.connectEnd - timing.connectStart)
          : -1;
      const ssl =
        timing.connectEnd !== -1
          ? import_helper.helper.millisToRoundishMillis(
      ...
      const wait =
        timing.responseStart !== -1
          ? import_helper.helper.millisToRoundishMillis(timing.responseStart - timing.requestStart)
          : -1;
      const receive = -1;
      harEntry.timings = {
        dns,
  ...
  stop() {
    ...
    const context =
      this._context instanceof import_browserContext.BrowserContext ? this._context : void 0;
    const log = {
      version: '1.2',
      creator: {
        name: 'Playwright',
  ...
  _postDataForBuffer(postData, contentType, content) {
    ...
    if (content === 'attach') {
      const sha1 =
        (0, import_utils.calculateSha1)(postData) +
        '.' +
        (import_utilsBundle.mime.getExtension(contentType) || 'dat');
    ...
    if (contentType === 'application/x-www-form-urlencoded') {
      ...
      for (const [name, value] of parsed.entries()) result.params.push({ name, value });
    }
    return result;
  }
}
