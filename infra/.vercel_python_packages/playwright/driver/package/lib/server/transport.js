...
class WebSocketTransport {
  constructor(progress, url, logUrl, options) {
    ...
    this._ws.addEventListener('error', error =>
      this._progress?.log(`<ws error> ${logUrl} ${error.type} ${error.message}`)
    );
  }
  ...
  static async _connect(progress, url, options, hadRedirects) {
    ...
    const transport = new WebSocketTransport(progress, url, logUrl, {
      ...options,
      followRedirects: !!options.followRedirects && hadRedirects,
    });
...
${Buffer.concat(chunks)}`
  ...
  async closeAndWait() {
    if (this._ws.readyState === import_utilsBundle.ws.CLOSED) return;
