...
function httpRequest(params, onResponse, onError) {
  ...
  const requestCallback = res => {
    ...
    if (statusCode >= 300 && statusCode < 400 && res.headers.location) {
      request.destroy();
      cancelRequest = httpRequest(
        { ...params, url: new URL(res.headers.location, params.url).toString() },
        onResponse,
        onError
      ).cancel;
    } else {
  ...
  const request =
    url.protocol === 'https:'
      ? import_https.default.request(url, options, requestCallback)
      : import_http.default.request(url, options, requestCallback);
...
function normalizeProxyURL(proxy) {
  proxy = proxy.trim();
  if (!/^\w+:\/\//.test(proxy)) proxy = 'http://' + proxy;
  return new URL(proxy);
}
function createProxyAgent(proxy, forUrl) {
  if (!proxy) return;
  if (forUrl && proxy.bypass && shouldBypassProxy(forUrl, proxy.bypass)) return;
...
async function startHttpServer(server, options) {
  ...
  try {
    ...
    await Promise.race([new Promise(cb => server.once('listening', cb)), errorPromise]);
  } finally {
