...
function hexToNumber(hex) {
  return [...hex].reduce((value, digit) => {
    ...
    if (code >= 48 && code <= 57) return value + code;
    if (code >= 97 && code <= 102) return value + (code - 97) + 10;
    if (code >= 65 && code <= 70) return value + (code - 65) + 10;
...
function parsePattern(pattern) {
  ...
  const matchers = pattern.split(',').map(token => {
    ...
    return (host, port) => {
      if (!portMatches(port)) return false;
      if (import_net.default.isIPv4(host) || import_net.default.isIPv6(host)) return false;
    ...
    try {
      ...
      socket.on('error', error => {
        ...
        import_debugLogger.debugLogger.log(
          'socks',
          `[${uid}] <= network socket error ${payload2.error}`
        );
