...
const printReceivedStringContainExpectedSubstring = (utils, received, start, length) =>
...
const printReceivedStringContainExpectedResult = (utils, received, result) =>
  result === null
    ? utils.printReceived(received)
    : printReceivedStringContainExpectedSubstring(utils, received, result.index, result[0].length);
function formatMatcherMessage(utils, details) {
  ...
  let message =
    ...
    utils.RECEIVED_COLOR(receiver) +
    utils.DIM_COLOR(
      ')' + (details.promise ? '.' + details.promise : '') + (details.isNot ? '.not' : '') + '.'
    ) +
    details.matcherName +
    utils.DIM_COLOR('(') +
    utils.EXPECTED_COLOR(details.expectation) +
    utils.DIM_COLOR(')') +
    ' failed\n\n';
...
const simpleMatcherUtils = {
  ...
  printDiffOrStringify: (expected, received, expectedLabel, receivedLabel) => {
    ...
    return (
      `${expectedLabel}: `.padEnd(maxLength) +
      printExpected(expected) +
      `
` +
      `${receivedLabel}: `.padEnd(maxLength) +
