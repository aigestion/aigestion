...
const existsAsync = path2 =>
  new Promise(resolve => import_fs.default.stat(path2, err => resolve(!err)));
async function mkdirIfNeeded(filePath) {
  await import_fs.default.promises
    .mkdir(import_path.default.dirname(filePath), { recursive: true })
    .catch(() => {});
}
    ...
    switch (op.op) {
      ...
      case 'writeFile': {
        if (op.skipIfExists)
          await import_fs.default.promises
            ...
            .catch(() => {});
        else await import_fs.default.promises.writeFile(op.file, op.content);
        return;
      }
      case 'copyFile': {
      ...
      case 'zip': {
        ...
        zipFile.outputStream
          .pipe(import_fs.default.createWriteStream(op.zipFileName))
