...
class HttpServer {
  ...
  createWebSocket(transport, guid) {
    (0, import_assert.assert)(
      !this._wsGuid,
      'can only create one main websocket transport per server'
    );
  ...
  async start(options = {}) {
    ...
    if (options.preferredPort) {
      try {
        await (0, import_network.startHttpServer)(this._server, {
  ...
  _serveFile(response, absoluteFilePath) {
    ...
    const contentType =
      import_utilsBundle.mime.getType(import_path.default.extname(absoluteFilePath)) ||
  ...
  _serveRangeFile(request, response, absoluteFilePath) {
    ...
    response.writeHead(206, {
      ...
      'Content-Type': import_utilsBundle.mime.getType(
        import_path.default.extname(absoluteFilePath)
      ),
    });
