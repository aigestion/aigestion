...
function compareImages(mimeType, actualBuffer, expectedBuffer, options = {}) {
  ...
  let actual =
    mimeType === 'image/png'
      ? import_utilsBundle3.PNG.sync.read(actualBuffer)
      : import_utilsBundle.jpegjs.decode(actualBuffer, {
          maxMemoryUsageInMB: JPEG_JS_MAX_BUFFER_SIZE_IN_MB,
        });
  let expected =
    mimeType === 'image/png'
      ? import_utilsBundle3.PNG.sync.read(expectedBuffer)
      : import_utilsBundle.jpegjs.decode(expectedBuffer, {
          maxMemoryUsageInMB: JPEG_JS_MAX_BUFFER_SIZE_IN_MB,
        });
  const size = {
  ...
  if (options.comparator === 'ssim-cie94') {
    count = (0, import_compare.compare)(
      expected.data,
      actual.data,
      diff2.data,
      size.width,
    ...
    count = (0, import_pixelmatch.default)(
      expected.data,
      actual.data,
      diff2.data,
      size.width,
  ...
  const maxDiffPixels2 =
    options.maxDiffPixelRatio !== void 0
  ...
  if (pixelsMismatchError || sizesMismatchError)
    return {
      ...
      diff: import_utilsBundle3.PNG.sync.write(diff2),
    };
  return null;
}
...
function compareText(actual, expectedBuffer) {
  ...
  const lines = import_utilsBundle2.diff
    .createPatch('file', expected, actual, void 0, void 0, { context: 5 })
    .split('\n');
  const coloredLines = lines.slice(4).map(line => {
    if (line.startsWith('-')) return import_utilsBundle2.colors.green(line);
    if (line.startsWith('+')) return import_utilsBundle2.colors.red(line);
    if (line.startsWith('@@')) return import_utilsBundle2.colors.dim(line);
    return line;
  });
