...
function calculatePlatform() {
  ...
  if (platform === 'darwin') {
    ...
    if (ver[0] < 18) {
      ...
      if (import_os.default.cpus().some(cpu => cpu.model.includes('Apple'))) macVersion += '-arm64';
    }
  ...
  if (platform === 'linux') {
    ...
    if (distroInfo?.id === 'debian' || distroInfo?.id === 'raspbian') {
      ...
      if (distroInfo?.version === '13')
        return {
          hostPlatform: 'debian13' + archSuffix,
          isOfficiallySupportedPlatform: isOfficiallySupportedPlatform2,
        };
  ...
  if (platform === 'win32') return { hostPlatform: 'win64', isOfficiallySupportedPlatform: true };
  return { hostPlatform: '<unknown>', isOfficiallySupportedPlatform: false };
}
const { hostPlatform, isOfficiallySupportedPlatform } = calculatePlatform();
function toShortPlatform(hostPlatform2) {
  if (hostPlatform2 === '<unknown>') return '<unknown>';
  if (hostPlatform2 === 'win64') return 'win-x64';
