...
function compare(actual, expected, diff, width, height, options = {}) {
  const { maxColorDeltaE94 = 1 } = options;
  ...
  for (let y = paddingSize; y < r1.height - paddingSize; ++y) {
    for (let x = paddingSize; x < r1.width - paddingSize; ++x) {
      ...
      const ssimRGB =
        ((0, import_stats.ssim)(fastR, ssimX1, ssimY1, ssimX2, ssimY2) +
          (0, import_stats.ssim)(fastG, ssimX1, ssimY1, ssimX2, ssimY2) +
          (0, import_stats.ssim)(fastB, ssimX1, ssimY1, ssimX2, ssimY2)) /
        3;
