...
class CRExecutionContext {
  ...
  async rawEvaluateJSON(expression) {
    ...
    if (exceptionDetails)
      throw new js.JavaScriptErrorInEvaluate(
        (0, import_crProtocolHelper.getExceptionMessage)(exceptionDetails)
      );
  ...
  async evaluateWithArguments(expression, returnByValue, utilityScript, values, handles) {
    ...
    return returnByValue
      ? (0, import_utilityScriptSerializers.parseEvaluationResultValue)(remoteObject.value)
      : createHandle(utilityScript._context, remoteObject);
  }
...
function rewriteError(error) {
  ...
  if (
    ...
    !(0, import_protocolError.isSessionClosedError)(error)
  )
