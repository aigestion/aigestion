...
class DragManager {
  ...
  async interceptDragCausedByMove(progress, x, y, button, buttons, modifiers, moveCallback) {
    ...
    if (this._dragState) {
      await progress.race(
        this._crPage._mainFrameSession._client.send('Input.dispatchDragEvent', {
          type: 'dragOver',
          x,
          y,
    ...
    try {
      ...
      await progress.race(
        this._crPage._page.safeNonStallingEvaluateInAllFrames(
        ...
        expectingDrag = (
          await Promise.all(
            this._crPage._page.frames().map(async frame => {
              return frame
                .nonStallingEvaluateInExistingContext('window.__cleanupDrag?.()', 'utility')
                .catch(() => false);
            })
          )
        ).some(x2 => x2);
      ...
      this._crPage._page
        .safeNonStallingEvaluateInAllFrames('window.__cleanupDrag?.()', 'utility')
        .catch(() => {});
      throw error;
    }
    if (this._dragState) {
