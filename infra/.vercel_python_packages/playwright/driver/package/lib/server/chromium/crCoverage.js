...
class CRCoverage {
  ...
  async startJSCoverage(progress, options) {
    await (0, import_progress.raceUncancellableOperationWithCleanup)(
      progress,
      () => this._jsCoverage.start(options),
      () => this._jsCoverage.stop()
    );
  }
  ...
  async startCSSCoverage(progress, options) {
    await (0, import_progress.raceUncancellableOperationWithCleanup)(
      progress,
      () => this._cssCoverage.start(options),
      () => this._cssCoverage.stop()
    );
  }
...
class JSCoverage {
  ...
  async stop() {
    if (!this._enabled) return { entries: [] };
    ...
    for (const entry of profileResponse.result) {
      ...
      if (source) coverage.entries.push({ ...entry, source });
