...
class CRNetworkManager {
    ...
    if (this._page) {
      sessionInfo.eventListeners.push(
        ...[
          ...
          import_eventsHelper.eventsHelper.addEventListener(session, 'Network.webSocketClosed', e =>
            this._page.frameManager.webSocketClosed(e.requestId)
          ),
          import_eventsHelper.eventsHelper.addEventListener(
            session,
  ...
  removeSession(session) {
    ...
    if (info) import_eventsHelper.eventsHelper.removeEventListeners(info.eventListeners);
  ...
  async _setExtraHTTPHeadersForSession(info, initial) {
    ...
    await info.session.send('Network.setExtraHTTPHeaders', {
      headers: (0, import_utils.headersArrayToObject)(
        this._extraHTTPHeaders,
        false
        /* lowerCase */
      ),
  ...
  _onRequest(
    ...
    let frame = requestWillBeSentEvent.frameId
      ? this._page?.frameManager.frame(requestWillBeSentEvent.frameId)
    ...
    if (
      !frame &&
      this._page &&
      requestWillBeSentEvent.frameId === (this._page?.delegate)._targetId
    ) {
    ...
    if (requestPausedEvent) {
      if (
        ...
        if (headersOverride) {
          const originalHeaders = Object.entries(requestPausedEvent.request.headers).map(
    ...
    if (route) {
      request.request.setRawRequestHeaders(
        (0, import_utils.headersObjectToArray)(requestPausedEvent.request.headers, '\n')
      );
    }
  ...
  _createResponse(request, responsePayload, hasExtraInfo) {
    ...
    const response = new network.Response(
      ...
      (0, import_utils.headersObjectToArray)(responsePayload.headers),
      timing,
      getResponseBody,
  ...
  _handleRequestRedirect(request, responsePayload, timestamp, hasExtraInfo) {
    ...
    (this._page?.frameManager || this._serviceWorker).reportRequestFinished(
      request.request,
      response
    );
  }
  ...
  _maybeUpdateRequestSession(sessionInfo, request) {
    if (
      ...
      !sessionInfo.isMain &&
      (request._documentId === request._requestId || sessionInfo.workerFrame)
    )
      request.session = sessionInfo.session;
  }
}
class InterceptableRequest {
  constructor(options) {
    ...
    this.request = new network.Request(
      ...
      documentId,
      url,
      toResourceType(requestWillBeSentEvent.type || 'Other'),
      method,
      postDataBuffer,
      headersOverride || (0, import_utils.headersObjectToArray)(headers)
    );
  }
}
class RouteImpl {
...
async function catchDisallowedErrors(callback) {
  try {
    ...
    if (
      (0, import_protocolError.isProtocolError)(e) &&
  ...
  _patchHeaders(info, index) {
    ...
    if (response && requestExtraInfo) {
      response
        ...
        .setRawRequestHeaders(
          (0, import_utils.headersObjectToArray)(requestExtraInfo.headers, '\n')
        );
    ...
    if (response && responseExtraInfo) {
      ...
      response.setRawResponseHeaders(
        (0, import_utils.headersObjectToArray)(responseExtraInfo.headers, '\n')
      );
