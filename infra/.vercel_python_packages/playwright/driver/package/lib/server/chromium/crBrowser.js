...
class CRBrowser extends import_browser.Browser {
  constructor(parent, connection, options) {
    ...
    this._connection.on(import_crConnection.ConnectionEvents.Disconnected, () =>
      this._didDisconnect()
    );
  ...
  static async connect(parent, transport, options, devtools) {
    options = { ...options };
    const connection = new import_crConnection.CRConnection(
      parent,
      transport,
  ...
  async _waitForAllPagesToBeInitialized() {
    await Promise.all(
      [...this._crPages.values()].map(crPage => crPage._page.waitForInitializedOrError())
    );
  }
  _onAttachedToTarget({ targetInfo, sessionId, waitingForDebugger }) {
    ...
    if (targetInfo.type === 'page' || treatOtherAsPage) {
      ...
      const crPage = new import_crPage.CRPage(session, targetInfo.targetId, context, opener, {
    ...
    if (targetInfo.type === 'service_worker') {
      const serviceWorker = new import_crServiceWorker.CRServiceWorker(
        context,
        session,
        targetInfo.url
      );
  ...
  async stopTracing() {
    ...
    const tracingPath = import_path.default.join(
      this.options.artifactsDir,
      (0, import_crypto.createGuid)() + '.crtrace'
    );
    await (0, import_crProtocolHelper.saveProtocolStream)(
      this._tracingClient,
      event.stream,
      tracingPath
    );
...
class CRBrowserContext extends import_browserContext.BrowserContext {
  ...
  async setGeolocation(geolocation) {
    ...
    for (const page of this.pages()) await page.delegate.updateGeolocation();
  }
  async doUpdateExtraHTTPHeaders() {
    for (const page of this.pages()) await page.delegate.updateExtraHTTPHeaders();
    for (const sw of this.serviceWorkers()) await sw.updateExtraHTTPHeaders();
  }
  async setUserAgent(userAgent) {
    this._options.userAgent = userAgent;
    for (const page of this.pages()) await page.delegate.updateUserAgent();
  }
  async doUpdateOffline() {
    for (const page of this.pages()) await page.delegate.updateOffline();
  ...
  async doAddInitScript(initScript) {
    for (const page of this.pages()) await page.delegate.addInitScript(initScript);
  }
  async doRemoveInitScripts(initScripts) {
    for (const page of this.pages()) await page.delegate.removeInitScripts(initScripts);
  }
  async doUpdateRequestInterception() {
    for (const page of this.pages()) await page.delegate.updateRequestInterception();
  ...
  async doExposePlaywrightBinding() {
    for (const page of this._crPages()) await page.exposePlaywrightBinding();
  }
  async doClose(reason) {
    ...
    for (const [targetId, serviceWorker] of this._browser._serviceWorkers) {
      if (serviceWorker.browserContext !== this) continue;
      serviceWorker.didClose();
  ...
  serviceWorkers() {
    return Array.from(this._browser._serviceWorkers.values()).filter(
      serviceWorker => serviceWorker.browserContext === this
    );
  }
