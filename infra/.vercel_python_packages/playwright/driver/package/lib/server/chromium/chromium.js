...
const ARTIFACTS_FOLDER = import_path.default.join(
...
class Chromium extends import_browserType.BrowserType {
  constructor(parent, bidiChromium) {
    ...
    if ((0, import_utils.debugMode)() === 'inspector') this._devtools = this._createDevTools();
  }
  ...
  async _connectOverCDPInternal(progress, endpointURL, options, onClose) {
    ...
    if (!headersMap) headersMap = { 'User-Agent': (0, import_userAgent.getUserAgent)() };
    ...
    try {
      ...
      chromeTransport = await import_transport.WebSocketTransport.connect(progress, wsEndpoint, {
        headers: headersMap,
      });
      ...
      const browser = await progress.race(
        import_crBrowser.CRBrowser.connect(
          this.attribution.playwright,
          chromeTransport,
          browserOptions
        )
      );
      if (!options.isLocal) browser._isCollocatedWithServer = false;
  ...
  _createDevTools() {
    ...
    return directory
      ? new import_crDevTools.CRDevTools(
          import_path.default.join(directory, 'devtools-preferences.json')
        )
      : void 0;
  }
  async connectToTransport(transport, options, browserLogsCollector) {
    try {
      return await import_crBrowser.CRBrowser.connect(
        this.attribution.playwright,
        transport,
        options,
        this._devtools
      ...
      if (
        browserLogsCollector
          .recentLogs()
          .some(log =>
            log.includes('Failed to create a ProcessSingleton for your profile directory.')
          )
      ) {
        throw new Error(
  ...
  doRewriteStartupLog(logs) {
    ...
    if (
      !logs.includes('crbug.com/357670') &&
      !logs.includes('No usable sandbox!') &&
      !logs.includes('crbug.com/638180')
    )
      return logs;
    return [
  ...
  attemptToGracefullyCloseBrowser(transport) {
    const message = {
      ...
      id: import_crConnection.kBrowserCloseMessageId,
      params: {},
    };
  ...
  async _launchWithSeleniumHub(progress, hubUrl, options) {
    ...
    if (process.env.SELENIUM_REMOTE_CAPABILITIES) {
      const remoteCapabilities = parseSeleniumRemoteParams(
        { name: 'capabilities', value: process.env.SELENIUM_REMOTE_CAPABILITIES },
        progress
      );
    ...
    if (process.env.SELENIUM_REMOTE_HEADERS) {
      const remoteHeaders = parseSeleniumRemoteParams(
        { name: 'headers', value: process.env.SELENIUM_REMOTE_HEADERS },
        progress
      );
    ...
    try {
      ...
      if (capabilities['se:cdp']) {
        ...
        const chromeOptionsURL =
          typeof maybeChromeOptions === 'string' ? maybeChromeOptions : void 0;
  ...
  _innerDefaultArgs(options) {
    ...
    const chromeArguments = [
      ...(0, import_chromiumSwitches.chromiumSwitches)(options.assistantMode, options.channel),
    ];
    ...
    if (proxy) {
      ...
      if (options.socksProxyPort) proxyBypassRules.push('<-loopback>');
      if (proxy.bypass)
...
async function waitForReadyState(options, browserLogsCollector) {
  ...
  browserLogsCollector.onMessage(message => {
    ...
    if (match) result.resolve({ wsEndpoint: match[1] });
  });
  return result;
}
async function urlToWSEndpoint(progress, endpointURL, headers) {
  ...
  const json = await (0, import_network.fetchData)(
    ...
    async (_, resp) =>
      new Error(`Unexpected status ${resp.statusCode} when connecting to ${httpURL}.
