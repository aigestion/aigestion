...
class CRPage {
  constructor(client, targetId, browserContext, opener, bits) {
    ...
    this.rawKeyboard = new import_crInput.RawKeyboardImpl(
      client,
    ...
    await Promise.all(
      frameSessions.map(frameSession => {
        ...
        return cb(frameSession).catch(e => {
          if ((0, import_protocolError.isSessionClosedError)(e)) return;
          throw e;
        });
      })
    );
  ...
  async takeScreenshot(progress, format, documentRect, viewportRect, quality, fitsViewport, scale) {
    const { visualViewport } = await progress.race(
  ...
  async getFrameElement(frame) {
    ...
    const { backendNodeId } = await parentSession._client
      .send('DOM.getFrameOwner', { frameId: frame._id })
      .catch(e => {
...
class FrameSession {
  ...
  _addRendererListeners() {
    this._eventListeners.push(
      ...[
        import_eventsHelper.eventsHelper.addEventListener(this._client, 'Log.entryAdded', event =>
  ...
  _addBrowserListeners() {
    this._eventListeners.push(
      ...[
        ...
        import_eventsHelper.eventsHelper.addEventListener(this._client, 'Page.windowOpen', event =>
  ...
  async _initialize(hasUIWindow) {
    ...
    const promises = [
      ...
      this._client.send('Page.getFrameTree').then(({ frameTree }) => {
        ...
        for (const event of attachedToTargetEvents) this._onAttachedToTarget(event);
  ...
  _onLifecycleEvent(event) {
    ...
    if (event.name === 'load') this._page.frameManager.frameLifecycleEvent(event.frameId, 'load');
  ...
  _onFrameAttached(frameId, parentFrameId) {
    ...
    if (frameSession && frameId !== this._targetId) {
      ...
      const frame = this._page.frameManager.frame(frameId);
      if (frame) this._page.frameManager.removeChildFramesRecursively(frame);
      return;
    }
  ...
  _onExecutionContextCreated(contextPayload) {
    const frame = contextPayload.auxData
      ? this._page.frameManager.frame(contextPayload.auxData.frameId)
      : null;
  ...
  _onAttachedToTarget(event) {
    ...
    session.once('Runtime.executionContextCreated', async event2 => {
      worker.createExecutionContext(
        new import_crExecutionContext.CRExecutionContext(session, event2.context)
      );
    });
    ...
    else worker.workerScriptLoaded();
    session._sendMayFail('Runtime.enable');
    this._crPage._networkManager
      .addSession(session, this._page.frameManager.frame(this._targetId) ?? void 0)
      .catch(() => {});
    ...
    session.on('Runtime.consoleAPICalled', event2 => {
      const args = event2.args.map(o =>
        (0, import_crExecutionContext.createHandle)(worker.existingExecutionContext, o)
      );
      this._page.addConsoleMessage(
        worker,
        event2.type,
        args,
        (0, import_crProtocolHelper.toConsoleMessageLocation)(event2.stackTrace)
      );
    });
    session.on('Runtime.exceptionThrown', exception =>
      this._page.addPageError(
        (0, import_crProtocolHelper.exceptionToError)(exception.exceptionDetails)
      )
    );
  }
  _onDetachedFromTarget(event) {
    ...
    this._client
      ...
      .catch(e => null)
      .then(() => {
        if (!childFrameSession._swappedIn) this._page.frameManager.frameDetached(event.targetId);
        childFrameSession.dispose();
      });
  }
  _onWindowOpen(event) {
  ...
  async _onConsoleAPI(event) {
    ...
    this._page.addConsoleMessage(
      ...
      event.type,
      values,
      (0, import_crProtocolHelper.toConsoleMessageLocation)(event.stackTrace)
    );
  }
  ...
  _onDialog(event) {
    if (!this._page.frameManager.frame(this._targetId)) return;
  ...
  async _updateViewport(preserveWindowBoundaries) {
    ...
    if (!preserveWindowBoundaries && this._windowId) {
      ...
      if (this._crPage._browserContext._browser.options.headful) {
        ...
        if (process.platform === 'win32') insets = { width: 16, height: 88 };
        else if (process.platform === 'linux') insets = { width: 8, height: 85 };
        else if (process.platform === 'darwin') insets = { width: 2, height: 80 };
  ...
  async _setDefaultFontFamilies(session) {
    const fontFamilies =
      import_defaultFontFamilies.platformToFontFamilies[
  ...
  async _framePosition() {
    ...
    if (frame === this._page.mainFrame()) return { x: 0, y: 0 };
