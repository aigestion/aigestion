...
class FFPage {
  constructor(session, browserContext, opener) {
    ...
    this._page.on(import_page2.Page.Events.FrameDetached, frame =>
    ...
    this._eventListeners = [
      ...
      import_eventsHelper.eventsHelper.addEventListener(this._session, 'Page.linkClicked', event =>
    ...
    if (videoOptions)
      screencast
        .startVideoRecording(videoOptions)
    ...
    this.addInitScript(new import_page.InitScript(''), UTILITY_WORLD_NAME).catch(e =>
      this._markAsError(e)
    );
  }
  ...
  _onWebSocketCreated(event) {
    this._page.frameManager.onWebSocketCreated(
  ...
  _onExecutionContextCreated(payload) {
    ...
    if (!frame) return;
    const delegate = new import_ffExecutionContext.FFExecutionContext(
      this._session,
      executionContextId
    );
    let worldName = null;
  ...
  _onLinkClicked(phase) {
    if (phase === 'before') this._page.frameManager.frameWillPotentiallyRequestNavigation();
  ...
  _onNavigationCommitted(params) {
    for (const [workerId, worker] of this._workers) {
      if (worker.frameId === params.frameId) this._onWorkerDestroyed({ workerId });
    }
  ...
  _onEventFired(payload) {
    ...
    if (name === 'load') this._page.frameManager.frameLifecycleEvent(frameId, 'load');
  ...
  _onConsole(payload) {
    ...
    this._page.addConsoleMessage(
      ...
      args.map(arg => (0, import_ffExecutionContext.createHandle)(context, arg)),
      location
    );
  }
  ...
  async _onWorkerCreated(event) {
    ...
    const workerSession = new import_ffConnection.FFSession(
        ...
        this._session
          ...
          .catch(e => {
            workerSession.dispatchMessage({
              ...
              method: '',
              params: {},
              error: { message: e.message, data: void 0 },
            });
          });
      }
    );
    ...
    workerSession.once('Runtime.executionContextCreated', event2 => {
      worker.createExecutionContext(
        new import_ffExecutionContext.FFExecutionContext(workerSession, event2.executionContextId)
      );
  ...
  async updateEmulateMedia() {
    ...
    const colorScheme =
      emulatedMedia.colorScheme === 'no-override' ? void 0 : emulatedMedia.colorScheme;
    const reducedMotion =
      emulatedMedia.reducedMotion === 'no-override' ? void 0 : emulatedMedia.reducedMotion;
    const forcedColors =
      emulatedMedia.forcedColors === 'no-override' ? void 0 : emulatedMedia.forcedColors;
  ...
  async takeScreenshot(progress, format, documentRect, viewportRect, quality, fitsViewport, scale) {
    if (!documentRect) {
      const scrollOffset = await this._page
        .mainFrame()
        .waitForFunctionValueInUtility(progress, () => ({ x: window.scrollX, y: window.scrollY }));
      documentRect = {
