...
class FFNetworkManager {
  ...
  _onRequestWillBeSent(event) {
    ...
    const frame = redirectedFrom
      ...
      : event.frameId
        ...
        : null;
    if (!frame) return;
    if (event.method === 'OPTIONS' && !event.isIntercepted) return;
  ...
  _onRequestFinished(event) {
    ...
    const responseEndTime = event.responseEndTime
      ? event.responseEndTime / 1e3 - response.timing().startTime
      : -1;
    if (isRedirected) {
...
function parseMultivalueHeaders(headers) {
  ...
  for (const header of headers) {
    ...
    for (const token of tokens) result.push({ name: header.name, value: token });
  }
  return result;
}
