...
class FFBrowser extends import_browser.Browser {
  constructor(parent, connection, options) {
    ...
    this._connection.on(import_ffConnection.ConnectionEvents.Disconnected, () =>
      this._onDisconnect()
    );
  ...
  static async connect(parent, transport, options) {
    const connection = new import_ffConnection.FFConnection(
      transport,
...
class FFBrowserContext extends import_browserContext.BrowserContext {
  ...
  async _initialize() {
    ...
    const promises = [super._initialize(), this._updateInitScripts()];
    ...
    if (this._options.hasTouch)
      promises.push(
        this._browser.session.send('Browser.setTouchOverride', { browserContextId, hasTouch: true })
      );
    ...
    if (this._options.bypassCSP)
      promises.push(
        this._browser.session.send('Browser.setBypassCSP', { browserContextId, bypassCSP: true })
      );
    ...
    if (this._options.recordVideo) {
      promises.push(
        this._browser.session.send('Browser.setScreencastOptions', {
  ...
  async doUpdateOffline() {
    await this._browser.session.send('Browser.setOnlineOverride', {
      browserContextId: this._browserContextId,
      override: this._options.offline ? 'offline' : 'online',
    });
  }
  ...
  async _updateInitScripts() {
    ...
    if (this.bindingsInitScript) bindingScripts.unshift(this.bindingsInitScript.source);
    const initScripts = this.initScripts.map(script => script.source);
    await this._browser.session.send('Browser.setInitScripts', {
      browserContextId: this._browserContextId,
      scripts: [...bindingScripts, ...initScripts].map(script => ({ script })),
    });
  }
  async doUpdateRequestInterception() {
    await Promise.all([
      this._browser.session.send('Browser.setRequestInterception', {
        browserContextId: this._browserContextId,
        enabled: this.requestInterceptors.length > 0,
      }),
      this._browser.session.send('Browser.setCacheDisabled', {
        browserContextId: this._browserContextId,
        cacheDisabled: this.requestInterceptors.length > 0,
      }),
    ]);
  }
  ...
  async doUpdateDefaultEmulatedMedia() {
    ...
    if (this._options.reducedMotion !== 'no-override') {
      await this._browser.session.send('Browser.setReducedMotion', {
        browserContextId: this._browserContextId,
        reducedMotion:
          this._options.reducedMotion !== void 0 ? this._options.reducedMotion : 'no-preference',
      });
    }
  ...
  async doExposePlaywrightBinding() {
    this._browser.session.send('Browser.addBinding', {
      browserContextId: this._browserContextId,
      name: import_page.PageBinding.kBindingName,
      script: '',
    });
  }
  onClosePersistent() {}
  ...
  async doClose(reason) {
    if (!this._browserContextId) {
      if (this._options.recordVideo)
        await Promise.all(
          this._ffPages().map(ffPage => ffPage._page.screencast.stopVideoRecording())
        );
