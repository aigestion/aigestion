...
async function launchApp(browserType, options) {
  ...
  let context;
  try {
    context = await controller.run(
      progress =>
        browserType.launchPersistentContext(progress, '', {
          ignoreDefaultArgs: ['--enable-automation'],
          ...options?.persistentContextOptions,
          channel,
          noDefaultViewport: options.persistentContextOptions?.noDefaultViewport ?? true,
          acceptDownloads:
            options?.persistentContextOptions?.acceptDownloads ??
            ((0, import_utils.isUnderTest)() ? 'accept' : 'internal-browser-default'),
    ...
    if (channel) {
      error = (0, import_utils.rewriteErrorMessage)(
        error,
        [
          ...
          (0, import_utils.wrapInASCIIBox)(
            `${(0, import_registry.buildPlaywrightCLICommand)(options.sdkLanguage, 'install')}`,
            2
          ),
        ].join('\n')
      );
...
async function syncLocalStorageWithSettings(page, appName) {
  ...
  const settingsFile = import_path.default.join(
    import_registry2.registryDirectory,
    '.settings',
    `${appName}.json`
  );
  const controller = new import_progress.ProgressController();
  await controller.run(async progress => {
    ...
    const settings = await import_fs.default.promises
