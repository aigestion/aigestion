...
class Android extends import_instrumentation.SdkObject {
  ...
  async devices(progress, options) {
    const devices = (await progress.race(this._backend.devices(options))).filter(
      d => d.status === 'device'
    );
    ...
    for (const d of devices) {
      ...
      await progress.race(
        AndroidDevice.create(this, d, options).then(device => this._devices.set(d.serial, device))
      );
    }
...
class AndroidDevice extends import_instrumentation.SdkObject {
  ...
  async _installDriver(progress) {
    ...
    if (!this._options.omitDriverInstall) {
      ...
      await progress.race(
        this.shell(`cmd package uninstall com.microsoft.playwright.androiddriver`)
      );
      await progress.race(
        this.shell(`cmd package uninstall com.microsoft.playwright.androiddriver.test`)
      );
      ...
      for (const file of ['android-driver.apk', 'android-driver-target.apk']) {
        ...
        await this.installApk(
          progress,
          await progress.race(import_fs.default.promises.readFile(fullName))
        );
      }
    } else {
    ...
    ).catch(e => (0, import_utilsBundle.debug)('pw:android')(e));
  ...
  async launchBrowser(progress, pkg = 'com.android.chrome', options) {
    ...
    const socketName = (0, import_debug.isUnderTest)()
      ? 'webview_devtools_remote_playwright_test'
      : 'playwright_' + (0, import_crypto.createGuid)() + '_devtools_remote';
    const commandLine = this._defaultArgs(options, socketName).join(' ');
    (0, import_utilsBundle.debug)('pw:android')('Starting', pkg, commandLine);
    await progress.race(
      this._backend.runCommand(
        `shell:echo "${Buffer.from(commandLine).toString('base64')}" | base64 -d > /data/local/tmp/chrome-command-line`
      )
    );
    await progress.race(
      this._backend.runCommand(`shell:am start -a android.intent.action.VIEW -d about:blank ${pkg}`)
    );
  ...
  async _connectToBrowser(progress, socketName, options = {}) {
    ...
    try {
      ...
      const artifactsDir = await progress.race(
        import_fs.default.promises.mkdtemp(ARTIFACTS_FOLDER)
      );
      const cleanupArtifactsDir = async () => {
        const errors = (await (0, import_fileUtils.removeFolders)([artifactsDir])).filter(Boolean);
        for (let i = 0; i < (errors || []).length; ++i)
          (0, import_utilsBundle.debug)('pw:android')(
      ...
      socket.on('close', async () => {
        ...
        cleanupArtifactsDir().catch(e =>
          (0, import_utilsBundle.debug)('pw:android')(`could not cleanup artifacts dir: ${e}`)
        );
      });
  ...
  _open(progress, command) {
    return (0, import_progress.raceUncancellableOperationWithCleanup)(
      progress,
      () => this._backend.open(command),
      socket => socket.close()
    );
  }
  webViews() {
  ...
  async installApk(progress, content, options) {
    ...
    const installSocket = await this._open(
      progress,
