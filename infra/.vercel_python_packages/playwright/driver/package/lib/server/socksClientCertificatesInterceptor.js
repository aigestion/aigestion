...
class SocksProxyConnection {
  constructor(socksProxy, uid, host, port) {
    ...
    this._browserEncrypted = new import_stream.default.Duplex({
      ...
      destroy: (error, callback) => {
        if (error) socksProxy._socksProxy.sendSocketError({ uid: this.uid, error: error.message });
        else socksProxy._socksProxy.sendSocketEnd({ uid: this.uid });
        callback();
      },
    });
  }
  async connect() {
    ...
    if (proxyAgent)
      this._serverEncrypted = await proxyAgent.connect(new import_events.EventEmitter(), {
        host: rewriteToLocalhostIfNeeded(this.host),
        port: this.port,
        secureEndpoint: false,
      });
    else
      this._serverEncrypted = await (0, import_happyEyeballs.createSocket)(
  ...
  onData(data) {
    if (!this._firstPackageReceived) {
      ...
      if (data[0] === 22) this._establishTlsTunnel(this._browserEncrypted, data);
      else this._establishPlaintextTunnel(this._browserEncrypted);
    }
  ...
  _establishTlsTunnel(browserEncrypted, clientHello) {
    ...
    import_debugLogger.debugLogger.log(
      'client-certificates',
      `Browser->Proxy ${this.host}:${this.port} offers ALPN ${browserALPNProtocols}`
    );
    const serverDecrypted = import_tls.default.connect(
      ...
      async () => {
        const browserDecrypted = await this._upgradeToTLSIfNeeded(
          browserEncrypted,
          serverDecrypted.alpnProtocol
        );
        import_debugLogger.debugLogger.log(
          'client-certificates',
          `Proxy->Server ${this.host}:${this.port} chooses ALPN ${browserDecrypted.alpnProtocol}`
        );
        ...
        if (this._closed) serverDecrypted.destroy();
      }
    );
    serverDecrypted.once('error', async error => {
      import_debugLogger.debugLogger.log(
        'client-certificates',
        `error when connecting to server: ${error.message.replaceAll('\n', ' ')}`
      );
      this._serverEncrypted.removeListener('close', this._serverCloseEventListener);
      this._serverEncrypted.destroy();
      const browserDecrypted = await this._upgradeToTLSIfNeeded(
        this._browserEncrypted,
        serverDecrypted.alpnProtocol
      );
      const responseBody = (0, import_utils.escapeHTML)(
...
class ClientCertificatesProxy {
  constructor(contextOptions) {
    ...
    this._socksProxy.addListener(
      import_socksProxy.SocksProxy.Events.SocksRequested,
      async payload => {
        try {
          const connection = new SocksProxyConnection(
            this,
            payload.uid,
            payload.host,
            payload.port
          ...
          import_debugLogger.debugLogger.log(
            'client-certificates',
            `Failed to connect to ${payload.host}:${payload.port}: ${error.message}`
          );
  ...
  getProxyAgent(host, port) {
    ...
    if (proxyFromOptions) return proxyFromOptions;
    const proxyFromEnv = (0, import_utilsBundle.getProxyForUrl)(`https://${host}:${port}`);
    if (proxyFromEnv) return (0, import_network.createProxyAgent)({ server: proxyFromEnv });
  }
    ...
    for (const [origin, certs] of origin2certs) {
      try {
        this.secureContextMap.set(
          origin,
          import_tls.default.createSecureContext(convertClientCertificatesToTLSOptions(certs))
        );
      } catch (error) {
        error = rewriteOpenSSLErrorIfNeeded(error);
        throw (0, import_utils.rewriteErrorMessage)(
          error,
...
function rewriteOpenSSLErrorIfNeeded(error) {
  if (error.message !== 'unsupported' && error.code !== 'ERR_CRYPTO_UNSUPPORTED_OPERATION')
    return error;
  return (0, import_utils.rewriteErrorMessage)(
    error,
    [
...
function parseALPNFromClientHello(buffer) {
  ...
  if (buffer[0] !== 22) return null;
  let offset = 5;
  if (buffer[offset] !== 1) return null;
  offset += 4;
  offset += 2;
  offset += 32;
  ...
  if (extensionsEnd > buffer.length) return null;
...
function parseALPNExtension(buffer) {
  ...
  if (listLength !== buffer.length - 2) return null;
  const protocols = [];
  let offset = 2;
  while (offset < buffer.length) {
    const protocolLength = buffer[offset];
    offset += 1;
    if (offset + protocolLength > buffer.length) break;
