...
class Dispatcher extends import_events.EventEmitter {
  constructor(parent, object, type, initializer, gcBucket) {
    ...
    if (this._parent) this.connection.sendCreate(this._parent, type, guid, initializer);
  ...
  addObjectListener(eventName, handler) {
    this._eventListeners.push(
      import_eventsHelper.eventsHelper.addEventListener(this._object, eventName, handler)
    );
  }
  adopt(child) {
  ...
  async _runCommand(callMetadata, method, validParams) {
    const controller = import_progress.ProgressController.createForSdkObject(
      this._object,
      callMetadata
    );
    ...
    try {
      return await controller.run(
        progress => this[method](validParams, progress),
        validParams?.timeout
      );
    } finally {
  ...
  async stopPendingOperations(error) {
    ...
    const collect = dispatcher => {
      ...
      for (const child of [...dispatcher._dispatchers.values()]) collect(child);
    };
    collect(this);
  ...
  _disposeRecursively(error) {
    ...
    for (const controller of this._activeProgressControllers) {
      if (!controller.metadata.potentiallyClosesScope) controller.abort(error).catch(() => {});
    }
    this._onDispose();
    this._disposed = true;
...
class DispatcherConnection {
  ...
  _tChannelImplFromWire(names, arg, path, context) {
    if (arg && typeof arg === 'object' && typeof arg.guid === 'string') {
      ...
      if (names !== '*' && !names.includes(dispatcher._type))
        throw new import_validator.ValidationError(
  ...
  async dispatch(message) {
    ...
    if (!dispatcher) {
      this.onmessage({
        id,
        error: (0, import_errors.serializeError)(new import_errors.TargetClosedError(void 0)),
      });
      return;
    }
    let validParams;
    ...
    try {
      ...
      if ((0, import_errors.isTargetClosedError)(e)) {
        ...
        if (reason) (0, import_utils.rewriteErrorMessage)(e, reason);
      ...
      if (metainfo?.slowMo) await this._doSlowMo(sdkObject);
    }
    if (response.error) response.log = (0, import_callLog.compressCallLog)(callMetadata.log);
    this.onmessage(response);
  }
  async _doSlowMo(sdkObject) {
    const slowMo = sdkObject.attribution.browser?.options.slowMo;
    if (slowMo) await new Promise(f => setTimeout(f, slowMo));
  }
}
