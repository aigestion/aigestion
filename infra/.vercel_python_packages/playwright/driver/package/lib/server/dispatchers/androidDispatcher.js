...
class AndroidDeviceDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, device) {
    ...
    this.addObjectListener(import_android.AndroidDevice.Events.WebViewAdded, webView =>
      this._dispatchEvent('webViewAdded', { webView })
    );
    this.addObjectListener(import_android.AndroidDevice.Events.WebViewRemoved, socketName =>
      this._dispatchEvent('webViewRemoved', { socketName })
    );
  ...
  async inputType(params, progress) {
    ...
    await progress.race(
      Promise.all(keyCodes.map(keyCode => this._object.send('inputPress', { keyCode })))
    );
  }
  ...
  async connectToWebView(params, progress) {
    return {
      context: import_browserContextDispatcher.BrowserContextDispatcher.from(
        this,
        await this._object.connectToWebView(progress, params.socketName)
      ),
    };
  }
}
class SocketSdkObject extends import_instrumentation.SdkObject {
  constructor(parent, socket) {
    ...
    this._eventListeners = [
      import_eventsHelper.eventsHelper.addEventListener(socket, 'data', data =>
        this.emit('data', data)
      ),
