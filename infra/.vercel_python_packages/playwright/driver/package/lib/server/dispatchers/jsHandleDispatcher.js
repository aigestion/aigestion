...
class JSHandleDispatcher extends import_dispatcher.Dispatcher {
  ...
  async evaluateExpression(params, progress) {
    const jsHandle = await progress.race(
  ...
  async getPropertyList(params, progress) {
    ...
    for (const [name, value] of map) {
      properties.push({
        name,
        value: import_elementHandlerDispatcher.ElementHandleDispatcher.fromJSOrElementHandle(
          this.parentScope(),
          value
        ),
      });
...
function parseArgument(arg) {
  return (0, import_serializers.parseSerializedValue)(
    arg.value,
