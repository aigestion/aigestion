...
class BrowserDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, browser, options = {}) {
    ...
    if (!options.isolateContexts) {
      this.addObjectListener(import_browser.Browser.Events.Context, context =>
        this._dispatchEvent('context', {
          context: import_browserContextDispatcher.BrowserContextDispatcher.from(this, context),
        })
      );
      this.addObjectListener(import_browser.Browser.Events.Disconnected, () => this._didClose());
      if (browser._defaultContext)
        this._dispatchEvent('context', {
          context: import_browserContextDispatcher.BrowserContextDispatcher.from(
            this,
  ...
  async newContext(params, progress) {
    ...
    if (!this._options.isolateContexts) {
      ...
      const contextDispatcher2 = import_browserContextDispatcher.BrowserContextDispatcher.from(
        this,
        context2
      );
    ...
    context.on(import_browserContext.BrowserContext.Events.Close, () =>
      this._isolatedContexts.delete(context)
    );
    const contextDispatcher = import_browserContextDispatcher.BrowserContextDispatcher.from(
      this,
      context
    );
  ...
  async close(params, progress) {
    if (this._options.ignoreStopAndKill) return;
  ...
  async stopTracing(params, progress) {
    ...
    const crBrowser = this._object;
    return {
      artifact: import_artifactDispatcher.ArtifactDispatcher.from(
        this,
        await crBrowser.stopTracing()
      ),
    };
  }
  async cleanupContexts() {
    await Promise.all(
      Array.from(this._isolatedContexts).map(context =>
        context.close({ reason: 'Global context cleanup (connection terminated)' })
      )
    );
  }
}
