...
class PageDispatcher extends import_dispatcher.Dispatcher {
  constructor(parentScope, page) {
    ...
    this._requestInterceptor = (route, request) => {
      ...
      this._dispatchEvent('route', {
        route: new import_networkDispatchers3.RouteDispatcher(
          import_networkDispatchers.RequestDispatcher.from(this.parentScope(), request),
          route
        ),
      });
    };
    ...
    this.addObjectListener(import_page.Page.Events.Download, download => {
      this._dispatchEvent('download', {
        ...
        artifact: import_artifactDispatcher.ArtifactDispatcher.from(parentScope, download.artifact),
      });
    });
    this.addObjectListener(import_page.Page.Events.EmulatedSizeChanged, () =>
      this._dispatchEvent('viewportSizeChanged', { viewportSize: page.emulatedSize()?.viewport })
    );
    this.addObjectListener(import_page.Page.Events.FileChooser, fileChooser =>
    ...
    this.addObjectListener(import_page.Page.Events.FrameAttached, frame =>
      this._onFrameAttached(frame)
    );
    this.addObjectListener(import_page.Page.Events.FrameDetached, frame =>
      this._onFrameDetached(frame)
    );
    this.addObjectListener(import_page.Page.Events.LocatorHandlerTriggered, uid =>
      this._dispatchEvent('locatorHandlerTriggered', { uid })
    );
    this.addObjectListener(import_page.Page.Events.WebSocket, webSocket =>
      this._dispatchEvent('webSocket', {
        webSocket: new import_networkDispatchers3.WebSocketDispatcher(this, webSocket),
      })
    );
    this.addObjectListener(import_page.Page.Events.Worker, worker =>
      this._dispatchEvent('worker', { worker: new WorkerDispatcher(this, worker) })
    );
    this.addObjectListener(import_page.Page.Events.Video, artifact =>
      this._dispatchEvent('video', {
        artifact: import_artifactDispatcher.ArtifactDispatcher.from(parentScope, artifact),
      })
    );
    if (page.video)
      this._dispatchEvent('video', {
        artifact: import_artifactDispatcher.ArtifactDispatcher.from(this.parentScope(), page.video),
      });
  ...
  async exposeBinding(params, progress) {
    const binding = await this._page.exposeBinding(
      progress,
      params.name,
      !!params.needsHandle,
      (source, ...args) => {
        if (this._disposed) return;
        const binding2 = new BindingCallDispatcher(
          this,
          params.name,
          !!params.needsHandle,
          source,
  ...
  async reload(params, progress) {
    return {
      response: import_networkDispatchers2.ResponseDispatcher.fromNullable(
        this.parentScope(),
        await this._page.reload(progress, params)
      ),
    };
  }
  async goBack(params, progress) {
    return {
      response: import_networkDispatchers2.ResponseDispatcher.fromNullable(
        this.parentScope(),
        await this._page.goBack(progress, params)
      ),
    };
  }
  async goForward(params, progress) {
    return {
      response: import_networkDispatchers2.ResponseDispatcher.fromNullable(
        this.parentScope(),
        await this._page.goForward(progress, params)
      ),
    };
  }
  ...
  async setNetworkInterceptionPatterns(params, progress) {
    ...
    if (!params.patterns.length) {
      if (hadMatchers) await this._page.removeRequestInterceptor(this._requestInterceptor);
      this._interceptionUrlMatchers = [];
    } else {
      ...
      if (!hadMatchers) await this._page.addRequestInterceptor(progress, this._requestInterceptor);
    }
  }
  ...
  async close(params, progress) {
    if (!params.runBeforeUnload) progress.metadata.potentiallyClosesScope = true;
  ...
  async consoleMessages(params, progress) {
    ...
    return {
      messages: this._page
        .consoleMessages()
        .map(message => this.parentScope().serializeConsoleMessage(message, this)),
    };
  }
  async pageErrors(params, progress) {
    return {
      errors: this._page.pageErrors().map(error => (0, import_errors.serializeError)(error)),
    };
  }
  ...
  _onFrameAttached(frame) {
    this._dispatchEvent('frameAttached', {
      frame: import_frameDispatcher.FrameDispatcher.from(this.parentScope(), frame),
    });
  }
  ...
  _onDispose() {
    if (this._page.isClosedOrClosingOrCrashed()) return;
    this._interceptionUrlMatchers = [];
    ...
    for (const uid of this._locatorHandlers) this._page.unregisterLocatorHandler(uid);
    this._locatorHandlers.clear();
    this._page.setFileChooserInterceptedBy(false, this).catch(() => {});
    if (this._jsCoverageActive) this._page.coverage.stopJSCoverage().catch(() => {});
    this._jsCoverageActive = false;
    if (this._cssCoverageActive) this._page.coverage.stopCSSCoverage().catch(() => {});
    this._cssCoverageActive = false;
  }
}
class WorkerDispatcher extends import_dispatcher.Dispatcher {
  ...
  async evaluateExpressionHandle(params, progress) {
    return {
      handle: import_jsHandleDispatcher.JSHandleDispatcher.fromJSHandle(
        this,
        await progress.race(
...
class BindingCallDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, name, needsHandle, source, args) {
    const frameDispatcher = import_frameDispatcher.FrameDispatcher.from(
      scope.parentScope(),
      source.frame
    );
    super(
      scope,
      new import_instrumentation.SdkObject(scope._object, 'bindingCall'),
      'BindingCall',
      {
        ...
        handle: needsHandle
          ? import_elementHandlerDispatcher.ElementHandleDispatcher.fromJSOrElementHandle(
              frameDispatcher,
              args[0]
            )
          : void 0,
