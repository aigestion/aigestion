...
class BrowserContextDispatcher extends import_dispatcher.Dispatcher {
  constructor(parentScope, context) {
    const requestContext = import_networkDispatchers.APIRequestContextDispatcher.from(
      parentScope,
    ...
    this._requestInterceptor = (route, request) => {
      const matchesSome = this._interceptionUrlMatchers.some(urlMatch =>
        (0, import_urlMatch.urlMatches)(this._context._options.baseURL, request.url(), urlMatch)
      );
      ...
      this._dispatchEvent('route', {
        route: new import_networkDispatchers.RouteDispatcher(
          import_networkDispatchers.RequestDispatcher.from(this, request),
          route
        ),
      });
    };
    ...
    const onVideo = artifact => {
      const artifactDispatcher = import_artifactDispatcher.ArtifactDispatcher.from(
        parentScope,
        artifact
      );
    ...
    this.addObjectListener(import_browserContext.BrowserContext.Events.PageError, (error, page) => {
      this._dispatchEvent('pageError', {
        error: (0, import_errors.serializeError)(error),
        page: import_pageDispatcher.PageDispatcher.from(this, page),
      });
    });
    this.addObjectListener(import_browserContext.BrowserContext.Events.Console, message => {
      const pageDispatcher = import_pageDispatcher.PageDispatcher.fromNullable(
        this,
        message.page()
      );
      const workerDispatcher = import_pageDispatcher.WorkerDispatcher.fromNullable(
        this,
        message.worker()
      );
      if (
        this._shouldDispatchEvent(message.page(), 'console') ||
        workerDispatcher?._subscriptions.has('console')
      ) {
    ...
    this._dialogHandler = dialog => {
      if (!this._shouldDispatchEvent(dialog.page(), 'dialog')) return false;
      this._dispatchEvent('dialog', {
        dialog: new import_dialogDispatcher.DialogDispatcher(this, dialog),
      });
      return true;
    };
    context.dialogManager.addDialogHandler(this._dialogHandler);
    if (
      context._browser.options.name === 'chromium' &&
      this._object._browser instanceof import_crBrowser.CRBrowser
    ) {
      for (const serviceWorker of context.serviceWorkers())
        this._dispatchEvent('serviceWorker', {
          worker: new import_pageDispatcher.WorkerDispatcher(this, serviceWorker),
        });
      this.addObjectListener(
        import_crBrowser.CRBrowserContext.CREvents.ServiceWorker,
        serviceWorker =>
          this._dispatchEvent('serviceWorker', {
            worker: new import_pageDispatcher.WorkerDispatcher(this, serviceWorker),
          })
    ...
    this.addObjectListener(import_browserContext.BrowserContext.Events.Request, request => {
      const redirectFromDispatcher =
        request.redirectedFrom() && this.connection.existingDispatcher(request.redirectedFrom());
      if (
      ...
      this._dispatchEvent('request', {
        ...
        page: import_pageDispatcher.PageDispatcher.fromNullable(
          this,
    ...
    this.addObjectListener(
      import_browserContext.BrowserContext.Events.RequestFinished,
      ({ request, response }) => {
        const requestDispatcher = this.connection.existingDispatcher(request);
    ...
    this.addObjectListener(
      import_browserContext.BrowserContext.Events.RecorderEvent,
      ({ event, data, page, code }) => {
        this._dispatchEvent('recorderEvent', {
          event,
          data,
          code,
          page: import_pageDispatcher.PageDispatcher.from(this, page),
  ...
  serializeConsoleMessage(message, jsScope) {
    return {
      ...
      args: message.args().map(a => {
        ...
        if (elementHandle)
          return import_elementHandlerDispatcher.ElementHandleDispatcher.from(
            import_frameDispatcher.FrameDispatcher.from(this, elementHandle._frame),
            elementHandle
          );
  ...
  async createTempFiles(params, progress) {
    ...
    const tempDirWithRootName = params.rootDirName
      ? import_path.default.join(tmpDir, import_path.default.basename(params.rootDirName))
      : tmpDir;
    await progress.race(import_fs.default.promises.mkdir(tempDirWithRootName, { recursive: true }));
    this._context._tempDirs.push(tmpDir);
    return {
      rootDir: params.rootDirName
        ? new import_writableStreamDispatcher.WritableStreamDispatcher(this, tempDirWithRootName)
        : void 0,
      writableStreams: await Promise.all(
        params.items.map(async item => {
          await progress.race(
            import_fs.default.promises.mkdir(
              import_path.default.dirname(import_path.default.join(tempDirWithRootName, item.name)),
              { recursive: true }
            )
          );
          const file = import_fs.default.createWriteStream(
            import_path.default.join(tempDirWithRootName, item.name)
          );
          return new import_writableStreamDispatcher.WritableStreamDispatcher(
            this,
            file,
            item.lastModifiedMs
          );
  ...
  async exposeBinding(params, progress) {
    const binding = await this._context.exposeBinding(
      progress,
      params.name,
      !!params.needsHandle,
      (source, ...args) => {
        ...
        const binding2 = new import_pageDispatcher.BindingCallDispatcher(
          pageDispatcher,
          params.name,
          !!params.needsHandle,
          source,
  ...
  async newPage(params, progress) {
    return {
      page: import_pageDispatcher.PageDispatcher.from(this, await this._context.newPage(progress)),
    };
  }
  ...
  async clearCookies(params, progress) {
    const nameRe =
      params.nameRegexSource !== void 0 && params.nameRegexFlags !== void 0
        ? new RegExp(params.nameRegexSource, params.nameRegexFlags)
        : void 0;
    const domainRe =
      params.domainRegexSource !== void 0 && params.domainRegexFlags !== void 0
        ? new RegExp(params.domainRegexSource, params.domainRegexFlags)
        : void 0;
    const pathRe =
      params.pathRegexSource !== void 0 && params.pathRegexFlags !== void 0
  ...
  async setNetworkInterceptionPatterns(params, progress) {
    ...
    if (!params.patterns.length) {
      if (hadMatchers) await this._context.removeRequestInterceptor(this._requestInterceptor);
      this._interceptionUrlMatchers = [];
    } else {
      this._interceptionUrlMatchers = params.patterns.map(pattern =>
  ...
  async setWebSocketInterceptionPatterns(params, progress) {
    ...
    if (params.patterns.length && !this._routeWebSocketInitScript)
      this._routeWebSocketInitScript =
        await import_webSocketRouteDispatcher.WebSocketRouteDispatcher.install(
          progress,
          this.connection,
          this._context
        );
  ...
  async disableRecorder(params, progress) {
    ...
    if (recorder) recorder.setMode('none');
  }
  ...
  async newCDPSession(params, progress) {
    ...
    return {
      session: new import_cdpSessionDispatcher.CDPSessionDispatcher(
        this,
        await progress.race(
          crBrowserContext.newCDPSession((params.page ? params.page : params.frame)._object)
        )
      ),
    };
  }
  ...
  _onDispose() {
    if (this._context.isClosingOrClosed()) return;
    ...
    if (this._routeWebSocketInitScript)
      import_webSocketRouteDispatcher.WebSocketRouteDispatcher.uninstall(
        this.connection,
        this._context,
        this._routeWebSocketInitScript
      ).catch(() => {});
    this._routeWebSocketInitScript = void 0;
    if (this._clockPaused) this._context.clock.resumeNoReply();
    this._clockPaused = false;
  }
}
