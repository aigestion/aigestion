...
class PageAgentDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, options) {
    super(scope, new import_instrumentation.SdkObject(scope._object, 'pageAgent'), 'PageAgent', {
      page: scope,
    });
  ...
  async expect(params, progress) {
    try {
      await (0, import_pageAgent.pageAgentExpect)(
        progress,
        this._context,
        params.expectation,
        params
  ...
  async extract(params, progress) {
    const result = await (0, import_pageAgent.pageAgentExtract)(
      progress,
      this._context,
      params.query,
      params.schema,
  ...
  _eventSupport() {
    ...
    return {
      ...
      onAfterTurn(params) {
        const usage = {
          inputTokens: params.totalUsage.input,
          outputTokens: params.totalUsage.output,
        };
        const intent = params.assistantMessage.content
        ...
        self._usage = {
          turns: self._usage.turns + 1,
          inputTokens: self._usage.inputTokens + usage.inputTokens,
          outputTokens: self._usage.outputTokens + usage.outputTokens,
        };
      },
      onBeforeToolCall(params) {
        self._dispatchEvent('turn', {
          role: 'assistant',
          message: `call tool "${params.toolCall.name}"`,
        });
      },
      onAfterToolCall(params) {
        ...
        self._dispatchEvent('turn', {
          role: 'user',
          message: `tool "${params.toolCall.name}" ${suffix}`,
        });
      },
      onToolCallError(params) {
        self._dispatchEvent('turn', {
          role: 'user',
          message: `tool "${params.toolCall.name}" failed: ${params.error.message}`,
        });
      },
    };
  }
