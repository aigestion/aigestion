...
class LocalUtilsDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, playwright) {
    const localUtils2 = new import_instrumentation.SdkObject(
      playwright,
      'localUtils',
      'localUtils'
    );
    localUtils2.logName = 'browser';
    const deviceDescriptors = Object.entries(import_deviceDescriptors.deviceDescriptors).map(
  ...
  async connect(params, progress) {
    ...
    const transport = await import_transport.WebSocketTransport.connect(progress, wsEndpoint, {
      headers: wsHeaders,
      followRedirects: true,
      debugLogHeader: 'x-playwright-debug-log',
    });
    const socksInterceptor = new import_socksInterceptor.SocksInterceptor(
      transport,
  ...
  async globToRegex(params, progress) {
    const regex = (0, import_urlMatch.resolveGlobToRegexPattern)(
      params.baseURL,
      params.glob,
      params.webSocketUrl
    );
