...
class WebSocketRouteDispatcher extends import_dispatcher.Dispatcher {
  constructor(scope, id, url, frame) {
    super(
      scope,
      new import_instrumentation.SdkObject(scope._object, 'webSocketRoute'),
      'WebSocketRoute',
      { url }
    );
    ...
    this._eventListeners.push(
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        frame._page,
        import_page.Page.Events.InternalFrameNavigatedToNewDocument,
        frame2 => {
          if (frame2 === this._frame) this._executionContextGone();
        }
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        frame._page,
        import_page.Page.Events.FrameDetached,
        frame2 => {
          if (frame2 === this._frame) this._executionContextGone();
        }
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        frame._page,
        import_page.Page.Events.Close,
        () => this._executionContextGone()
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        frame._page,
        import_page.Page.Events.Crash,
        () => this._executionContextGone()
      )
    );
  ...
  static async install(progress, connection, target) {
    ...
    if (!data) {
      ...
      data.binding = await context.exposeBinding(
        progress,
        kBindingName,
        false,
        (source, payload) => {
          if (payload.type === 'onCreate') {
            ...
            const pageDispatcher = contextDispatcher
              ? import_pageDispatcher.PageDispatcher.fromNullable(contextDispatcher, source.page)
              : void 0;
            let scope;
            if (
              pageDispatcher &&
              ...
              contextDispatcher &&
              matchesPattern(contextDispatcher, context._options.baseURL, payload.url)
            )
  ...
  static async uninstall(connection, target, initScript) {
    ...
    if (!data || data.connection !== connection) return;
    if (--data.counter <= 0) await context.removeExposedBindings([data.binding]);
  ...
  async sendToPage(params, progress) {
    await this._evaluateAPIRequest(progress, {
      id: this._id,
      type: 'sendToPage',
      data: { data: params.message, isBase64: params.isBase64 },
    });
  }
...
function matchesPattern(dispatcher, baseURL, url) {
  for (const pattern of dispatcher._webSocketInterceptionPatterns || []) {
    const urlMatch = pattern.regexSource
      ...
      : pattern.glob;
    if ((0, import_urlMatch.urlMatches)(baseURL, url, urlMatch, true)) return true;
  }
  return false;
}
