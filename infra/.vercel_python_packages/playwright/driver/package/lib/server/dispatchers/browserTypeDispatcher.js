...
class BrowserTypeDispatcher extends import_dispatcher.Dispatcher {
  ...
  async launch(params, progress) {
    if (this._denyLaunch) throw new Error(`Launching more browsers is not allowed.`);
  ...
  async launchPersistentContext(params, progress) {
    ...
    const browserDispatcher = new import_browserDispatcher.BrowserDispatcher(
      this,
  ...
  async connectOverCDP(params, progress) {
    ...
    return {
      ...
      defaultContext: browser._defaultContext
        ? import_browserContextDispatcher.BrowserContextDispatcher.from(
            browserDispatcher,
