...
async function filesExceedUploadLimit(files) {
  const sizes = await Promise.all(
    files.map(async file => (await import_fs.default.promises.stat(file)).size)
  );
...
async function prepareFilesForUpload(frame, params) {
  ...
  if (localPaths) {
    for (const p of localPaths)
      (0, import_assert.assert)(
        import_path.default.isAbsolute(p) && import_path.default.resolve(p) === p,
  ...
  const filePayloads = fileBuffers?.map(payload => ({
    ...
    mimeType:
      payload.mimeType ||
      import_utilsBundle.mime.getType(payload.name) ||
