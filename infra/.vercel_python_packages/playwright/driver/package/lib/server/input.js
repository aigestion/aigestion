...
class Keyboard {
  ...
  _keyDescriptionForString(str) {
    ...
    if (!description) throw new import_dom.NonRecoverableDOMError(`Unknown key: "${keyString}"`);
  ...
  async press(progress, key, options = {}) {
    ...
    for (let i = 0; i < tokens.length - 1; ++i) await this.down(progress, tokens[i]);
    await this.down(progress, key);
    if (options.delay) await progress.wait(options.delay);
    await this.up(progress, key);
    for (let i = tokens.length - 2; i >= 0; --i) await this.up(progress, tokens[i]);
  }
  async ensureModifiers(progress, mm) {
    ...
    for (const key of kModifiers) {
      ...
      if (needDown && !isDown) await this.down(progress, key);
      else if (!needDown && isDown) await this.up(progress, key);
    }
    return restore;
  }
  _modifiers() {
  ...
  async click(progress, x, y, options = {}) {
    ...
    if (delay) {
      ...
      for (let cc = 1; cc <= clickCount; ++cc) {
        ...
        if (cc < clickCount) await progress.wait(delay);
      }
    } else {
      const promises = [];
      const movePromise = this.move(progress, x, y, { forClick: true, steps });
      if (steps !== void 0 && steps > 1) await movePromise;
      else promises.push(movePromise);
      for (let cc = 1; cc <= clickCount; ++cc) {
