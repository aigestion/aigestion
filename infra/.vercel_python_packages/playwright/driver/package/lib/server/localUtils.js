...
async function zip(progress, stackSessions, params) {
  ...
  const addFile = (file, name) => {
    try {
      if (import_fs.default.statSync(file).isFile()) zipFile.addFile(file, name);
    } catch (e) {}
  };
  ...
  if (stackSession?.callStacks.length) {
    ...
    const buffer = Buffer.from(
      JSON.stringify(
        (0, import_traceUtils.serializeClientSideCallMetadata)(stackSession.callStacks)
      )
    );
  ...
  if (params.includeSources) {
    ...
    for (const sourceFile of sourceFiles)
      addFile(
        sourceFile,
        'resources/src@' + (await (0, import_crypto.calculateSha1)(sourceFile)) + '.txt'
      );
  }
  if (params.mode === 'write') {
    await progress.race(
      import_fs.default.promises.mkdir(import_path.default.dirname(params.zipFile), {
        recursive: true,
      })
    );
    zipFile.end(void 0, () => {
      zipFile.outputStream
        .pipe(import_fs.default.createWriteStream(params.zipFile))
...
async function deleteStackSession(progress, stackSessions, stacksId) {
  ...
  if (session.tmpDir) await progress.race((0, import_fileUtils.removeFolders)([session.tmpDir]));
}
async function harOpen(progress, harBackends, params) {
  ...
  if (params.file.endsWith('.zip')) {
    ...
    try {
      ...
      if (!harEntryName) return { error: 'Specified archive does not have a .har file' };
    ...
    const harFile = JSON.parse(
      await progress.race(import_fs.default.promises.readFile(params.file, 'utf-8'))
    );
    harBackend = new import_harBackend.HarBackend(
      harFile,
      import_path.default.dirname(params.file),
      null
    );
  }
...
async function harLookup(progress, harBackends, params) {
  ...
  if (!harBackend) return { action: 'error', message: `Internal error: har was not opened` };
  ...
  try {
    for (const entry of await progress.race(zipFile.entries())) {
      ...
      if (entry === 'har.har')
        ...
        await progress.race(
          import_fs.default.promises.writeFile(import_path.default.join(dir, entry), buffer)
        );
    }
...
async function tracingStarted(progress, stackSessions, params) {
  let tmpDir = void 0;
  if (!params.tracesDir)
    tmpDir = await progress.race(
      import_fs.default.promises.mkdtemp(
        import_path.default.join(import_os.default.tmpdir(), 'playwright-tracing-')
      )
    );
  const traceStacksFile = import_path.default.join(
  ...
  for (const session of stackSessions.values()) {
    ...
    if (session.live) {
      session.writer = session.writer.then(() => {
        const buffer = Buffer.from(
          JSON.stringify((0, import_traceUtils.serializeClientSideCallMetadata)(session.callStacks))
        );
