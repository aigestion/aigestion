  ...
  async _loadContent(content) {
    ...
    if (file) {
      if (this._zipFile) buffer = await this._zipFile.read(file);
        ...
        buffer = await import_fs.default.promises.readFile(
          import_path.default.resolve(this._baseDir, file)
        );
    } else {
...
function multipartBoundary(headers) {
  ...
  if (!contentType?.value.includes('multipart/form-data')) return void 0;
