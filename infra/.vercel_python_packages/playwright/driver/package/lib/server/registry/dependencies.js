...
async function writeDockerVersion(dockerImageNameTemplate) {
  await import_fs.default.promises.mkdir(import_path.default.dirname(dockerVersionFilePath), {
    recursive: true,
  });
...
const checkExecutable = filePath => {
  ...
  return import_fs.default.promises
    .access(filePath, import_fs.default.constants.X_OK)
    .then(() => true)
    .catch(() => false);
};
function isSupportedWindowsVersion() {
  if (import_os.default.platform() !== 'win32' || import_os.default.arch() !== 'x64') return false;
  const [major, minor] = import_os.default
    .release()
    .split('.')
...
async function installDependenciesWindows(targets, dryRun) {
  if (targets.has('chromium')) {
    ...
    const args = [
      ...
      'Bypass',
      '-File',
      import_path.default.join(BIN_DIRECTORY, 'install_media_pack.ps1'),
    ];
    if (dryRun) {
...
async function installDependenciesLinux(targets, dryRun) {
  ...
  for (const target of targets) {
    ...
    if (!info) {
      console.warn(
        `Cannot install dependencies for ${platform} with Playwright ${(0, import_userAgent.getPlaywrightVersion)()}!`
      );
      return;
    }
...
async function validateDependenciesWindows(sdkLanguage, windowsExeAndDllDirectories) {
  ...
  const allMissingDeps = await Promise.all(
    lddPaths.map(lddPath => missingFileDependenciesWindows(sdkLanguage, lddPath))
  );
...
async function validateDependenciesLinux(sdkLanguage, linuxLddDirectories, dlOpenLibraries) {
  ...
  if (
    ...
    !dockerInfo.driverVersion.startsWith(
      (0, import_userAgent.getPlaywrightVersion)(
        true
        /* majorMinorOnly */
      ) + '.'
    )
...
async function executablesOrSharedLibraries(directoryPath) {
  if (!import_fs.default.existsSync(directoryPath)) return [];
  const allPaths = (await import_fs.default.promises.readdir(directoryPath)).map(file =>
    import_path.default.resolve(directoryPath, file)
  );
...
async function missingDLOPENLibraries(libraries) {
  ...
  const { stdout, code, error } = await (0, import_spawnAsync.spawnAsync)(
    '/sbin/ldconfig',
    ['-p'],
    {}
  );
