...
function cftUrl(suffix) {
  return ({ browserVersion }) => {
    return {
      ...
      mirrors: ['https://cdn.playwright.dev/chrome-for-testing-public'],
    };
  };
}
...
const registryDirectory = (() => {
  ...
  if (!import_path.default.isAbsolute(result)) {
    result = import_path.default.resolve(
      (0, import_utils.getFromENV)('INIT_CWD') || process.cwd(),
      result
    );
  }
  return result;
...
function readDescriptors(browsersJSON) {
  return browsersJSON['browsers'].map(obj => {
    ...
    const browserDirectoryPrefix = revisionOverride
      ? `${name}_${import_hostPlatform.hostPlatform}_special`
      : `${name}`;
    const descriptor = {
      name,
      revision,
...
const allDownloadableDirectoriesThatEverExisted = [
  'android',
  'chromium',
  'firefox',
  'webkit',
...
class Registry {
  constructor(browsersJSON) {
    ...
    const executablePathOrDie = (name, e, installByDefault, sdkLanguage) => {
      if (!e) throw new Error(`${name} is not supported on ${import_hostPlatform.hostPlatform}`);
      ...
      if (!(0, import_fileUtils.canAccessFile)(e)) {
        ...
        const preferredDockerVersion = currentDockerVersion
          ? (0, import_dependencies.dockerVersion)(currentDockerVersion.dockerImageNameTemplate)
          : null;
...
${(0, import_ascii.wrapInASCIIBox)(prettyMessage, 1)}`);
    ...
    const chromiumHeadlessShellExecutable = findExecutablePath(
      chromiumHeadlessShell.dir,
      'chromium-headless-shell'
    );
    this._executables.push({
      ...
      executablePathOrDie: sdkLanguage =>
        executablePathOrDie(
          'chromium',
          chromiumHeadlessShellExecutable,
          chromiumHeadlessShell.installByDefault,
          sdkLanguage
        ),
      ...
      _install: force =>
        this._downloadExecutable(chromiumHeadlessShell, force, chromiumHeadlessShellExecutable),
      _dependencyGroup: 'chromium',
      _isHermeticInstallation: true,
    });
    const chromiumTipOfTreeHeadlessShell = descriptors.find(
      d => d.name === 'chromium-tip-of-tree-headless-shell'
    );
    const chromiumTipOfTreeHeadlessShellExecutable = findExecutablePath(
      chromiumTipOfTreeHeadlessShell.dir,
      'chromium-tip-of-tree-headless-shell'
    );
    this._executables.push({
      ...
      executablePathOrDie: sdkLanguage =>
        executablePathOrDie(
          ...
          chromiumTipOfTreeHeadlessShell.installByDefault,
          sdkLanguage
        ),
    ...
    const chromiumTipOfTreeExecutable = findExecutablePath(
    ...
    this._executables.push({
      ...
      _install: force =>
        this._downloadExecutable(chromiumTipOfTree, force, chromiumTipOfTreeExecutable),
      _dependencyGroup: 'chromium',
      _isHermeticInstallation: true,
    });
    this._executables.push(
    ...
    this._executables.push(
      this._createBidiFirefoxChannel('moz-firefox', {
    ...
    this._executables.push(
      this._createBidiFirefoxChannel('moz-firefox-beta', {
    ...
    this._executables.push(
      this._createBidiFirefoxChannel('moz-firefox-nightly', {
    ...
    this._executables.push(
      this._createBidiChromiumChannel('bidi-chrome-stable', {
        linux: '/opt/google/chrome/chrome',
        darwin: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',
        win32: `\\Google\\Chrome\\Application\\chrome.exe`,
      })
    ...
    this._executables.push(
      this._createBidiChromiumChannel('bidi-chrome-canary', {
        linux: '/opt/google/chrome-canary/chrome',
        darwin: '/Applications/Google Chrome Canary.app/Contents/MacOS/Google Chrome Canary',
        win32: `\\Google\\Chrome SxS\\Application\\chrome.exe`,
      })
    ...
    this._executables.push({
      ...
      _validateHostRequirements: sdkLanguage =>
        this._validateHostRequirements(sdkLanguage, firefox.dir, ['firefox'], [], ['firefox']),
    ...
    this._executables.push({
      ...
      _validateHostRequirements: sdkLanguage =>
        this._validateHostRequirements(sdkLanguage, firefoxBeta.dir, ['firefox'], [], ['firefox']),
    ...
    this._executables.push({
      ...
      _install: async () => {
        ...
        const { code } = await (0, import_spawnAsync.spawnAsync)(
          'powershell.exe',
        ...
        if (code !== 0) throw new Error(`Failed to install WebKit via WSL`);
      },
    });
    ...
    this._executables.push({
      ...
      executablePathOrDie: sdkLanguage =>
        executablePathOrDie('winldd', winlddExecutable, winldd.installByDefault, sdkLanguage),
  ...
  _createChromiumChannel(name, lookAt, install) {
    const executablePath = (sdkLanguage, shouldThrow) => {
      ...
      for (const prefix of prefixes) {
        ...
        if ((0, import_fileUtils.canAccessFile)(executablePath2)) return executablePath2;
      }
      if (!shouldThrow) return void 0;
      const location = prefixes.length
        ? ` at ${import_path.default.join(prefixes[0], suffix)}`
        : ``;
...
Run "${buildPlaywrightCLICommand(sdkLanguage, 'install ' + name)}"`
  ...
  async _validateHostRequirements(
    ...
    if (import_os.default.platform() === 'linux')
      return await (0, import_dependencies2.validateDependenciesLinux)(
        sdkLanguage,
        linuxLddDirectories.map(d => import_path.default.join(browserDirectory, d)),
        dlOpenLibraries
      );
    if (import_os.default.platform() === 'win32' && import_os.default.arch() === 'x64')
      return await (0, import_dependencies2.validateDependenciesWindows)(
        sdkLanguage,
        windowsExeAndDllDirectories.map(d => import_path.default.join(browserDirectory, d))
      );
  }
  ...
  async install(executablesToInstall, options) {
    ...
    try {
      ...
      await import_fs.default.promises.writeFile(
        import_path.default.join(linksDir, (0, import_utils.calculateSha1)(PACKAGE_PATH)),
        PACKAGE_PATH
      );
      ...
      for (const executable of executables) {
        ...
        if (
          !(0, import_utils.getAsBooleanFromENV)('CI') &&
  ...
  async uninstall(all) {
    ...
    if (all) {
      ...
      await import_fs.default.promises
        .unlink(import_path.default.join(linksDir, (0, import_utils.calculateSha1)(PACKAGE_PATH)))
        .catch(() => {});
    }
    await this._validateInstallationCache(linksDir);
    return {
      numberOfBrowsersLeft: (
        await import_fs.default.promises.readdir(registryDirectory).catch(() => [])
      ).filter(browserDirectory => isBrowserDirectory(browserDirectory)).length,
    };
  }
  ...
  _downloadURLs(descriptor) {
    ...
    const customHostOverride =
      (downloadHostEnv && (0, import_utils.getFromENV)(downloadHostEnv)) ||
      (0, import_utils.getFromENV)('PLAYWRIGHT_DOWNLOAD_HOST');
    if (customHostOverride) mirrors = [customHostOverride];
  ...
  async _downloadExecutable(descriptor, force, executablePath) {
    ...
    if (!downloadURLs.length)
      throw new Error(
        `ERROR: Playwright does not support ${descriptor.name} on ${import_hostPlatform.hostPlatform}`
      );
    if (!import_hostPlatform.isOfficiallySupportedPlatform)
      (0, import_browserFetcher.logPolitely)(
        `BEWARE: your OS is not officially supported by Playwright; downloading fallback build for ${import_hostPlatform.hostPlatform}.`
      );
    if (descriptor.hasRevisionOverride) {
      ...
      else (0, import_browserFetcher.logPolitely)(message);
    }
    ...
    const downloadSocketTimeoutEnv = (0, import_utils.getFromENV)(
      'PLAYWRIGHT_DOWNLOAD_CONNECTION_TIMEOUT'
    );
    const downloadSocketTimeout =
      +(downloadSocketTimeoutEnv || '0') || import_network.NET_DEFAULT_TIMEOUT;
    await (0, import_browserFetcher.downloadBrowserWithProgressBar)(
      title,
      descriptor.dir,
      executablePath,
      downloadURLs,
      downloadFileName,
      downloadSocketTimeout,
      force
    ).catch(e => {
...
${e.stack}`);
    ...
    if (process.platform !== 'linux') {
      ...
      const release = searchConfig
        ? product.releases.find(
            release2 =>
              release2.platform === searchConfig.platform &&
              release2.architecture === searchConfig.arch &&
              release2.artifacts.length > 0
          )
        : null;
      const artifact = release
        ? release.artifacts.find(artifact2 => artifact2.artifactname === searchConfig.artifact)
        : null;
      if (artifact)
        scriptArgs.push(
          artifact.location
  ...
  async _installChromiumChannel(channel, scripts, scriptArgs = []) {
    ...
    if (isPowerShell) {
      ...
      const { code } = await (0, import_spawnAsync.spawnAsync)('powershell.exe', args, {
        cwd,
        stdio: 'inherit',
      });
      ...
      import_dependencies.transformCommandsForRoot)([
        `bash "${import_path.default.join(BIN_PATH, scriptName)}" ${scriptArgs.join('')}`,
      ]);
      if (elevatedPermissions) console.log('Switching to root user to install dependencies...');
      const { code } = await (0, import_spawnAsync.spawnAsync)(command, args, {
        cwd,
        stdio: 'inherit',
      });
      if (code !== 0) throw new Error(`Failed to install ${channel}`);
  ...
  async _deleteStaleBrowsers(browserList) {
    ...
    for (const browser of browserList) {
      ...
      const shouldHaveMarkerFile =
        (browserName === 'chromium' && (browserVersion >= 786218 || browserVersion < 3e5)) ||
        (browserName === 'firefox' && browserVersion >= 1128) ||
        (browserName === 'webkit' && browserVersion >= 1307) || // All new applications have a marker file right away.
        (browserName !== 'firefox' && browserName !== 'chromium' && browserName !== 'webkit');
      if (
        !shouldHaveMarkerFile ||
        (await (0, import_fileUtils.existsAsync)(browserDirectoryToMarkerFilePath(browserPath)))
      )
        usedBrowserPaths.add(browserPath);
    }
    let downloadedBrowsers = (await import_fs.default.promises.readdir(registryDirectory)).map(
      file => import_path.default.join(registryDirectory, file)
    );
  ...
  _defaultBrowsersToInstall(options) {
    ...
    if (options.shell === 'no')
      executables = executables.filter(
        e =>
          e.name !== 'chromium-headless-shell' && e.name !== 'chromium-tip-of-tree-headless-shell'
      );
    if (options.shell === 'only')
      executables = executables.filter(
        e => e.name !== 'chromium' && e.name !== 'chromium-tip-of-tree'
      );
    return executables;
  }
  suggestedBrowsersToInstall() {
    const names = this.executables()
      .filter(e => e.installType !== 'none')
      .map(e => e.name);
  ...
  resolveBrowsers(aliases, options) {
    if (aliases.length === 0) return this._defaultBrowsersToInstall(options);
    ...
    const handleArgument = arg => {
      ...
      if (executable?.browserName) executables.push(this.findExecutable('ffmpeg'));
    };
    for (const alias of aliases) {
      if (alias === 'chromium' || chromiumAliases.includes(alias)) {
        if (options.shell !== 'only') handleArgument('chromium');
        if (options.shell !== 'no') handleArgument('chromium-headless-shell');
      } else if (alias === 'chromium-tip-of-tree') {
        if (options.shell !== 'only') handleArgument('chromium-tip-of-tree');
        if (options.shell !== 'no') handleArgument('chromium-tip-of-tree-headless-shell');
      } else {
        handleArgument(alias);
      }
    }
    if (process.platform === 'win32') executables.push(this.findExecutable('winldd'));
