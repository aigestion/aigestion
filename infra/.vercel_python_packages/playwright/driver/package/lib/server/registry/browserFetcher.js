...
async function downloadBrowserWithProgressBar(
  ...
  if (
    await (0, import_fileUtils.existsAsync)(
      (0, import__.browserDirectoryToMarkerFilePath)(browserDirectory)
    )
  ) {
    import_debugLogger.debugLogger.log('install', `${title} is already downloaded.`);
    if (force) import_debugLogger.debugLogger.log('install', `force-downloading ${title}.`);
    else return;
  }
  const uniqueTempDir = await import_fs.default.promises.mkdtemp(
    import_path.default.join(import_os.default.tmpdir(), 'playwright-download-')
  );
...
function downloadBrowserWithProgressBarOutOfProcess(
  ...
  cp.on('exit', code => {
    ...
    if (
      ...
      promise.resolve({
        error: new Error(
          `Download failure, ${(0, import__.browserDirectoryToMarkerFilePath)(browserDirectory)} does not exist`
        ),
      });
    else promise.resolve({ error: null });
  });
  cp.on('error', error => {
  ...
  return (downloadedBytes, totalBytes) => {
    if (!progressBar) {
      progressBar = new import_utilsBundle.progress(
        `${toMegabytes(totalBytes)} [:bar] :percent :etas`,
        {
          complete: '=',
          incomplete: ' ',
          width: 20,
