...
class Recorder extends import_events.default {
  constructor(context, params) {
    ...
    this._signalProcessor = new import_recorderSignalProcessor.RecorderSignalProcessor({
      addAction: actionInContext => {
        if (this._enabled) this.emit(RecorderEvent.ActionAdded, actionInContext);
      },
      addSignal: signal => {
        if (this._enabled) this.emit(RecorderEvent.SignalAdded, signal);
      },
    });
    ...
    this._listeners.push(
      import_utils2.eventsHelper.addEventListener(process, 'exit', () => {
        this.emit(RecorderEvent.ContextClosed);
      })
    );
  ...
  async _install() {
    ...
    await controller.run(async progress => {
      ...
      this._context.on(import_browserContext.BrowserContext.Events.Page, page =>
        this._onPage(page)
      );
  ...
  setHighlightedSelector(selector) {
    this._highlightedElement = {
      selector: (0, import_locatorParser.locatorOrSelectorAsSelector)(
  ...
  async _scopeHighlightedSelectorToFrame(frame) {
    ...
    try {
      ...
      const resolved = await mainFrame.selectors.resolveFrameForSelector(
        this._highlightedElement.selector
      );
      if (!resolved) return '';
  ...
  _onDownload(page) {
    ...
    this._signalProcessor.signal(pageAlias, page.mainFrame(), {
      name: 'download',
