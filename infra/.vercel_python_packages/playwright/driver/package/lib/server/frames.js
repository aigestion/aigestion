...
class FrameManager {
  ...
  frameRequestedNavigation(frameId, documentId) {
    ...
    const request = documentId
      ? Array.from(frame._inflightRequests).find(request2 => request2._documentId === documentId)
      : void 0;
  ...
  requestStarted(request, route) {
    ...
    if (request._documentId) frame.setPendingDocument({ documentId: request._documentId, request });
  ...
  _removeFramesRecursively(frame) {
    ...
    if (!this._page.isClosed()) this._page.emit(import_page.Page.Events.FrameDetached, frame);
  }
...
class Frame extends import_instrumentation.SdkObject {
  ...
  async goto(progress, url, options = {}) {
    ...
    return this.raceNavigationAction(progress, async () =>
      this.gotoImpl(progress, constructedNavigationURL, options)
    );
  }
  async gotoImpl(progress, url, options) {
    ...
    const navigateResult = await progress
      .race(this._page.delegate.navigateFrame(this, url, referer))
    ...
    if (navigateResult.newDocumentId) {
      const predicate = event2 => {
        return (
          event2.newDocument &&
          (event2.newDocument.documentId === navigateResult.newDocumentId || !event2.error)
        );
      };
  ...
  async waitForSelector(progress, selector, performActionPreChecksAndLog, options, scope) {
    ...
    const promise = this.retryWithProgressAndTimeouts(
      ...
      async continuePolling => {
        if (performActionPreChecksAndLog) await this._page.performActionPreChecks(progress);
        ...
        const { log, visible, attached } = await progress.race(
          result.evaluate(r => ({ log: r.log, visible: r.visible, attached: r.attached }))
        );
        ...
        const element =
          state === 'attached' || state === 'visible'
            ? await progress.race(result.evaluateHandle(r => r.element))
            : null;
        result.dispose();
  ...
  async setContent(progress, html, options) {
    ...
    await this.raceNavigationAction(progress, async () => {
      ...
      const lifecyclePromise = progress
        .race(tagPromise)
        .then(() => this.waitForLoadState(progress, waitUntil));
  ...
  async _retryWithProgressIfNotConnected(progress, selector, options, action) {
    ...
    const performActionPreChecks =
      (options.performActionPreChecks ?? !options.force) && !noAutoWaiting;
    return this.retryWithProgressAndTimeouts(
      ...
      async continuePolling => {
        ...
        const resolved = await progress.race(
          this.selectors.resolveInjectedForSelector(selector, { strict: options.strict })
        );
        if (!resolved) {
          if (noAutoWaiting) throw new dom.NonRecoverableDOMError('Element(s) not found');
          return continuePolling;
        }
        const result = await progress.race(
          resolved.injected.evaluateHandle(
  ...
  async rafrafTimeoutScreenshotElementWithProgress(progress, selector, timeout, options) {
    return await this._retryWithProgressIfNotConnected(
      progress,
      selector,
      { strict: true, performActionPreChecks: true },
      async handle => {
  ...
  async click(progress, selector, options) {
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
        handle._click(progress, { ...options, waitAfter: !options.noWaitAfter })
      )
    );
  }
  async dblclick(progress, selector, options) {
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
        handle._dblclick(progress, options)
      )
    );
  }
  async dragAndDrop(progress, source, target, options) {
    dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, source, options, async handle => {
    ...
    dom.assertDone(
      await this._retryWithProgressIfNotConnected(
        progress,
        target,
        { ...options, performActionPreChecks: false },
        async handle => {
          return handle._retryPointerAction(
            progress,
            'move and up',
  ...
  async fill(progress, selector, value, options) {
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
        handle._fill(progress, value, options)
      )
    );
  }
  ...
  async resolveSelector(progress, selector, options = {}) {
    ...
    if (!generated) throw new Error(`Unable to generate locator for ${selector}`);
      ...
      return await progress.race(
        resolved.injected.evaluate(
          (injected, { info, root }) => {
            ...
            const state = element
              ? injected.elementState(element, 'visible')
  ...
  async hover(progress, selector, options) {
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
        handle._hover(progress, options)
      )
    );
  }
  async selectOption(progress, selector, elements, values, options) {
    return await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
      handle._selectOption(progress, elements, values, options)
    );
  }
  async setInputFiles(progress, selector, params) {
    ...
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, params, handle =>
        handle._setInputFiles(progress, inputFileItems)
      )
    );
  }
  async type(progress, selector, text, options) {
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
        handle._type(progress, text, options)
      )
    );
  }
  async press(progress, selector, key, options) {
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
        handle._press(progress, key, options)
      )
    );
  }
  async check(progress, selector, options) {
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
        handle._setChecked(progress, true, options)
      )
    );
  }
  async uncheck(progress, selector, options) {
    return dom.assertDone(
      await this._retryWithProgressIfNotConnected(progress, selector, options, handle =>
        handle._setChecked(progress, false, options)
      )
    );
  }
  ...
  async expect(progress, selector, options) {
    progress.log(
      `${(0, import_utils.renderTitleForCall)(progress.metadata)}${options.timeoutForLogs ? ` with timeout ${options.timeoutForLogs}ms` : ''}`
    );
    ...
    try {
      ...
      if (!options.noAutoWaiting) await this._page.performActionPreChecks(progress);
      try {
        ...
        if (options.noAutoWaiting || resultOneShot.matches !== options.isNot) return resultOneShot;
      } catch (e) {
        if (options.noAutoWaiting || this.isNonRetriableError(e)) throw e;
      }
  ...
  async _expectInternal(progress, selector, options, lastIntermediateResult, noAbort) {
    ...
    const selectorInFrame = selector
      ? await race(this.selectors.resolveFrameForSelector(selector, { strict: true }))
      : void 0;
    ...
    const { log, matches, received, missingReceived } = await race(
      injected.evaluate(
        async (injected2, { info: info2, options: options2, callId }) => {
          ...
          const isArray =
            options2.expression === 'to.have.count' || options2.expression.endsWith('.array');
          let log2 = '';
          if (isArray)
          ...
          if (info2) injected2.checkDeprecatedSelectorUsage(info2.parsed, elements);
  ...
  async waitForFunctionExpression(progress, expression, isFunction, arg, options, world = 'main') {
    ...
    return this.retryWithProgressAndTimeouts(progress, [100], async () => {
      const context =
        world === 'main'
          ? await progress.race(this._mainContext())
          : await progress.race(this._utilityContext());
      ...
      async continuePolling => {
        ...
        const { log, success, value } = await progress.race(
          resolved.injected.evaluate(
            (
              injected,
              { info, callbackText: callbackText2, taskData: taskData2, callId, root }
            ) => {
