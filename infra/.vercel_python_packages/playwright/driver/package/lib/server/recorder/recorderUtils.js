...
function isSameAction(a, b) {
  return (
    ...
    a.frame.pageAlias === b.frame.pageAlias &&
    a.frame.framePath.join('|') === b.frame.framePath.join('|')
  );
}
...
function shouldMergeAction(action, lastAction) {
  ...
  switch (action.action.name) {
    ...
    case 'click':
      return (
        isSameAction(action, lastAction) &&
        isSameSelector(action, lastAction) &&
        isShortlyAfter(action, lastAction) &&
        action.action.clickCount > lastAction.action.clickCount
      );
  }
  return false;
}
...
async function generateFrameSelectorInParent(parent, frame) {
  const result = await (0, import_timeoutRunner.raceAgainstDeadline)(
    async () => {
      try {
  ...
  if (frame.name()) return `iframe[name=${(0, import_utils.quoteCSSAttributeValue)(frame.name())}]`;
