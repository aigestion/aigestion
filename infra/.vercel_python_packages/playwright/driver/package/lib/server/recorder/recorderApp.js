...
class RecorderApp {
  constructor(recorder, params, page, wsEndpointForTest) {
    ...
    this._throttledOutputFile = params.outputFile
      ? new import_throttledFile.ThrottledFile(params.outputFile)
      : null;
  ...
  async _init(inspectedContext) {
              ...
              headers: [
                {
                  name: 'Content-Type',
                  value:
                    import_utilsBundle.mime.getType(import_path.default.extname(file)) ||
    ...
    if (url) this._frontend.pageNavigated({ url });
  ...
  async _createDispatcher(progress) {
    const dispatcher = {
      ...
      highlightRequested: async params => {
        if (params.selector) this._recorder.setHighlightedSelector(params.selector);
        if (params.ariaTemplate) this._recorder.setHighlightedAriaTemplate(params.ariaTemplate);
      },
    };
  ...
  static async _show(recorder, inspectedContext, params) {
    ...
    const { context: appContext, page } = await (0, import_launchApp2.launchApp)(
      recorderPlaywright.chromium,
      {
        ...
        persistentContextOptions: {
          ...
          headless:
            !!process.env.PWTEST_CLI_HEADLESS || ((0, import_debug.isUnderTest)() && !headed),
  ...
  _updateActions(reveal) {
    ...
    let revealSourceId;
    for (const languageGenerator of (0, import_languages.languageSet)()) {
      const { header, footer, actionTexts, text } = (0, import_language.generateCode)(
        actions,
        languageGenerator,
        this._languageGeneratorOptions
      );
...
class ProgrammaticRecorderApp {
  static async run(inspectedContext, recorder, browserName, params) {
    ...
    recorder.on(import_recorder.RecorderEvent.ActionAdded, action => {
      ...
      if (!page) return;
      const { actionTexts } = (0, import_language.generateCode)(
        [action],
        languageGenerator,
        languageGeneratorOptions
      );
      if (!lastAction || !(0, import_recorderUtils.shouldMergeAction)(action, lastAction))
        inspectedContext.emit(import_browserContext.BrowserContext.Events.RecorderEvent, {
          event: 'actionAdded',
          data: action,
          page,
