...
class VideoRecorder {
    ...
    const { launchedProcess, gracefullyClose } = await (0, import_processLauncher.launchProcess)({
      ...
      onExit: (exitCode, signal) => {
        import_utils.debugLogger.log(
          'browser',
          `ffmpeg onkill exitCode=${exitCode} signal=${signal}`
        );
      },
    });
  ...
  _writeFrame(frame, timestamp) {
    ...
    if (!this._firstFrameTimestamp) this._firstFrameTimestamp = timestamp;
    ...
    if (this._lastFrame) {
      ...
      for (let i = 0; i < repeatCount; ++i) this._frameQueue.push(this._lastFrame.buffer);
  ...
  async stop() {
    ...
    if (this._isStopped || !this._lastFrame) return;
    const addTime = Math.max(
      ((0, import_utils.monotonicTime)() - this._lastWriteNodeTime) / 1e3,
      1
    );
