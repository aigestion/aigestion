...
class BidiNetworkManager {
  ...
  _onBeforeRequestSent(param) {
    ...
    const frame = redirectedFrom
      ...
      : param.context
        ? this._page.frameManager.frame(param.context)
        : null;
    if (!frame) return;
    if (redirectedFrom) this._deleteRequest(redirectedFrom._id);
    let route;
    if (param.intercepts) {
      if (redirectedFrom) {
        let params = {};
        if (redirectedFrom._originalRequestRoute?._alreadyContinuedHeaders)
          params = toBidiRequestHeaders(
  ...
  _onResponseStarted(params) {
    ...
    const getResponseBody = async () => {
      const { bytes } = await this._session.send('network.getData', {
  ...
  _onFetchError(params) {
    ...
    this._page.frameManager.requestFailed(
      request.request,
      params.errorText === 'NS_BINDING_ABORTED'
    );
  }
  _onAuthRequired(params) {
    const isBasic = params.response.authChallenges?.some(challenge =>
      challenge.scheme.startsWith('Basic')
    );
    ...
    if (
      ...
      credentials &&
      (!credentials.origin ||
        new URL(params.request.url).origin.toLowerCase() === credentials.origin.toLowerCase())
    ) {
...
class BidiRequest {
  constructor(frame, redirectedFrom, payload, route) {
    ...
    this.request = new network.Request(
      ...
      payload.request.url,
      resourceTypeFromBidi(
        payload.request.destination,
        payload.request.initiatorType,
        payload.initiator?.type
      ),
  ...
  async continue(overrides) {
    ...
    await this._session.sendMayFail('network.continueRequest', {
      ...
      body: overrides.postData
        ? { type: 'base64', value: Buffer.from(overrides.postData).toString('base64') }
        : void 0,
    });
  }
