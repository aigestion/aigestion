...
class BidiPage {
  constructor(browserContext, bidiSession, opener) {
    ...
    this._networkManager = new import_bidiNetworkManager.BidiNetworkManager(
      this._session,
      this._page
    );
    ...
    this._sessionListeners = [
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        this._onDownloadEnded.bind(this)
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        bidiSession,
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        this._onFileDialogOpened.bind(this)
      ),
    ];
  ...
  _onRealmCreated(realmInfo) {
    if (realmInfo.type === 'dedicated-worker') {
      const delegate2 = new import_bidiExecutionContext.BidiExecutionContext(
        this._session,
        realmInfo
      );
  ...
  _onNavigationCommitted(params) {
    ...
    this._browserContext
      .doGrantGlobalPermissionsForURL(params.url)
      .catch(error => import_debugLogger.debugLogger.log('error', error));
  ...
  _onDownloadWillBegin(event) {
    ...
    if (!originPage && this._opener) originPage = this._opener._page.initializedOrUndefined();
    if (!originPage) return;
    this._browserContext._browser._downloadCreated(
      originPage,
      event.navigation,
      event.url,
  ...
  _onDownloadEnded(event) {
    if (!event.navigation) return;
    this._browserContext._browser._downloadFinished(
      event.navigation,
      event.status === 'canceled' ? 'canceled' : void 0
    );
  }
...
${params.stackTrace?.callFrames
  ...
  .join('\n')}`;
    ...
    const entry = params;
    const context =
      this._realmToContext.get(params.source.realm) ??
      this._realmToWorkerContext.get(params.source.realm);
    if (!context) return;
    ...
    this._page.addConsoleMessage(
      null,
      entry.method,
      entry.args.map(arg => (0, import_bidiExecutionContext.createHandle)(context, arg)),
      location
    );
  }
  async _onFileDialogOpened(params) {
    ...
    try {
      const handle = await toBidiExecutionContext(executionContext).remoteObjectForNodeId(
        executionContext,
        { sharedId: params.element.sharedId }
      );
  ...
  async updateEmulatedViewportSize() {
    ...
    await Promise.all([
      ...
      this._session.send('emulation.setScreenOrientationOverride', {
        contexts: [this._session.sessionId],
        screenOrientation: (0, import_bidiBrowser.getScreenOrientation)(
          !!options.isMobile,
          screenSize
        ),
      }),
  ...
  async removeInitScripts(initScripts) {
    ...
    await Promise.all(
      ids.map(script => this._session.send('script.removePreloadScript', { script }))
    );
  }
  ...
  async takeScreenshot(progress, format, documentRect, viewportRect, quality, fitsViewport, scale) {
    ...
    const { data } = await progress.race(
      this._session.send('browsingContext.captureScreenshot', {
        context: this._session.sessionId,
        format: {
  ...
  async getFrameElement(frame) {
    ...
    if (!node?.sharedId) throw new Error('Frame has been detached.');
    const parentFrameExecutionContext = await parent._mainContext();
    return await toBidiExecutionContext(parentFrameExecutionContext).remoteObjectForNodeId(
      parentFrameExecutionContext,
