...
Workaround: Set the HOME=/root environment variable${process.env.GITHUB_ACTION ? ' in your GitHub Actions workflow file' : ''} when running Playwright.`,
  ...
  amendEnvironment(env) {
    if (!import_path.default.isAbsolute(import_os.default.homedir()))
      throw new Error(
        `Cannot launch Firefox with relative home directory. Did you set ${import_os.default.platform() === 'win32' ? 'USERPROFILE' : 'HOME'} to a relative path?`
      );
    env = {
      ...env,
      MOZ_CRASHREPORTER: '1',
  ...
  async _attemptToGracefullyCloseBrowser(transport) {
    if (!transport.onmessage) {
      transport.send({
        method: 'session.new',
        params: { capabilities: {} },
        id: import_bidiConnection.kShutdownSessionNewMessageId,
      });
      await new Promise(resolve => {
        transport.onmessage = message => {
          if (message.id === import_bidiConnection.kShutdownSessionNewMessageId) resolve(true);
        };
      });
    }
    transport.send({
  ...
  async waitForReadyState(options, browserLogsCollector) {
    ...
    browserLogsCollector.onMessage(message => {
      ...
      if (match) result.resolve({ wsEndpoint: match[1] + '/session' });
    });
    return result;
  }
}
