...
class BidiBrowser extends import_browser.Browser {
  constructor(parent, transport, options) {
    ...
    this._connection = new import_bidiConnection.BidiConnection(
      transport,
  ...
  static async connect(parent, transport, options) {
    ...
    browser._bidiSessionInfo = await browser._browserSession.send('session.new', {
      capabilities: {
        alwaysMatch: {
          acceptInsecureCerts:
            options.persistent?.internalIgnoreHTTPSErrors || options.persistent?.ignoreHTTPSErrors,
          proxy: getProxyConfiguration(
            options.originalLaunchOptions.proxyOverride ?? options.proxy
          ),
    ...
    await browser._browserSession.send('session.subscribe', {
      events: ['browsingContext', 'network', 'log', 'script', 'input'],
    });
  ...
  _findPageForFrame(frameId) {
    for (const page of this._bidiPages.values()) {
      if (page._page.frameManager.frame(frameId)) return page;
    }
  }
}
class BidiBrowserContext extends import_browserContext.BrowserContext {
  ...
  async _initialize() {
    ...
    if (this._options.locale) {
      promises.push(
        this._browser._browserSession.send('emulation.setLocaleOverride', {
    ...
    if (this._options.timezoneId) {
      promises.push(
        this._browser._browserSession.send('emulation.setTimezoneOverride', {
    ...
    if (this._options.userAgent) {
      promises.push(
        this._browser._browserSession.send('emulation.setUserAgentOverride', {
          userAgent: this._options.userAgent,
          userContexts: [this._userContextId()],
        })
      );
    }
    if (this._options.extraHTTPHeaders) promises.push(this.doUpdateExtraHTTPHeaders());
  ...
  async doGrantPermissions(origin, permissions) {
    ...
    if (origin === '*') {
      await Promise.all(
        this._bidiPages().flatMap(page =>
          page._page
            .frames()
            .map(frame => this.doGrantPermissions(new URL(frame._url).origin, permissions))
        )
      );
    } else {
      await Promise.all(
        toGrant.map(permission =>
          this._setPermission(origin, permission, bidi.Permissions.PermissionState.Granted)
        )
      );
    }
  }
  ...
  async doClearPermissions() {
    ...
    this._originToPermissions = /* @__PURE__ */ new Map();
    await Promise.all(
      currentPermissions.flatMap(([origin, permissions]) => {
        if (origin !== '*')
          return permissions.map(p =>
            this._setPermission(origin, p, bidi.Permissions.PermissionState.Prompt)
          );
      })
    );
  }
  ...
  async doSetHTTPCredentials(httpCredentials) {
    ...
    for (const page of this.pages()) await page.delegate.updateHttpCredentials();
  }
  ...
  async doRemoveInitScripts(initScripts) {
    ...
    await Promise.all(
      ids.map(script =>
        this._browser._browserSession.send('script.removePreloadScript', { script })
      )
    );
  }
  ...
  async doUpdateDefaultViewport() {
    if (!this._options.viewport && !this._options.screen) return;
  ...
  async doExposePlaywrightBinding() {
    ...
    const promises = [];
    promises.push(
      this._browser._browserSession.send('script.addPreloadScript', {
        functionDeclaration,
        arguments: args,
        userContexts: [this._userContextId()],
      })
    ...
    promises.push(
      ...this._bidiPages().map(page => {
        const realms = [...page._realmToContext]
          .filter(([realm, context]) => context.world === 'main')
          .map(([realm, context]) => realm);
        return Promise.all(
          realms.map(realm => {
...
function getProxyConfiguration(proxySettings) {
  if (!proxySettings) return void 0;
  const proxy = {
    proxyType: 'manual',
  };
  ...
  if (bypass) proxy.noProxy = bypass.split(',');
  return proxy;
}
function getScreenOrientation(isMobile, viewportSize) {
  ...
  if (isMobile) {
    ...
    if (viewportSize.width <= viewportSize.height) screenOrientation.type = 'portrait-primary';
  }
