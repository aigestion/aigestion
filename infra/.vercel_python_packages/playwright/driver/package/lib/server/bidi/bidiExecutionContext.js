...
class BidiExecutionContext {
  ...
  async evaluateWithArguments(functionDeclaration, returnByValue, utilityScript, values, handles) {
    ...
    if (response.type === 'success') {
      if (returnByValue)
        return (0, import_utilityScriptSerializers.parseEvaluationResultValue)(
          (0, import_bidiDeserializer.deserializeBidiValue)(response.result)
        );
  ...
  async getProperties(handle) {
    const names = await handle.evaluate(object => {
      ...
      for (const name in descriptors) {
        if (descriptors[name]?.enumerable) names2.push(name);
      }
      return names2;
    });
    const values = await Promise.all(
      names.map(async name => {
        const value = await this._rawCallFunction(
          '(object, name) => object[name]',
          [{ handle: handle._objectId }, { type: 'string', value: name }],
          true,
          false
        );
  ...
  switch (remoteObject.type) {
    ...
    case 'object':
      ...
      for (const [name, value] of remoteObject.value || []) {
        if (typeof name === 'string') tokens.push(`${name}: ${renderPreview(value, true)}`);
      }
      return `{${tokens.join(', ')}}`;
    case 'array':
    case 'htmlcollection':
...
function createHandle(context, remoteObject) {
  ...
  const handle = new js.JSHandle(
    context,
    remoteObject.type,
    preview,
    objectId,
    (0, import_bidiDeserializer.deserializeBidiValue)(remoteObject)
  );
