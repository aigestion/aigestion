...
class DebugController extends import_instrumentation.SdkObject {
  constructor(playwright) {
    super(
      {
        attribution: { isInternalPlaywright: true },
        instrumentation: (0, import_instrumentation.createInstrumentation)(),
      },
      void 0,
      'DebugController'
    );
  ...
  async setRecorderMode(progress, params) {
    ...
    if (!this._playwright.allBrowsers().length)
      await this._playwright.chromium.launch(progress, {
  ...
  async highlight(progress, params) {
    if (params.selector)
      (0, import_locatorParser.unsafeLocatorOrSelectorAsSelector)(
        this._sdkLanguage,
        params.selector,
        'data-testid'
      );
    const ariaTemplate = params.ariaTemplate
      ? (0, import_ariaSnapshot.parseAriaSnapshotUnsafe)(
          import_utilsBundle.yaml,
          params.ariaTemplate
        )
      : void 0;
    for (const recorder of await progress.race(this._allRecorders())) {
      if (ariaTemplate) recorder.setHighlightedAriaTemplate(ariaTemplate);
      else if (params.selector) recorder.setHighlightedSelector(params.selector);
    }
  }
  ...
  async resume(progress) {
    for (const recorder of await progress.race(this._allRecorders())) recorder.resume();
  }
  kill() {
  ...
  async _allRecorders() {
    ...
    for (const page of this._playwright.allPages()) contexts.add(page.browserContext);
    const recorders = await Promise.all(
      [...contexts].map(c => import_recorder.Recorder.forContext(c, { omitCallTracking: true }))
    );
    const nonNullRecorders = recorders.filter(Boolean);
    for (const recorder of recorders) wireListeners(recorder, this);
    return nonNullRecorders;
  }
  async _closeBrowsersWithoutPages() {
    for (const browser of this._playwright.allBrowsers()) {
      for (const context of browser.contexts()) {
        if (!context.pages().length) await context.close({ reason: 'Browser collected' });
      }
      if (!browser.contexts()) await browser.close({ reason: 'Browser collected' });
    }
  }
}
const wiredSymbol = Symbol('wired');
function wireListeners(recorder, debugController) {
  if (recorder[wiredSymbol]) return;
  ...
  const actionsChanged = () => {
    ...
    const { header, footer, text, actionTexts } = (0, import_language.generateCode)(
      aa,
      languageGenerator,
      {
    ...
    debugController.emit(DebugController.Events.SourceChanged, {
      text,
      header,
      footer,
      actions: actionTexts,
  ...
  recorder.on(import_recorder.RecorderEvent.ElementPicked, elementInfo => {
    ...
    debugController.emit(DebugController.Events.InspectRequested, {
