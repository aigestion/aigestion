...
class ElementHandle extends js.JSHandle {
  ...
  async evaluateInUtility(pageFunction, arg) {
    try {
      ...
      if (this._frame.isNonRetriableError(e)) throw e;
  ...
  async _waitAndScrollIntoViewIfNeeded(progress, waitForVisible) {
    const result = await this._retryAction(
      ...
      async () => {
        ...
        const waitResult = await progress.race(
          this.evaluateInUtility(
            async ([injected, node, { waitForVisible: waitForVisible2 }]) => {
              return await injected.checkElementStates(
                node,
                waitForVisible2 ? ['visible', 'stable'] : ['stable']
              );
            },
            { waitForVisible }
          )
  ...
  async _clickablePoint() {
    ...
    const [quads, metrics] = await Promise.all([
      ...
      this._page
        .mainFrame()
        ._utilityContext()
        .then(utility => utility.evaluate(() => ({ width: innerWidth, height: innerHeight }))),
    ]);
    ...
    while (true) {
      if (retry) {
        ...
        if (timeout) {
          ...
          const result2 = await progress.race(
            this.evaluateInUtility(
              ([injected, node, timeout2]) => new Promise(f => setTimeout(f, timeout2)),
              timeout
            )
          );
      ...
      if (result === 'error:optionsnotfound') {
        if (noAutoWaiting) throw new NonRecoverableDOMError('Did not find some options');
        progress.log('  did not find some options');
        continue;
      }
      if (result === 'error:optionnotenabled') {
        if (noAutoWaiting) throw new NonRecoverableDOMError('Option being selected is not enabled');
        progress.log('  option being selected is not enabled');
        continue;
      }
      if (typeof result === 'object' && 'hitTargetDescription' in result) {
        if (noAutoWaiting)
          throw new NonRecoverableDOMError(
  ...
  async _performPointerAction(
    ...
    const doScrollIntoView = async () => {
      ...
      return await this._scrollRectIntoViewIfNeeded(
        progress,
        position ? { x: position.x, y: position.y, width: 0, height: 0 } : void 0
      );
    };
    if (this._frame.parentFrame()) {
      await progress.race(doScrollIntoView().catch(() => {}));
    }
    if (options.__testHookBeforeStable) await progress.race(options.__testHookBeforeStable());
    if (!force) {
      ...
      const result = await progress.race(
        this.evaluateInUtility(
          async ([injected, node, { elementStates: elementStates2 }]) => {
    ...
    if (options.__testHookAfterStable) await progress.race(options.__testHookAfterStable());
    ...
    const maybePoint = position
      ? await progress.race(this._offsetPoint(position))
      : await progress.race(this._clickablePoint());
    ...
    if (force) {
      ...
      const handle = await progress.race(
        this.evaluateHandleInUtility(
          ([injected, node, { actionType: actionType2, hitPoint: hitPoint2, trial }]) =>
            injected.setupHitTargetInterceptor(node, actionType2, hitPoint2, trial),
          { actionType, hitPoint, trial: !!options.trial }
        )
      );
    ...
    const actionResult = await this._page.frameManager
      .waitForSignalsCreatedBy(progress, options.waitAfter === true, async () => {
        ...
        await action(point);
        if (restoreModifiers) await this._page.keyboard.ensureModifiers(progress, restoreModifiers);
  ...
  async _markAsTargetElement(progress) {
    if (!progress.metadata.id) return;
    await progress.race(
      this.evaluateInUtility(([injected, node, callId]) => {
        if (node.nodeType === 1)
  ...
  _click(progress, options) {
    return this._retryPointerAction(
      ...
      'click',
      true,
      point => this._page.mouse.click(progress, point.x, point.y, options),
      options
    );
  }
  ...
  _dblclick(progress, options) {
    return this._retryPointerAction(
      ...
      'dblclick',
      true,
      point => this._page.mouse.click(progress, point.x, point.y, { ...options, clickCount: 2 }),
      { ...options, waitAfter: 'disabled' }
    );
  }
  ...
  _tap(progress, options) {
    return this._retryPointerAction(
      ...
      'tap',
      true,
      point => this._page.touchscreen.tap(progress, point.x, point.y),
      { ...options, waitAfter: 'disabled' }
    );
  }
  ...
  async _selectOption(progress, elements, values, options) {
    ...
    const result = await this._retryAction(
      ...
      async () => {
        ...
        const result2 = await progress.race(
          this.evaluateInUtility(
            async ([injected, node, { optionsToSelect: optionsToSelect2, force }]) => {
              if (!force) {
  ...
  async _fill(progress, value, options) {
    ...
    return await this._retryAction(
      ...
      async () => {
        ...
        const result = await progress.race(
          this.evaluateInUtility(
            async ([injected, node, { value: value2, force }]) => {
              if (!force) {
        ...
        if (result === 'needsinput') {
          if (value) await this._page.keyboard.insertText(progress, value);
          else await this._page.keyboard.press(progress, 'Delete');
          return 'done';
        } else {
          return result;
        }
  ...
  async setInputFiles(progress, params) {
    const inputFileItems = await progress.race(
      (0, import_fileUploadUtils.prepareFilesForUpload)(this._frame, params)
    );
    await this._markAsTargetElement(progress);
  ...
  async _setInputFiles(progress, items) {
    ...
    const result = await progress.race(
      this.evaluateHandleInUtility(
        ([injected, node, { multiple: multiple2, directoryUpload }]) => {
      ...
      await progress.race(
        Promise.all(
          localPathsOrDirectory.map(localPath =>
            import_fs.default.promises.access(localPath, import_fs.default.constants.F_OK)
          )
        )
      );
  ...
  async _focus(progress, resetSelectionIfNotFocused) {
    return await progress.race(
      this.evaluateInUtility(
        ([injected, node, resetSelectionIfNotFocused2]) =>
          injected.focusNode(node, resetSelectionIfNotFocused2),
  ...
  async _blur(progress) {
    return await progress.race(
      this.evaluateInUtility(([injected, node]) => injected.blurNode(node), {})
    );
  }
  ...
  async _press(progress, key, options) {
    ...
    await progress.race(this.instrumentation.onBeforeInputAction(this, progress.metadata));
    return this._page.frameManager.waitForSignalsCreatedBy(
      progress,
      !options.noWaitAfter,
      async () => {
        const result = await this._focus(
  ...
  async _setChecked(progress, state, options) {
    const isChecked = async () => {
      const result2 = await progress.race(
        this.evaluateInUtility(([injected, node]) => injected.elementState(node, 'checked'), {})
      );
  ...
  async waitForElementState(progress, state) {
    ...
    const result = await this._retryAction(
      ...
      async () => {
        return await progress.race(
          this.evaluateInUtility(async ([injected, node, state2]) => {
...
function isPointInsideQuad(point, quad) {
  const area1 =
    triangleArea(point, quad[0], quad[1]) +
    triangleArea(point, quad[1], quad[2]) +
    triangleArea(point, quad[2], quad[3]) +
    triangleArea(point, quad[3], quad[0]);
  const area2 = triangleArea(quad[0], quad[1], quad[2]) + triangleArea(quad[1], quad[2], quad[3]);
  if (Math.abs(area1 - area2) > 0.1) return false;
  return (
    point.x < Math.max(quad[0].x, quad[1].x, quad[2].x, quad[3].x) &&
