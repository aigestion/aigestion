...
class Selectors {
  ...
  parseSelector(selector, strict) {
    const parsed =
      typeof selector === 'string' ? (0, import_selectorParser.parseSelector)(selector) : selector;
    ...
    (0, import_selectorParser.visitAllSelectorParts)(parsed, part => {
      ...
      if (!custom && !this._builtinEngines.has(name))
        throw new import_selectorParser.InvalidSelectorError(
          `Unknown engine "${name}" while parsing selector ${(0, import_selectorParser.stringifySelector)(parsed)}`
        );
