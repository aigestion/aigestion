...
class FrameSelectors {
  ...
  _parseSelector(selector, options) {
    const strict =
      typeof options?.strict === 'boolean'
        ? options.strict
        : !!this.frame._page.browserContext._options.strictSelectors;
  ...
  _jumpToAriaRefFrameIfNeeded(selector, info, frame) {
    ...
    const jumptToFrame = this.frame._page.frameManager
      .frames()
      .find(frame2 => frame2.seq === frameSeq);
    if (!jumptToFrame)
      throw new import_selectorParser.InvalidSelectorError(
  ...
  async resolveFrameForSelector(selector, options = {}, scope) {
    ...
    for (const chunk of frameChunks) {
      (0, import_selectorParser.visitAllSelectorParts)(chunk, (part, nested) => {
        if (nested && part.name === 'internal:control' && part.body === 'enter-frame') {
          const locator = (0, import_utils.asLocator)(
            this.frame._page.browserContext._browser.sdkLanguage(),
            selector
          );
          throw new import_selectorParser.InvalidSelectorError(
    ...
    for (let i = 0; i < frameChunks.length - 1; ++i) {
      ...
      const handle = await injectedScript.evaluateHandle(
        (injected, { info: info2, scope: scope2, selectorString }) => {
          ...
          if (element2 && element2.nodeName !== 'IFRAME' && element2.nodeName !== 'FRAME')
            throw injected.createStacklessError(
              `Selector "${selectorString}" resolved to ${injected.previewNode(element2)}, <iframe> was expected`
            );
          return element2;
        },
        {
          ...
          selectorString: (0, import_selectorParser.stringifySelector)(info.parsed),
        }
      );
  ...
  async resolveInjectedForSelector(selector, options, scope) {
    ...
    const context = await resolved.frame._context(
      options?.mainWorld ? 'main' : resolved.info.world
    );
