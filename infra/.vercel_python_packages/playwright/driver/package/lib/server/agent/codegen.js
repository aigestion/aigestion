...
async function generateCode(sdkLanguage, action) {
  switch (action.method) {
    ...
    case 'drag': {
      const sourceLocator = (0, import_locatorGenerators.asLocator)(
        sdkLanguage,
        action.sourceSelector
      );
      const targetLocator = (0, import_locatorGenerators.asLocator)(
        sdkLanguage,
    ...
    case 'pressSequentially': {
      const locator = (0, import_locatorGenerators.asLocator)(sdkLanguage, action.selector);
      const code = [
        `await page.${locator}.pressSequentially(${(0, import_stringUtils.escapeWithQuotes)(action.text)});`,
      ];
      if (action.submit) code.push(`await page.keyboard.press('Enter');`);
      return code.join('\n');
    }
    case 'fill': {
      const locator = (0, import_locatorGenerators.asLocator)(sdkLanguage, action.selector);
      const code = [
        `await page.${locator}.fill(${(0, import_stringUtils.escapeWithQuotes)(action.text)});`,
      ];
      if (action.submit) code.push(`await page.keyboard.press('Enter');`);
      return code.join('\n');
    }
    case 'setChecked': {
      const locator = (0, import_locatorGenerators.asLocator)(sdkLanguage, action.selector);
      if (action.checked) return `await page.${locator}.check();`;
      else return `await page.${locator}.uncheck();`;
    }
    case 'expectVisible': {
      const locator = (0, import_locatorGenerators.asLocator)(sdkLanguage, action.selector);
...
\`);`;
    ...
    case 'expectURL': {
      const arg = action.regex
        ? (0, import_stringUtils.parseRegex)(action.regex).toString()
        : (0, import_stringUtils.escapeWithQuotes)(action.value);
      const notInfix = action.isNot ? 'not.' : '';
