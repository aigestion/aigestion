...
async function runAction(progress, mode, page, action, secrets) {
  ...
  let error;
  const result = await innerRunAction(progress, mode, page, action, secrets).catch(
    e => (error = e)
  );
...
async function innerRunAction(progress, mode, page, action, secrets) {
  ...
  switch (action.method) {
    ...
    case 'click':
      ...
      break;
    case 'drag':
      await frame.dragAndDrop(progress, action.sourceSelector, action.targetSelector, {
        ...commonOptions,
      });
      break;
    case 'hover':
    ...
    case 'pressSequentially': {
      ...
      if (action.submit) await page.keyboard.press(progress, 'Enter');
      break;
    }
    case 'fill': {
      const secret = secrets?.find(s => s.name === action.text)?.value ?? action.text;
    ...
    case 'setChecked':
      if (action.checked) await frame.check(progress, action.selector, { ...commonOptions });
      else await frame.uncheck(progress, action.selector, { ...commonOptions });
      break;
    case 'expectVisible': {
      await runExpect(
        frame,
        ...
        mode,
        action.selector,
        { expression: 'to.be.visible', isNot: !!action.isNot },
        'visible',
        'toBeVisible',
        ''
      );
    ...
    case 'expectValue': {
      if (action.type === 'textbox' || action.type === 'combobox' || action.type === 'slider') {
        ...
        await runExpect(
          ...
          mode,
          action.selector,
          { expression: 'to.have.value', expectedText, isNot: !!action.isNot },
          action.value,
          'toHaveValue',
          'expected'
        );
    ...
    case 'expectAria': {
      const expectedValue = (0, import_ariaSnapshot.parseAriaSnapshotUnsafe)(
      ...
      await runExpect(
        ...
        mode,
        'body',
        { expression: 'to.match.aria', expectedValue, isNot: !!action.isNot },
        '\n' + action.template,
        'toMatchAriaSnapshot',
        'expected'
      );
    ...
    case 'expectURL': {
      if (!action.regex && !action.value) throw new Error('Either url or regex must be provided');
      if (action.regex && action.value) throw new Error('Only one of url or regex can be provided');
      const expected = action.regex
        ? (0, import_stringUtils.parseRegex)(action.regex)
        : (0, import_urlMatch.constructURLBasedOnBaseURL)(
            page.browserContext._options.baseURL,
            action.value
          );
      ...
      await runExpect(
        ...
        mode,
        void 0,
        { expression: 'to.have.url', expectedText, isNot: !!action.isNot },
        expected,
        'toHaveURL',
        'expected'
      );
    ...
    case 'expectTitle': {
      ...
      await runExpect(
        ...
        mode,
        void 0,
        { expression: 'to.have.title', expectedText, isNot: !!action.isNot },
        action.value,
        'toHaveTitle',
        'expected'
      );
...
async function runExpect(
  ...
  if (!result.matches === !options.isNot) {
    ...
    throw new Error(
      (0, import_expectUtils.formatMatcherMessage)(import_expectUtils.simpleMatcherUtils, {
        isNot: options.isNot,
        matcherName,
        expectation,
        locator: selector
          ? (0, import_locatorGenerators.asLocatorDescription)('javascript', selector)
          : void 0,
        timedOut: result.timedOut,
        timeout: mode === 'generate' ? void 0 : progress.timeout,
        printedExpected: options.isNot
          ? `Expected${expectedSuffix}: not ${expectedDisplay}`
          : `Expected${expectedSuffix}: ${expectedDisplay}`,
