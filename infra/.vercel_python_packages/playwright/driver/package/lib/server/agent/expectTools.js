...
const expectVisible = (0, import_tool.defineTool)({
  ...
  handle: async (progress, context, params) => {
    return await context.runActionAndWait(progress, {
      ...
      selector: (0, import_locatorUtils.getByRoleSelector)(params.role, {
        name: params.accessibleName,
      }),
      isNot: params.isNot,
    });
  },
...
const expectVisibleText = (0, import_tool.defineTool)({
  schema: {
    ...
    inputSchema: import_mcpBundle.z.object({
      text: import_mcpBundle.z
        .string()
        .describe(
          'TEXT to expect. Can be found in the snapshot like this: `- role "Accessible Name": {TEXT}` or like this: `- text: {TEXT}`'
        ),
      isNot: import_mcpBundle.z.boolean().optional().describe('Expect the opposite'),
    }),
  },
...
const expectValue = (0, import_tool.defineTool)({
  schema: {
    ...
    inputSchema: import_mcpBundle.z.object({
      ...
      ref: import_mcpBundle.z
        .string()
        .describe('Exact target element reference from the page snapshot'),
      value: import_mcpBundle.z
        .string()
        .describe('Value to expect. For checkbox, use "true" or "false".'),
      isNot: import_mcpBundle.z.boolean().optional().describe('Expect the opposite'),
    }),
  },
  handle: async (progress, context, params) => {
    const [selector] = await context.refSelectors(progress, [
      { ref: params.ref, element: params.element },
    ]);
    return await context.runActionAndWait(progress, {
      method: 'expectValue',
      selector,
...
const expectList = (0, import_tool.defineTool)({
  schema: {
    ...
    description:
      'Expect list is visible on the page, ensures items are present in the element in the exact order',
    inputSchema: import_mcpBundle.z.object({
      listRole: import_mcpBundle.z
        .string()
        .describe('Aria role of the list element as in the snapshot'),
      listAccessibleName: import_mcpBundle.z
        .string()
        .optional()
        .describe('Accessible name of the list element as in the snapshot'),
      itemRole: import_mcpBundle.z
        .string()
        .describe('Aria role of the list items as in the snapshot, should all be the same'),
      items: import_mcpBundle.z.array(
        import_mcpBundle.z
          .string()
          .describe(
            'Text to look for in the list item, can be either from accessible name of self / nested text content'
          )
      ),
      isNot: import_mcpBundle.z.boolean().optional().describe('Expect the opposite'),
    }),
...
const expectURL = (0, import_tool.defineTool)({
  schema: {
    ...
    title: 'Expect URL',
    description:
      'Expect the page URL to match the expected value. Either provide a url string or a regex pattern.',
    inputSchema: import_mcpBundle.z.object({
      url: import_mcpBundle.z
        .string()
        .optional()
        .describe('Expected URL string. Relative URLs are resolved against the baseURL.'),
      regex: import_mcpBundle.z
        .string()
        .optional()
        .describe('Regular expression pattern to match the URL against, e.g. /foo.*/i.'),
      isNot: import_mcpBundle.z.boolean().optional().describe('Expect the opposite'),
    }),
  },
  handle: async (progress, context, params) => {
