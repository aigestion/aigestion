...
const clickSchema = elementSchema.extend({
  doubleClick: import_mcpBundle.z
    .boolean()
    .optional()
    .describe('Whether to perform a double click instead of a single click'),
  button: import_mcpBundle.z
    .enum(['left', 'right', 'middle'])
    .optional()
    .describe('Button to click, defaults to left'),
  modifiers: import_mcpBundle.z
    .array(import_mcpBundle.z.enum(['Alt', 'Control', 'ControlOrMeta', 'Meta', 'Shift']))
    .optional()
...
const selectOptionSchema = elementSchema.extend({
  values: import_mcpBundle.z
    .array(import_mcpBundle.z.string())
    .describe(
...
const fillForm = (0, import_tool.defineTool)({
  schema: {
    ...
    title: 'Fill form',
    description:
      'Fill multiple form fields. Always use this tool when you can fill more than one field at a time.',
    inputSchema: import_mcpBundle.z.object({
      fields: import_mcpBundle.z
        .array(
          import_mcpBundle.z.object({
  ...
  handle: async (progress, context, params) => {
    ...
    for (const field of params.fields) {
      const [selector] = await context.refSelectors(progress, [
        { ref: field.ref, element: field.name },
      ]);
