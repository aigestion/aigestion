...
class Context {
  ...
  async runActionsAndWait(progress, action, options) {
    const error = await this.waitForCompletion(
      progress,
      async () => {
  ...
  consumeTokens(tokens) {
    if (this._budget.tokens === void 0) return;
  ...
  async waitForCompletion(progress, callback, options) {
    ...
    for (const request of requests) {
      ...
      else promises.push(request.response());
    }
    await progress.race([...promises, progress.wait(5e3)]);
    if (!promises.length) await progress.wait(500);
    return result;
  }
  async takeSnapshot(progress) {
    const { full } = await this.page.snapshotForAI(progress, {
      doNotRenderActive: this.agentParams.doNotRenderActive,
    });
    return full;
  }
  ...
  async refSelectors(progress, params) {
    return Promise.all(
      params.map(async param => {
        try {
          const { resolvedSelector } = await this.page
            .mainFrame()
            .resolveSelector(progress, `aria-ref=${param.ref}`);
