...
async function pageAgentPerform(progress, context, userTask, callParams) {
  ...
  if (await cachedPerform(progress, context, cacheKey)) return;
  const task = `
### Instructions
...
async function runLoop(progress, context, toolDefinitions, userTask, resultSchema, params) {
  ...
  const { tools, callTool, reportedResult, refusedToPerformReason } = (0, import_tool.toolsForLoop)(
    progress,
    context,
    toolDefinitions,
    { resultSchema, refuseToPerform: 'allow' }
  );
  const secrets = Object.fromEntries(
    (context.agentParams.secrets || [])?.map(s => [s.name, s.value])
  );
  const apiCacheTextBefore = context.agentParams.apiCacheFile
    ? await import_fs.default.promises
        .readFile(context.agentParams.apiCacheFile, 'utf-8')
        .catch(() => '{}')
    : '{}';
  ...
  if (context.agentParams.apiCacheFile) {
    ...
    const sortedCache = Object.fromEntries(
      Object.entries(apiCacheAfter).sort(([a], [b]) => a.localeCompare(b))
    );
    ...
    if (apiCacheTextAfter !== apiCacheTextBefore) {
      await import_fs.default.promises.mkdir(
        import_path.default.dirname(context.agentParams.apiCacheFile),
        { recursive: true }
      );
      await import_fs.default.promises.writeFile(
        context.agentParams.apiCacheFile,
        apiCacheTextAfter
      );
    }
  }
...
async function updateCache(context, cacheKey) {
  ...
  if (cacheOutFile) {
    ...
    await import_fs.default.promises.writeFile(
      cacheOutFile,
      JSON.stringify(Object.fromEntries(entries), void 0, 2)
    );
  } else if (cacheFile) {
