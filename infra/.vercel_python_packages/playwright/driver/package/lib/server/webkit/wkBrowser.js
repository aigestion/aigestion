...
class WKBrowser extends import_browser.Browser {
  constructor(parent, transport, options) {
    ...
    this._connection = new import_wkConnection.WKConnection(
      transport,
    ...
    this._browserSession.on(
      import_wkConnection.kPageProxyMessageReceived,
    ...
    const createOptions = proxy
      ? {
          ...
          proxyServer:
            ...
            this.attribution.browser?.options.channel !== 'webkit-wsl'
  ...
  _onDownloadCreated(payload) {
    ...
    if (!page._page.frameManager.frame(frameId)) frameId = page._page.mainFrame()._id;
  ...
  _onPageProxyCreated(event) {
    ...
    if (!context) return;
    const pageProxySession = new import_wkConnection.WKSession(
      this._connection,
      pageProxyId,
      message => {
...
class WKBrowserContext extends import_browserContext.BrowserContext {
  ...
  async _initialize() {
    ...
    promises.push(
      this._browser._browserSession.send('Playwright.setDownloadBehavior', {
        ...
        downloadPath:
          this._browser.options.channel === 'webkit-wsl'
            ? await (0, import_webkit.translatePathToWSL)(this._browser.options.downloadsPath)
            : this._browser.options.downloadsPath,
        browserContextId,
      })
    );
    ...
    if (this._options.offline) promises.push(this.doUpdateOffline());
  ...
  async doGrantPermissions(origin, permissions) {
    await Promise.all(
      this.pages().map(page => page.delegate._grantPermissions(origin, permissions))
    );
  }
  ...
  async doAddInitScript(initScript) {
    for (const page of this.pages()) await page.delegate._updateBootstrapScript();
  }
  ...
  async doExposePlaywrightBinding() {
    for (const page of this.pages()) await page.delegate.exposePlaywrightBinding();
  }
  onClosePersistent() {}
  async clearCache() {
  ...
  async doClose(reason) {
    if (!this._browserContextId) {
      await Promise.all(
        this._wkPages().map(wkPage => wkPage._page.screencast.stopVideoRecording())
      );
