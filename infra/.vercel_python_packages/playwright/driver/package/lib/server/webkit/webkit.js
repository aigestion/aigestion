...
class WebKit extends import_browserType.BrowserType {
  ...
  amendEnvironment(env, userDataDir, isPersistent, options) {
    return {
      ...env,
      CURL_COOKIE_JAR_PATH:
        process.platform === 'win32' && isPersistent
          ? import_path.default.join(userDataDir, 'cookiejar.db')
          : void 0,
    };
  }
  ...
  attemptToGracefullyCloseBrowser(transport) {
    transport.send({
      ...
      params: {},
      id: import_wkConnection.kBrowserCloseMessageId,
    });
  }
  async defaultArgs(options, isPersistent, userDataDir) {
    ...
    if (isPersistent)
      webkitArguments.push(
        `--user-data-dir=${options.channel === 'webkit-wsl' ? await translatePathToWSL(userDataDir) : userDataDir}`
      );
    ...
    if (proxy) {
      if (process.platform === 'darwin') {
        ...
        (process.platform === 'win32' && options.channel === 'webkit-wsl')
      ) {
        webkitArguments.push(`--proxy=${proxy.server}`);
        if (proxy.bypass)
...
async function translatePathToWSL(path2) {
  const { stdout } = await (0, import_spawnAsync.spawnAsync)('wsl.exe', [
    '-d',
    'playwright',
    '--cd',
    '/home/pwuser',
