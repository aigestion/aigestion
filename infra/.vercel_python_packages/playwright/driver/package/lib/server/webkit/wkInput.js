...
class RawKeyboardImpl {
  ...
  async keydown(progress, modifiers, keyName, description, autoRepeat) {
    ...
    if ((0, import_utils.isString)(commands)) commands = [commands];
    await progress.race(
      this._pageProxySession.send('Input.dispatchKeyEvent', {
        type: 'keyDown',
...
class RawMouseImpl {
  ...
  async move(progress, x, y, button, buttons, modifiers, forClick) {
    await progress.race(
      this._pageProxySession.send('Input.dispatchMouseEvent', {
        type: 'move',
        button,
  ...
  async wheel(progress, x, y, buttons, modifiers, deltaX, deltaY) {
    ...
    await progress.race(
      this._page
        .mainFrame()
        .evaluateExpression(`new Promise(requestAnimationFrame)`, { world: 'utility' })
    );
    await progress.race(
      this._pageProxySession.send('Input.dispatchWheelEvent', {
        x,
        y,
        deltaX,
        deltaY,
...
class RawTouchscreenImpl {
  ...
  async tap(progress, x, y, modifiers) {
    await progress.race(
      this._pageProxySession.send('Input.dispatchTapEvent', {
        x,
        y,
        modifiers: toModifiersMask(modifiers),
      })
