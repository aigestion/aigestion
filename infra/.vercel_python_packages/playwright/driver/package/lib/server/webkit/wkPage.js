...
class WKPage {
  constructor(browserContext, pageProxySession, opener) {
    ...
    this._page.on(import_page.Page.Events.FrameDetached, frame =>
    ...
    if (
      ...
      const viewportSize = import_helper.helper.getViewportSizeFromWindowFeatures(
        opener._nextWindowOpenPopupFeatures
      );
  ...
  async _initializePageProxySession() {
    if (this._page.isStorageStatePage) return;
    const promises = [
    ...
    if (contextOptions.javaScriptEnabled === false)
      promises.push(
        this._pageProxySession.send('Emulation.setJavaScriptEnabled', { enabled: false })
      );
  ...
  async _initializeSessionMayThrow(session, resourceTreeHandler) {
    ...
    if (enableFrameSessions) this._initializeFrameSessions(frameTree.frameTree, promises);
    else promises.push(session.send('Console.enable'));
    if (this._page.browserContext.needsPlaywrightBinding())
      promises.push(
        session.send('Runtime.addBinding', { name: import_page.PageBinding.kBindingName })
      );
    if (this._page.needsRequestInterception()) {
      promises.push(session.send('Network.setInterceptionEnabled', { enabled: true }));
      promises.push(session.send('Network.setResourceCachingDisabled', { disabled: true }));
      promises.push(
        session.send('Network.addInterception', { url: '.*', stage: 'request', isRegex: true })
      );
    }
    ...
    promises.push(
      session.send('Network.setExtraHTTPHeaders', {
        headers: (0, import_headers.headersArrayToObject)(
    ...
    promises.push(
      session.send('Page.setTouchEmulationEnabled', { enabled: !!contextOptions.hasTouch })
    );
    if (contextOptions.timezoneId) {
      promises.push(
        session.send('Page.setTimeZone', { timeZone: contextOptions.timezoneId }).catch(e => {
  ...
  _initializeFrameSessions(frame, promises) {
    ...
    if (session) promises.push(session.initialize());
  ...
  didClose() {
    ...
    this._firstNonInitialNavigationCommittedReject(
      new import_errors.TargetClosedError(this._page.closeReason())
    );
  ...
  handleProvisionalLoadFailed(event) {
    ...
    this._page.frameManager.frameAbortedNavigation(
  ...
  async _onTargetCreated(event) {
    ...
    const session = new import_wkConnection.WKSession(
    ...
    if (!targetInfo.isProvisional) {
      ...
      this._page.reportAsNew(
        this._opener?._page,
        pageOrError instanceof import_page.Page ? void 0 : pageOrError
      );
    } else {
  ...
  _addSessionListeners() {
    this._sessionListeners = [
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
        'Page.loadEventFired',
        event => this._page.frameManager.frameLifecycleEvent(event.frameId, 'load')
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
        'Page.domContentEventFired',
        event => this._page.frameManager.frameLifecycleEvent(event.frameId, 'domcontentloaded')
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        event => this._onBindingCalled(event.contextId, event.argument)
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        e => this._onResponseReceived(this._session, e)
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
      ...
      import_eventsHelper.eventsHelper.addEventListener(this._session, 'Network.loadingFailed', e =>
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        e => this._page.frameManager.onWebSocketRequest(e.requestId)
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
        'Network.webSocketHandshakeResponseReceived',
        e =>
          this._page.frameManager.onWebSocketResponse(
            e.requestId,
            e.response.status,
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        e =>
          ...
          this._page.frameManager.onWebSocketFrameSent(
            e.requestId,
            e.response.opcode,
            e.response.payloadData
          )
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        e =>
          e.response.payloadData &&
          this._page.frameManager.webSocketFrameReceived(
            e.requestId,
            e.response.opcode,
            e.response.payloadData
          )
      ...
      import_eventsHelper.eventsHelper.addEventListener(
        ...
        e => this._page.frameManager.webSocketClosed(e.requestId)
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
        'Network.webSocketFrameError',
        e => this._page.frameManager.webSocketError(e.requestId, e.errorMessage)
      ),
    ];
  }
  ...
  _onExecutionContextCreated(contextPayload) {
    ...
    if (!frame) return;
    const delegate = new import_wkExecutionContext.WKExecutionContext(
      this._session,
      contextPayload.id
    );
    let worldName = null;
  ...
  async _onBindingCalled(contextId, argument) {
    ...
    if (!(pageOrError instanceof Error)) {
      ...
      if (context) await this._page.onBindingCalled(argument, context);
    }
  }
  ...
  _onConsoleRepeatCountUpdated(event) {
    if (this._lastConsoleMessage) {
      ...
      for (let i = count; i < event.count; ++i)
        this._page.addConsoleMessage(
          null,
          derivedType,
          handles,
          location,
          handles.length ? void 0 : text
        );
      this._lastConsoleMessage.count = event.count;
    }
  }
  _onDialog(event) {
    this._page.browserContext.dialogManager.dialogDidOpen(
      new dialog.Dialog(
        this._page,
        event.type,
        event.message,
        async (accept, promptText) => {
          if (event.type === 'beforeunload' && !accept)
            this._page.frameManager.frameAbortedNavigation(
              this._page.mainFrame()._id,
              'navigation cancelled by beforeunload dialog'
            );
  ...
  static async _setEmulateMedia(
    ...
    const promises = [];
    promises.push(
      session.send('Page.setEmulatedMedia', { media: mediaType === 'no-override' ? '' : mediaType })
    );
    ...
    promises.push(
      session.send('Page.overrideUserPreference', { name: 'PrefersColorScheme', value: appearance })
    );
    ...
    promises.push(
      session.send('Page.overrideUserPreference', { name: 'PrefersContrast', value: contrastWk })
    );
    await Promise.all(promises);
  }
  ...
  async _updateViewport() {
    ...
    await Promise.all(promises);
    if (
      !this._browserContext._browser?.options.headful &&
      (import_hostPlatform.hostPlatform === 'ubuntu22.04-x64' ||
        import_hostPlatform.hostPlatform.startsWith('debian12'))
    )
  ...
  async _initializeVideoRecording() {
    ...
    if (videoOptions) await screencast.startVideoRecording(videoOptions);
  }
  ...
  async takeScreenshot(progress, format, documentRect, viewportRect, quality, fitsViewport, scale) {
    ...
    if (format === 'jpeg')
      buffer = import_utilsBundle.jpegjs.encode(
        import_utilsBundle.PNG.sync.read(buffer),
        quality
      ).data;
    return buffer;
  }
  ...
  _onRequest(session, event, intercepted) {
    ...
    const frame = redirectedFrom
      ...
      : this._page.frameManager.frame(event.frameId);
    if (!frame) return;
    const isNavigationRequest = event.type === 'Document';
    const documentId = isNavigationRequest ? event.loaderId : void 0;
    const request = new import_wkInterceptableRequest.WKInterceptableRequest(
      session,
      frame,
      event,
      redirectedFrom,
  ...
  _handleRequestRedirect(request, requestId, responsePayload, timestamp) {
    ...
    response._requestFinished(
      responsePayload.timing
        ? import_helper.helper.secondsToRoundishMillis(timestamp - request._timestamp)
        : -1
    );
    this._requestIdToRequest.delete(requestId);
    ...
    if (response) {
      ...
      response._securityDetailsFinished({
        protocol: isLoadedSecurely(response.url(), response.timing())
          ? event.metrics?.securityConnection?.protocol
          : void 0,
      ...
      response._requestFinished(
        import_helper.helper.secondsToRoundishMillis(event.timestamp - request._timestamp)
      );
    } else {
      request.request.setRawRequestHeaders(null);
    }
  ...
  async _initializeImpl() {
    this._sessionListeners = [
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
        'Console.messageAdded',
        event => this._page._onConsoleMessage(event)
      ),
      import_eventsHelper.eventsHelper.addEventListener(
        this._session,
        'Console.messageRepeatCountUpdated',
        event => this._page._onConsoleRepeatCountUpdated(event)
      ),
    ];
