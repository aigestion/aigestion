...
function serializeValue(value, handleSerializer) {
  return innerSerializeValue(
    value,
    handleSerializer,
    { lastId: 0, visited: /* @__PURE__ */ new Map() },
    []
  );
}
...
function innerSerializeValue(value, handleSerializer, visitorInfo, accessChain) {
  ...
  if (isError(value)) return { e: { n: value.name, m: value.message, s: value.stack || '' } };
...
const constructorToTypedArrayKind = new Map(
