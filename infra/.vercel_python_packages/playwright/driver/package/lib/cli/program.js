...
function printInstalledBrowsers(browsers2) {
  ...
  for (const browser of browsers2) browserPaths.add(browser.browserPath);
  console.log(`  Browsers:`);
  for (const browserPath of [...browserPaths].sort()) console.log(`    ${browserPath}`);
  console.log(`  References:`);
  const references = /* @__PURE__ */ new Set();
  for (const browser of browsers2) references.add(browser.referenceDir);
  for (const reference of [...references].sort()) console.log(`    ${reference}`);
}
function printGroupedByPlaywrightVersion(browsers2) {
  const dirToVersion = /* @__PURE__ */ new Map();
  for (const browser of browsers2) {
    if (dirToVersion.has(browser.referenceDir)) continue;
...
import_utilsBundle.program
  ...
  .action(async function (args, options) {
    ...
    try {
      ...
      if (options.withDeps) await import_server.registry.installDeps(executables, !!options.dryRun);
...
import_utilsBundle.program
  ...
  .action(async function (args, options) {
    try {
      await import_server.registry.installDeps(
        import_server.registry.resolveBrowsers(args, {}),
        !!options.dryRun
      );
    } catch (e) {
...
commandWithOpenOptions('pdf <url> <filename>', 'save page as pdf', [
  [
    '--paper-format <format>',
    'paper format: Letter, Legal, Tabloid, Ledger, A0, A1, A2, A3, A4, A5, A6',
  ],
...
import_utilsBundle.program
  ...
  .action(function (trace, options) {
    ...
    if (options.port !== void 0 || options.host !== void 0)
      ...
      (0, import_traceViewer.runTraceViewerApp)(trace, options.browser, openOptions, true).catch(
        logErrorAndExit
      );
  })
  .addHelpText(
...
async function launchContext(options, extraOptions) {
  ...
  if (options.saveHar) {
    contextOptions.recordHar = {
      path: import_path.default.resolve(process.cwd(), options.saveHar),
      mode: 'minimal',
    };
...
async function openPage(context, url) {
  ...
  if (url) {
    if (import_fs.default.existsSync(url)) url = 'file://' + import_path.default.resolve(url);
    else if (
      !url.startsWith('http') &&
      !url.startsWith('file://') &&
...
async function codegen(options, url) {
  ...
  const tracesDir = import_path.default.join(
    import_os.default.tmpdir(),
    `playwright-recorder-trace-${Date.now()}`
  );
  const { context, browser, launchOptions, contextOptions, closeBrowser } = await launchContext(
    options,
    {
      headless: !!process.env.PWTEST_CLI_HEADLESS,
      executablePath: process.env.PWTEST_CLI_EXECUTABLE_PATH,
...
async function maybeSetupTestHooks(browser, closeBrowser, donePromise) {
  if (!process.env.PWTEST_CLI_IS_UNDER_TEST) return;
  const logs = [];
...
function commandWithOpenOptions(command, description, options) {
  ...
  return result
    ...
    .option(
      ...
      'use the specified user data directory instead of a new context'
    )
    .option(
      '--viewport-size <size>',
